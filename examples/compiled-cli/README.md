...
> **This directory is a snapshot of what `bun create @bastani/atomic-cli`
> produces** with the `standalone-cli` template. Read it to see the shape
> a fresh project gets; if you want one of your own, run the create
> command instead of copying these files manually:
>
> ```bash
...
```bash
bun run build
./dist/compiled-cli hello --prompt "say hi"
```

The output `./dist/compiled-cli` is ~100 MB and includes the bun runtime —
