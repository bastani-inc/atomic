...
class Driver {
	...
	async waitFor(predicate: (report: Report) => boolean, from = 0, timeoutMs = 10_000): Promise<Report> {
		...
		return new Promise<Report>((resolve, reject) => {
			...
			const timeout = setTimeout(() => {
				this.waiters.delete(check);
				reject(new Error(`Timed out waiting for report: ${JSON.stringify(this.reports.slice(-8))}; stderr=${this.stderr.slice(-2000)}`));
			}, timeoutMs);
	...
	async waitForState(predicate: (report: Report) => boolean, from = 0): Promise<Report> {
		...
		throw new Error(`Timed out waiting for state: ${JSON.stringify(this.reports.filter((report) => report.type === "state").slice(-8).map((report) => ({ modelProvider: report.modelProvider, modelId: report.modelId, fallback: report.modelFallbackReason })))}`);
	}
...
serialTest("isolated Ctrl+P cycle clears stale fallback and accepts the next prompt", async () => {
	...
	const server = createServer((_request, response) => {
		requests += 1;
		const message = {
			id: "msg_cycle", type: "message", role: "assistant", status: "completed",
			content: [{ type: "output_text", text: "cycle recovered", annotations: [] }],
		};
		const events = [
			{ type: "response.output_item.added", output_index: 0, item: { ...message, status: "in_progress", content: [] } },
			{ type: "response.output_text.delta", output_index: 0, content_index: 0, item_id: message.id, delta: "cycle recovered" },
			{ type: "response.output_item.done", output_index: 0, item: message },
			{ type: "response.completed", response: {
				id: "resp_cycle", status: "completed", output: [message],
				usage: { input_tokens: 1, input_tokens_details: { cached_tokens: 0 }, output_tokens: 2, total_tokens: 3 },
			} },
		];
	...
	writeFileSync(join(agentDir, "models.json"), JSON.stringify({ providers: { recovery: {
		baseUrl, apiKey: "test-key", api: "openai-responses",
		models: ["recovery-one", "recovery-two"].map((id) => ({
			id, name: id, reasoning: false, input: ["text"],
			cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 16_000, maxTokens: 1024,
		})),
	} } }));
	...
	persisted.appendMessage({
		role: "assistant", content: [{ type: "text", text: "persisted stale response" }],
		api: "anthropic-messages", provider: removedProvider, model: removedModel,
	...
	const driver = new Driver([
		"--session", sessionFile, "--no-extensions", "--no-skills", "--no-prompt-templates", "--no-themes", "--offline", "--approve",
	], {
	...
	try {
		...
		const locked = await driver.waitForState((state) => state.modelFallbackReason === "configured-provider-unsupported", readyIndex);
		assert.equal(locked.modelProvider, "unknown");
		assert.deepEqual(driver.reports.filter((report) => report.type === "warning").map((report) => report.message), [warning]);

		const beforeCycle = driver.reports.length;
		driver.send("input", "\x10");
		const cycled = await driver.waitForState((state) => state.modelProvider === "recovery" && state.modelFallbackReason === undefined, beforeCycle);
		assert.match(cycled.modelId ?? "", /^recovery-/u);

		driver.send("input", "cycle prompt");
		driver.send("input", "\r");
		await driver.waitFor((report) => report.type === "session_event" && report.eventType === "agent_start", beforeCycle);
		await driver.waitFor((report) => report.type === "session_event" && report.eventType === "agent_end", beforeCycle);
		const final = await driver.waitForState((state) => state.output?.includes("cycle recovered") === true, beforeCycle);
