...
test("fresh DBOS resumes an active root-child-grandchild with composed scope and exactly-once tool", async () => {
  ...
  const leafGated = new Promise<void>((resolve) => { reportLeafGated = resolve; });
  ...
  const physicalStages = firstBackend.listCheckpoints(rootRunId).filter(
    (checkpoint): checkpoint is DurableStageCheckpoint => checkpoint.kind === "stage",
  );
  const middleBoundaryCheckpoint = physicalStages.find((checkpoint) =>
    checkpoint.topology?.boundary?.child.runId === firstMiddle.id
    && checkpoint.topology.boundary.event === "start"
  )!;
  const leafBoundaryCheckpoint = physicalStages.find((checkpoint) =>
    checkpoint.topology?.boundary?.child.runId === firstLeaf.id
    && checkpoint.topology.boundary.event === "start"
  )!;
  ...
  const toolCheckpoint = firstBackend.listCheckpoints(rootRunId).find((checkpoint) =>
    checkpoint.kind === "tool" && checkpoint.name === "leaf-side-effect"
  );
  ...
  const freshLeafBoundaryCheckpoint = freshBackend.listCheckpoints(rootRunId).find((checkpoint) =>
    checkpoint.kind === "stage" && checkpoint.topology?.boundary?.child.runId === freshLeaf.id
  );
  assert.equal(freshLeafBoundaryCheckpoint?.kind === "stage" ? freshLeafBoundaryCheckpoint.replayKey : undefined, composedScope);
  assert.equal(
