...
test("modern result requires its own nonempty identity to match terminal status", async () => {
	...
	const watcher = createResultWatcher(
		...
		resultsDir,
		60_000,
		{ statusRecheckBaseMs: 5, timers: manual.timers },
	);
...
test("explicit negative local acknowledgement retries while synchronous observation remains compatible", async () => {
	...
	const watcher = createResultWatcher(
		...
		resultsDir,
		60_000,
		{ deliveryRetryBaseMs: 10, timers: manual.timers },
	);
	watcher.primeExistingResults();
	await manual.runNext(0);
...
test("prime and rescan activity cannot bypass delivery backoff", async () => {
	...
	fs.writeFileSync(
		...
		JSON.stringify({ id: "backoff", sessionId: "session", intercomTarget: "missing" }),
	);
...
test("identical result aliases coalesce while completed conflicting aliases are quarantined", async () => {
	...
	const events = {
		on: () => () => {},
		emit(event: string) {
			if (event === SUBAGENT_ASYNC_COMPLETE_EVENT) deliveries += 1;
		},
	};
	...
	fs.writeFileSync(
		path.join(resultsDir, "summary-conflict.json"),
		JSON.stringify({ ...identical, summary: "different" }),
	);
	watcher.primeExistingResults();
	await manual.runNext(0);
	fs.writeFileSync(
		path.join(resultsDir, "result-conflict.json"),
		JSON.stringify({
			...identical,
	...
	const retainedContents = retained
		.map((file) => fs.readFileSync(path.join(resultsDir, ".undelivered", file), "utf-8"))
		.join("\n");
...
test("in-flight aliases with different targets or payloads never join another watcher claim", async () => {
	...
	const original = {
		...
		sessionId: "session",
		summary: "one",
		intercomTarget: "parent-a",
	};
	fs.writeFileSync(path.join(resultsDir, "original.json"), JSON.stringify(original));
	const first = createResultWatcher({ events }, createState(), resultsDir, 60_000, {
		timers: manual.timers,
		intercomTimeoutMs: 10_000,
	});
	...
	fs.writeFileSync(
		path.join(resultsDir, "target-conflict.json"),
		JSON.stringify({ ...original, intercomTarget: "parent-b" }),
	);
