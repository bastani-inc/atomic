...
describe("ctx.tool recoverable failures", () => {
  test("returns a typed failure after retries while the workflow continues", async () => {
    ...
    const definition = workflow({
      ...
      run: async (ctx) => {
        const outcome = await ctx.tool("check", {}, async () => {
          attempts += 1;
  ...
  test("replays the durable failed outcome without rerunning the callback", async () => {
    ...
    const definition = workflow({
      ...
      run: async (ctx) => {
        const outcome = await ctx.tool("check", { suite: "unit" }, async () => {
          callbackCalls += 1;
    ...
    assert.deepEqual(outcomes.map((outcome) => ({ ...outcome })), [
      { ok: false, error: { name: "Error", message: "still red", exitCode: 1, stderr: "failed test" }, attempts: 1, cached: false },
      { ok: false, error: { name: "Error", message: "still red", exitCode: 1, stderr: "failed test" }, attempts: 1, cached: true },
    ]);
  ...
  test("keeps a recoverable failure node failed in completed durable inspection", async () => {
    ...
    const definition = workflow({
      ...
      outputs: { done: Type.Boolean() },
      run: async (ctx) => {
        await ctx.tool("red-check", {}, async () => {
  ...
  test("retries process errors whose message contains cancellation words", async () => {
    ...
    assert.equal(result.toolNodes?.[0]?.status, "failed");
    assert.equal(backend.listCheckpoints(result.runId).some((entry) => entry.kind === "tool" && entry.name === "cancelled-command-text"), true);
  });
  ...
  for (const maxAttempts of [0, -1, 1.5]) {
    test(`rejects invalid maxAttempts ${maxAttempts} without invoking or checkpointing`, async () => {
      ...
      const definition = workflow({
        ...
        outputs: {},
        run: async (ctx) => {
          await ctx.tool("invalid-attempts", {}, async () => {
            callbackCalls += 1;
      ...
      assert.equal(downstreamRan, false);
      assert.equal(backend.listCheckpoints(result.runId).some((entry) => entry.kind === "tool"), false);
    });
  }
  ...
  test("supports explicit repair evidence and a bounded rerun", async () => {
    ...
    const definition = workflow({
      ...
      run: async (ctx) => {
        for (let iteration = 1; iteration <= 2; iteration += 1) {
          const outcome = await ctx.tool("run-check", { iteration }, async () => {
            checks += 1;
            if (iteration === 1) {
    ...
    assert.deepEqual(result.toolNodes?.map((node) => node.status), ["failed", "completed"]);
  ...
  test("redacts and UTF-8 byte-bounds persisted process output", async () => {
    ...
    const definition = workflow({
      ...
      outputs: { done: Type.Boolean() },
      run: async (ctx) => {
        const outcome = await ctx.tool("safe-check", {}, async () => {
    ...
    const checkpoint = backend.listCheckpoints(result.runId).find((entry) => entry.kind === "tool" && entry.name === "safe-check");
  ...
  test("preserves fields from a non-Error callback rejection after retries", async () => {
    ...
    const definition = workflow({
      ...
      outputs: { done: Type.Boolean() },
      run: async (ctx) => {
        const outcome = await ctx.tool("structured-check", {}, async () => {
          throw {
            name: "CommandFailure",
  ...
  test("keeps durable storage failures throwing in return mode", async () => {
    ...
    await assert.rejects(
      () => tool("check", {}, async () => { throw new Error("expected check failure"); }, { failureMode: "return" }),
