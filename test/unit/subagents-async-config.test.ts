...
import { writeAsyncRunnerConfig } from "../../packages/subagents/src/runs/background/async-execution.js";
...
describe("async runner config", () => {
	test("writes config files with owner-only permissions", () => {
		const cfgPath = writeAsyncRunnerConfig(
			{ nestedRoute: { capabilityToken: "secret-token" } },
			`mode-test-${Date.now()}`,
		);
