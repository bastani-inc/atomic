...
test.sequential("fresh root resume republishes and answers a pending depth-2 durable ctx.ui node", async () => {
  ...
  const sibling = workflow({
    name: "pending-ui-sibling", description: "", inputs: { branch: Type.String() }, outputs: { branch: Type.String() },
    run: async (ctx) => {
  ...
  const grandchild = workflow({
    name: "pending-ui-grandchild", description: "", inputs: {}, outputs: { answer: Type.String() },
    run: async (ctx) => {
  ...
  const activeChild = workflow({
    name: "pending-ui-active-child", description: "", inputs: {}, outputs: { answer: Type.String() },
    run: async (ctx) => {
      count("active-body");
  ...
  const root = workflow({
    name: "pending-ui-root", description: "", inputs: {}, outputs: { answer: Type.String() },
    run: async (ctx) => {
      count("root-body");
  ...
  const grandchildRun = writerStore.runs().find((candidate) => candidate.id === grandchildBoundary.workflowChildRun?.runId)!;
  ...
  assert.deepEqual([...calls], [
    ["root-body", 1], ["root-before", 1],
    ["sibling:alpha", 1], ["stage:sibling-alpha", 1],
    ["sibling:beta", 1], ["stage:sibling-beta", 1],
    ["active-body", 1], ["grandchild-body", 1],
    ["stage:grandchild-owner", 1], ["grandchild-once", 1],
  ]);
  ...
  const terminalBoundariesBeforeResume = [...persisted.state.steps.keys()]
    .filter((key) => key.includes("boundary-terminal:") && key.endsWith(":completed")).length;
  ...
  const resumeDeps = {
    ...
    baseRunOpts: {
      ...
      adapters: { complete: { complete } },
      onRunEnd: () => { rootTerminalCallbacks += 1; },
    },
  };
  ...
  assert.deepEqual(graph.stages.map((stage) => stage.name), [
    "sibling-owner", "sibling-owner", "grandchild-owner", "input",
  ]);
  ...
  const terminalBoundariesAfterResume = [...persisted.state.steps.keys()]
    .filter((key) => key.includes("boundary-terminal:") && key.endsWith(":completed")).length;
  ...
  assert.deepEqual(catalog.map((entry) => entry.workflowId), [ROOT_ID]);
  const catalogRuns = completedWorkflowRunSnapshots(catalogBackend, catalog[0]!);
  assert.equal(catalogRuns.length, 5);
  assert.deepEqual(
    graphShape({ runs: catalogRuns, notices: [], version: 1 }, ROOT_ID),
    terminalGraph,
  );
