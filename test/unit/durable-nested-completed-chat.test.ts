...
function boundaryTopology(
  ...
  return event === "start"
    ? {
        version: 1, stageId: fixture.boundaryId, parentIds: [], sourceOrder: 0,
        status: "running", run: { runId: fixture.rootId, runName: "chat-root" },
        boundary: { ...identity, event: "start", status: "running" },
      }
    : {
        version: 1, stageId: fixture.boundaryId, parentIds: [], sourceOrder: 0,
        status: "completed", run: { runId: fixture.rootId, runName: "chat-root" },
...
function createTranscript(directory: string, id: string): string {
  ...
  writeFileSync(sessionFile, [
    ...
    JSON.stringify({ type: "message", id: "message", parentId: null, timestamp: new Date().toISOString(), message: { role: "user", content: "prior" } }),
  ].join("\n") + "\n");
  return sessionFile;
}
...
test("fresh DBOS root-only catalog opens nested post-mortem chat detached from execution", async () => {
  ...
  try {
    ...
    assert.deepEqual(catalog.completed.map((entry) => entry.workflowId), [fixture.rootId]);
    assert.deepEqual(listOpenableCompletedWorkflows(fresh).map((entry) => entry.workflowId), [fixture.rootId]);
    ...
    const opened = openCompletedDurableWorkflow(fixture.rootId, {
    ...
    assert.deepEqual(freshStore.runs().find((run) => run.id === fixture.rootId), rootBefore);
    assert.deepEqual(freshStore.runs().find((run) => run.id === fixture.childId), childBefore);
    registry.clear();
  } finally {
...
test("fresh DBOS hides malformed completed child output and registers no chat attachment", async () => {
  ...
  try {
    ...
    const opened = openCompletedDurableWorkflow(fixture.rootId, {
      ...
      store: freshStore,
      stageControlRegistry: registry,
      adapters: { agentSession: { async create() { attachments += 1; return mockSession(); } } },
    }, catalog.completed);
    assert.deepEqual(opened, {
      ok: false,
      reason: "stale",
      message: "Completed workflow malforme is stale or missing durable checkpoint/session data and cannot be opened.",
    });
