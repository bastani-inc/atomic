...
function immutableRunShape(store: ReturnType<typeof createStore>, runId: string) {
  return store.runs().find((candidate) => candidate.id === runId)!.stages.map((stage) => ({
    id: stage.id,
    name: stage.name,
...
test.sequential("fresh DBOS restores a cache-hit twig through arbitrarily stacked child scopes", async () => {
  ...
  const twigCompleted = new Promise<void>((resolve) => { reportTwigCompleted = resolve; });
  let releaseLeaf = (): void => {};
  const leafGate = new Promise<void>((resolve) => { releaseLeaf = resolve; });

  const twig = workflow({
    name: "round2-twig", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      twigRuns += 1;
  ...
  const leaf = workflow({
    name: "round2-leaf", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const middle = workflow({
    name: "round2-middle", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const root = workflow({
    name: "round2-root", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const priorTwigGraph = expandWorkflowGraph(writerStore.snapshot(), rootRunId).stages
    .filter((stage) => stage.workflowGraphTarget.runId === twigRun.id)
    .map((stage) => ({
      id: stage.id,
      name: stage.name,
      parentIds: [...stage.parentIds],
  ...
  for (const runId of runIds) {
    ...
    assert.deepEqual(immutableRunShape(freshStore, runId).slice(0, priorShapes.get(runId)!.length), priorShapes.get(runId));
  }
  ...
  assert.deepEqual(twigStages.map((stage) => ({
    id: stage.id,
    name: stage.name,
    parentIds: [...stage.parentIds],
    target: { ...stage.workflowGraphTarget },
