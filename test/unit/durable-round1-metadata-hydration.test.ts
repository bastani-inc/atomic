...
test("fresh DBOS completed inspection retains handled failed stage ancestry into a child descendant", async () => {
  ...
  const shape = (graph: typeof liveGraph) => graph.stages.map((stage) => ({
    id: stage.id,
    name: stage.name,
    status: stage.status,
  ...
  assert.deepEqual(hydratedGraph.stages.map((stage) => stage.name), ["handled-failure", "descendant"]);
...
test("answered prompt-node topology survives fresh DBOS active-child resume without asking again", async () => {
  ...
  const promptWriteGate = new Promise<void>((resolve) => { releasePromptWrite = resolve; });
  let reportPromptWrite = (): void => {};
  const promptWriteStarted = new Promise<void>((resolve) => { reportPromptWrite = resolve; });
  ...
  const sdk: ReturnType<typeof createMockSdk> = {
    ...
    async recordStepOutput(workflowId, stepName, output) {
      const envelope = typeof output === "object" && output !== null && !Array.isArray(output)
        ? output as Record<string, WorkflowSerializableValue>
        : undefined;
      const topology = typeof envelope?.["topology"] === "object" && envelope["topology"] !== null
        ? envelope["topology"] as Record<string, WorkflowSerializableValue>
        : undefined;
      if (holdPromptWrite && envelope?.["kind"] === "stage" && envelope["name"] === "input"
  ...
  const childStarted = new Promise<void>((resolve) => { reportChildStarted = resolve; });
  const child = workflow({
  ...
  const promptCheckpoints = firstBackend.listCheckpoints(runId).filter((checkpoint) =>
    checkpoint.kind === "stage" && checkpoint.name === "input" && checkpoint.topology?.stageId === originalPrompt.id
  );
