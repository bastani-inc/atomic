...
function childRun(): DurableStageRunTopology {
  return {
    runId: CHILD, runName: "round3-lifecycle-child", parentRunId: ROOT,
    parentStageId: BOUNDARY_ID, rootRunId: ROOT,
  };
}
...
function topology(
  ...
  if (event === "start") {
    ...
    return {
      version: 1, stageId: BOUNDARY_ID, parentIds: [], sourceOrder: 0,
      status, run: rootRun(), boundary: { ...identity, event, status },
    };
  }
...
function seedLifecycle(terminals: readonly TerminalSeed[]): ReturnType<typeof createMockSdk> {
  ...
  seedMockCheckpoint(sdk, ROOT, {
    kind: "stage", workflowId: ROOT, checkpointId: "boundary-start", name: "workflow:child",
    replayKey: KEY, completedAt: 1, topology: topology("start", "running"),
  });
  seedMockCheckpoint(sdk, ROOT, {
    kind: "stage", workflowId: ROOT, checkpointId: `${KEY}:child-stage`, name: "child-stage",
    replayKey: `${KEY}:stage:child:1`, output: "child-stage-output", completedAt: 2,
    topology: {
      version: 1, stageId: "child-stage", parentIds: [], sourceOrder: 0,
      status: "completed", run: childRun(),
    },
  });
  for (const terminal of terminals) {
    seedMockCheckpoint(sdk, ROOT, {
      kind: "stage", workflowId: ROOT,
      checkpointId: `terminal:${terminal.status}:${terminal.suffix ?? terminal.at}`,
      name: "workflow:child", replayKey: KEY, completedAt: terminal.at, endedAt: terminal.at,
...
test("fresh DBOS uses the latest legal atomic boundary terminal without hybrid child output or links", async () => {
  for (const [name, terminals, expectedStatus, hasChild] of [
    ...
    const opened = openCompletedDurableWorkflow(ROOT, {
      durableBackend: backend, store, stageControlRegistry: createStageControlRegistry(),
    });
...
test("fresh DBOS rejects impossible event/status pairs, malformed outputs, and conflicting repeats", async () => {
  ...
  for (const status of ["running", "blocked", "paused"] as const) {
    ...
    seedMockCheckpoint(sdk, ROOT, {
      kind: "stage", workflowId: ROOT, checkpointId: `invalid-terminal-${status}`,
      name: "workflow:child", replayKey: KEY, completedAt: 3, topology: raw,
    } as unknown as DurableCheckpoint);
    invalidCases.push([`terminal-${status}`, sdk]);
  }
  for (const [name, output] of [["null-output", null], ["array-output", []]] as const) {
