...
test("workflow models action lists the host model registry catalog", async () => {
  ...
  try {
    ...
    assert.deepEqual(result.models.map(({ fullId }) => fullId), [
      "provider-a/current",
      "provider-b/alternate",
    ]);
    assert.deepEqual(result.models.map(({ isCurrent }) => isCurrent), [true, false]);
  } finally {
...
test("workflow stage model catalog includes alternatives beyond the current model", async () => {
  ...
  assert.deepEqual(available.map(({ fullId }) => fullId), [
    "provider-a/current",
    "provider-b/alternate",
  ]);
