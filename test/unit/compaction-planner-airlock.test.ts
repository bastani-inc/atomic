...
async function plan(text: string, stopReason: "stop" | "length", protectedLines: number[] = [], lineCount = 40) {
	...
	return planDeletedLineRanges(
		region(lineCount, protectedLines),
		PARAMETERS,
...
test("ranked ranges are clamped to the region bounds", async () => {
	...
	assert.deepEqual(ranges(outcome), [{ start: 1, end: 3 }, { start: 30, end: 40 }]);
});
...
test("re-validating the returned ranges is a no-op", async () => {
	...
	const { validateDeletedRanges } = await import(
		"../../packages/coding-agent/src/core/compaction/deleted-ranges.js"
	);
