...
function fixture(behavior: RunBehavior, names = ["alpha", "beta", "gamma"]) {
	...
	const deps = {
		...
		config: {
			...
			chain: { dynamicFanout: { maxItems: 4 } },
		},
		...
		runtime: {
			runSync: async (
				_cwd: string,
				_agents: ReturnType<typeof agent>[],
				name: string,
				task: string,
				options: RunOptions,
			) => behavior(name, task, options),
			executeAsyncChain: () => {
				throw new Error("unexpected background chain");
			},
			executeAsyncSingle: () => {
				throw new Error("unexpected background single");
			},
...
describe("executor-level foreground chain detached recovery", () => {
	...
	test("static-parallel chain replaces only the detached flat child after registration", async () => {
		...
		assert.deepEqual(
			before.children.map((child) => child.index),
			[0, 1, 2],
		);
		assert.deepEqual(
			before.children.map((child) => child.status),
			["completed", "completed", "detached"],
		);
		recover?.();
		const after = remembered(f);
		assert.deepEqual(
			after.children.map((child) => child.index),
			[0, 1, 2],
		);
	...
	test("dynamic-parallel chain preserves materialized flat indices and replaces the recovered item", async () => {
		...
		const f = fixture(
			(name, task, options) => {
				if (options.index === 0)
					return {
						...result(name, task, 0, "targets"),
						structuredOutput: { items: [{ path: "a.ts" }, { path: "b.ts" }] },
					};
				if (options.index === 2) {
	...
	test("public status shows a detached foreground child and its eventual recovered output", async () => {
		...
		const detached = await f.executor.execute(
			"subagent",
			{ action: "status", id: "recover-status" },
		...
		replaceForegroundRunChild(
			...
			"recover-status-1234",
			0,
			result("alpha", "task", 0, "actual recovered output"),
		);
	...
	test("public status rejects prefixes ambiguous across retained foreground runs", async () => {
		...
		for (const runId of ["retained-one", "retained-two"]) {
			rememberForegroundRun(f.state, {
				...
				mode: "single",
				cwd: f.context.cwd,
				results: [result("alpha", "task", 0, runId)],
			});
		}
		const response = await f.executor.execute(
			"subagent",
			{ action: "status", id: "retained-" },
			new AbortController().signal,
			undefined,
			f.context,
		);
	...
	test("compacts recovered results before registered and early retention", () => {
		...
		rememberForegroundRun(registered.state, {
			runId: "registered",
			mode: "single",
			cwd: "/tmp",
			results: [result("alpha", "task", -2, "waiting", true)],
		...
		rememberForegroundRun(early.state, {
			runId: "early-compacted",
			mode: "single",
			cwd: "/tmp",
			results: [result("alpha", "task", -2, "waiting", true)],
		});
	...
	test("bounds and consumes early detached results", () => {
		...
		assert.equal(
			f.state.foregroundRuns?.get("early-0")?.children[0]?.status,
			"detached",
			"oldest early result was evicted",
		);
