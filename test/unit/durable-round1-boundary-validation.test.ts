...
function boundaryTopology(
  ...
  return event === "start"
    ? {
        version: 1, stageId: "boundary-id", parentIds: ["source-id"], sourceOrder: 1,
        status: "running", run: rootTopology(runId),
        boundary: { ...identity, event: "start", status: "running" },
      }
    : {
        version: 1, stageId: "boundary-id", parentIds: ["source-id"], sourceOrder: 1,
        status: "completed", run: rootTopology(runId),
...
async function runSeededBoundary(caseName: string, options: SeededBoundaryOptions = {}) {
  ...
  seedMockCheckpoint(sdk, runId, {
    ...
    completedAt: 1,
    topology: {
      version: 1, stageId: "source-id", parentIds: [], sourceOrder: 0,
      status: "completed", run: rootTopology(runId),
    },
  });
  seedMockCheckpoint(sdk, runId, {
  ...
  for (const [index, childRun] of childRuns.entries()) {
    seedMockCheckpoint(sdk, runId, {
      ...
      completedAt: 3 + index,
      topology: {
        version: 1, stageId: "child-stage-id", parentIds: [], sourceOrder: 0,
  ...
  const terminalTopology = options.terminalTopology?.(
  ...
  if (options.duplicateTerminal === true) {
    seedMockCheckpoint(sdk, runId, {
      kind: "stage", workflowId: runId,
      checkpointId: `boundary-terminal:${REPLAY_KEY}:duplicate`,
      name: options.terminalName ?? `workflow:${CHILD_NAME}`,
      replayKey: options.terminalReplayKey ?? REPLAY_KEY,
      output: options.output ?? validOutput,
      result: "completed", startedAt: 1, endedAt: 6, completedAt: 6,
      topology: terminalTopology,
    });
  }
...
test("WorkflowChildResult parser accepts only the exact completed/exited discriminated union", () => {
  ...
  ] as const) assert.equal(parseWorkflowChildResult(malformed), undefined);
  ...
  assert.equal(parseWorkflowChildResult({ ...identity, status: "completed", exited: false, outputs: exoticOutputs }), undefined);
});
...
test("malformed cached WorkflowChildResult records fail closed before attach or execution", async () => {
  ...
  for (const [index, value] of malformed.entries()) {
    ...
    const runId = `round1-${caseName}`;
    const output = typeof value === "object" && value !== null && !Array.isArray(value)
    ...
    assert.equal(observed.store.runs().some((candidate) => candidate.parentRunId === runId), false, caseName);
  }
});

test("every reciprocal child ownership field and mixed replay-key group is validated", async () => {
  const caseNames = [
    "missing-parent", "missing-stage", "missing-root",
    "wrong-parent", "wrong-stage", "wrong-root", "mixed",
  ] as const;
