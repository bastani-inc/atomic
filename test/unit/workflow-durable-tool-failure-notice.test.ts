...
import type { ExtensionAPI, PiExecuteContext, PiToolOpts, WorkflowToolArgs } from "../../packages/workflows/src/extension/public-types.js";
...
describe("interactive durable tool failure lifecycle", () => {
  ...
  test("reports one failed notice and reconciliation when a post-admission durable callback throws", async () => {
    ...
    const executeWorkflow = makeExecuteWorkflowTool(runtime, () => undefined, () => undefined);
    ...
    harness.setResponses([
      fauxAssistantMessage(fauxToolCall("workflow", {
        action: "run",
        workflow: "post-admission-tool-failure",
      }, { id: "workflow-call-post-admission" }), { stopReason: "toolUse" }),
      (context) => {
    ...
    assert.equal(
      (persistedRunEnd?.payload["failedToolNode"] as { status?: string } | undefined)?.status,
      "failed",
    );
    const failureCheckpoint = backend.listCheckpoints(runId).find(
      (checkpoint) => checkpoint.kind === "tool" && checkpoint.throwingFailureError !== undefined,
    );
    assert.equal(failureCheckpoint?.kind, "tool");
    assert.equal(failureCheckpoint?.kind === "tool" ? failureCheckpoint.throwingFailureError : undefined, "commit hook rejected docs");
    ...
    const details = cards[0]?.role === "custom"
      ? cards[0].details as WorkflowLifecycleNoticeDetails
      : undefined;
    ...
    assert.equal(providerContext.messages.filter(
      (message) => message.role === "user" && getMessageText(message).includes(`failed (run ${runId}`),
    ).length, 1, "the failed notice must trigger one hidden reconciliation turn");
    ...
    assert.equal(harness.session.messages.filter(
      (message) => message.role === "custom" && message.customType === LIFECYCLE_NOTICE_CUSTOM_TYPE,
    ).length, 1, "status inspection must not duplicate the notice");
    ...
    assert.equal(harness.session.messages.filter(
      (message) => message.role === "custom" && message.customType === LIFECYCLE_NOTICE_CUSTOM_TYPE,
    ).length, 1, "durable catalog inspection must not duplicate the notice");
    ...
    assert.deepEqual(restoredNotices, [], "restoring a historical terminal run must seed dedupe without re-notifying");
  });
  ...
  test("lets nested command failure evidence beat a cancellation-like wrapper", () => {
    const failure = classifyWorkflowFailure(Object.assign(new Error("wrapper aborted"), {
      code: "CANCELLED",
      cause: Object.assign(new Error("commit hook rejected docs"), {
        exitCode: 1,
        stderr: "docs check failed",
