		...
		await withFakeCli(
			...
			async (dir) => {
				...
				const recovered: Array<{
					...
					modelAttempts?: Array<{ exitCode: number }>;
				}> = [];
				...
				const metadata = JSON.parse(fs.readFileSync(actual.artifactPaths.metadataPath, "utf8")) as {
					exitCode: number;
					modelAttempts: Array<{ exitCode: number }>;
				};
		...
		await withFakeCli(
			...
			async (dir) => {
				...
				const first = runSync(dir, [bridgedAgent()], "fake-worker", "A", {
					...
					allowIntercomDetach: true,
					intercomEvents: bus,
					onDetachedExit: () => resolveFirstExit(),
				});
  ...
  console.log(${JSON.stringify(successEvent("parallel child recovered"))});
		...
		await withFakeCli(
			...
			async (dir) => {
				...
				const launch = (index: number, target: string) =>
	...
	test("background-style execution ignores targeted detach requests", async () => {
		await withFakeCliEvent(
			successEvent("background result"),
			80,
			async (dir) => {
				const emitter = new EventEmitter();
				const bus = eventBus(emitter);
