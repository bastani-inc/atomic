...
import {
	createChildEventJournal,
	sanitizeChildEvent,
} from "../../packages/subagents/src/runs/background/async-event-journal.js";
...
test("child event persistence strips cumulative delta snapshots and retains incremental metadata", () => {
	...
	assert.equal(
		"content" in
			(sanitizeChildEvent({ type: "message_update", assistantMessageEvent: { type: "text_end", content: "full" } })
				.assistantMessageEvent as Record<string, unknown>),
		false,
	);
	assert.equal(
		"partialResult" in
			sanitizeChildEvent({ type: "tool_execution_update", partialResult: { content: "cumulative" } }),
		false,
	);
...
test("child event journal bounds telemetry while retaining one full final message and drains", async () => {
	...
	const journal = createChildEventJournal(
		"events.jsonl",
		source,
		{ runId: "run", agent: "worker" },
		{
	...
	assert.equal(
		records.filter((record) => record.type === "turn_start").length,
		1,
		"lifecycle records bypass the telemetry cap",
	);
	...
	assert.ok(
		Buffer.byteLength(chunks.join("")) < 4_000,
		"bounded delta telemetry must not grow with cumulative snapshots",
	);
});
...
test("run-level telemetry cap is shared across journals and nested terminal updates bypass it", async () => {
	...
	const source = { pause() {}, resume() {} };
	const first = createChildEventJournal(
		eventsPath,
		source,
		{ runId: "run", stepIndex: 0, agent: "one" },
		{ maxTelemetryBytes: 500, now: () => 1 },
	);
	const second = createChildEventJournal(
		eventsPath,
		source,
		{ runId: "run", stepIndex: 1, agent: "two" },
		{ maxTelemetryBytes: 500, now: () => 1 },
	);
	for (let i = 0; i < 20; i++) {
	...
	const retry = createChildEventJournal(
		eventsPath,
		source,
		{ runId: "run", stepIndex: 1, agent: "two" },
		{ maxTelemetryBytes: 500, now: () => 2 },
	);
	...
	assert.equal(records.filter((record) => record.type === "subagent.child.telemetry_truncated").length, 1);
	assert.equal(
		records.filter((record) => record.type === "subagent.control").length,
		1,
		"critical writes share the open writer",
	);
	...
	assert.ok(
		Buffer.byteLength(fs.readFileSync(eventsPath)) < 2_000,
		"parallel journals share one bounded telemetry budget",
	);
});
...
test("journal acquired during delayed close hands off without losing or reordering lines", async () => {
	...
	assert.deepEqual(
		writes.map((line) => JSON.parse(line).sequence),
		[1],
	);
...
test("writer reacquisition detects externally appended truncation markers and resets after shrink", async () => {
	...
	const first = createChildEventJournal(
		...
		source,
		{ runId: "run", agent: "one" },
		{ maxTelemetryBytes: 10_000, now: () => 1 },
	);
	...
	const second = createChildEventJournal(
		...
		source,
		{ runId: "run", agent: "two" },
		{ maxTelemetryBytes: 10_000, now: () => 2 },
	);
	second.append({
		type: "message_update",
		assistantMessageEvent: { type: "text_delta", delta: "blocked-after-external-marker" },
	});
	await second.close();
	assert.doesNotMatch(fs.readFileSync(eventsPath, "utf-8"), /blocked-after-external-marker/);

	fs.writeFileSync(eventsPath, "", "utf-8");
	const afterShrink = createChildEventJournal(
		eventsPath,
		source,
		{ runId: "run", agent: "three" },
		{ maxTelemetryBytes: 10_000, now: () => 3 },
	);
	afterShrink.append({
		type: "message_update",
		assistantMessageEvent: { type: "text_delta", delta: "accepted-after-shrink" },
	});
...
test("hydration detects same-inode rewrites and truncate-then-grow boundary changes", async () => {
	...
	const equalRewrite = createChildEventJournal(
		eventsPath,
		source,
		{ runId: "run", agent: "equal" },
		{ maxTelemetryBytes: 10_000 },
	);
	equalRewrite.append({
		type: "message_update",
		assistantMessageEvent: { type: "text_delta", delta: "blocked-equal-rewrite" },
	});
	...
	const markerRemoved = createChildEventJournal(
		eventsPath,
		source,
		{ runId: "run", agent: "marker-removed" },
		{ maxTelemetryBytes: 10_000 },
	);
	markerRemoved.append({
		type: "message_update",
		assistantMessageEvent: { type: "text_delta", delta: "accepted-after-same-size-rewrite" },
	});
	...
	const regrown = createChildEventJournal(
		eventsPath,
		source,
		{ runId: "run", agent: "regrown" },
		{ maxTelemetryBytes: 10_000 },
	);
...
test("sanitized delta persistence grows linearly below the cap", async () => {
	const persistedSize = async (count: number, file: string): Promise<number> => {
		...
		const journal = createChildEventJournal(
			file,
			{ pause() {}, resume() {} },
			{ runId: "linear", agent: "worker" },
			{
				maxTelemetryBytes: 10_000_000,
				createWriteStream: () => stream,
				now: () => 1,
			},
		);
		for (let i = 0; i < count; i++)
			journal.append({
				type: "message_update",
				message: { content: "x".repeat(i + 1) },
				assistantMessageEvent: { type: "text_delta", delta: "x", partial: { content: "x".repeat(i + 1) } },
			});
		await journal.close();
...
`,
	...
	const result = await withBunRuntime(() =>
		runPiStreaming([], root, outputPath, undefined, undefined, fakeCli, undefined, undefined, {
			eventsPath,
			runId: "e2e",
			stepIndex: 0,
			agent: "worker",
...
test("raw child stdout and stderr share the bounded telemetry budget", async () => {
	...
	fs.writeFileSync(
		fakeCli,
		`for (let i = 0; i < 1500; i++) console.error("e".repeat(1024));\nconsole.log(JSON.stringify({ type: "message_end", message: { role: "assistant", content: "FINAL" } }));`,
	);
	const result = await withBunRuntime(() =>
		runPiStreaming([], root, path.join(root, "output.log"), undefined, undefined, fakeCli, undefined, undefined, {
			eventsPath,
			runId: "raw",
			stepIndex: 0,
			agent: "worker",
...
test("asynchronous event stream failure is non-fatal and releases paused sources", async () => {
	...
	const createWriteStream = (): JsonlWriteStream => {
		...
		return {
			...
			on(event, listener) {
				if (event === "error" && failing) errorListener = listener;
				return this;
			},
			once() {
				return this;
