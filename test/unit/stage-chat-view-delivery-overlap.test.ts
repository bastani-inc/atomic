...
test("settling one of two leases sharing a reconciled generation keeps Working", async () => {
  ...
  try {
    ...
    assert.equal(
      isWorking(view),
      true,
      "lease B still owns the reconciled generation, so Working must stay",
    );
    assert.equal(view._hasAnimationTick, true);
    assert.equal(
      renderRequests,
      0,
      "dropping an internal map entry must not request a render on its own",
    );
...
test("unknown and stale settlements cannot clear a shared reconciled generation", async () => {
  const { handle, emit, emitDeliveryActivity, deliveryActivity } = makeHandle(
    undefined,
    [],
    "completed",
  );
...
test("the terminal fence returns only after the last shared-generation lease settles", async () => {
  ...
  try {
    ...
    assert.equal(
      isWorking(view),
      false,
      "a start after the last retained lease settled must stay fenced",
    );
    assert.equal(view._hasAnimationTick, false);
  } finally {
    view.dispose();
...
test("distinct lease generations keep their existing independent settlement", async () => {
  ...
  try {
    ...
    assert.equal(
      isWorking(view),
      false,
      "settling the current distinct generation must still clear it",
    );
    assert.equal(view._hasAnimationTick, false);
