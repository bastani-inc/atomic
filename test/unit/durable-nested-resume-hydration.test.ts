...
import { store as workflowStore } from "../../packages/workflows/src/shared/store.js";
...
test("fresh DBOS backend resumes an active nested child with stable identity and exactly-once work", async () => {
  ...
  const secondStageGate = new Promise<void>((resolve) => { releaseSecondStage = resolve; });
  let reportSecondStageStarted = (): void => {};
  const secondStageStarted = new Promise<void>((resolve) => { reportSecondStageStarted = resolve; });
  ...
  const firstRunPromise = run(parent, {}, {
    runId,
    store: firstStore,
  ...
  const resumed = await run(parent, {}, {
    runId,
    store: freshStore,
...
test.sequential("colliding child-local stage IDs and names require an exact virtual target", () => {
  ...
  workflowStore.recordRunStart({
    id: rootId, name: "root", inputs: {}, status: "completed", startedAt: 1, endedAt: 2,
    stages: [
      {
        id: "boundary-first", name: "workflow:child", status: "completed", parentIds: [],
        startedAt: 1, endedAt: 2, toolEvents: [], attachable: false,
        workflowChild: { alias: "child", workflow: "child", runId: firstRunId, status: "completed", outputs: {} },
      },
      {
        id: "boundary-second", name: "workflow:child", status: "completed", parentIds: [],
        startedAt: 1, endedAt: 2, toolEvents: [], attachable: false,
  ...
  for (const childRunId of [firstRunId, secondRunId]) {
    workflowStore.recordRunStart({
      ...
      rootRunId: rootId,
      stages: [{
        id: "shared-local-id", name: "shared-name", status: "completed", parentIds: [],
        startedAt: 1, endedAt: 2, toolEvents: [], attachable: false,
      }],
    });
  }
...
test("fresh DBOS completed inspection preserves repeated parallel multi-level topology", async () => {
  ...
  const leaf = workflow({
    name: "topology-leaf", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => ({ value: await ctx.stage("leaf-stage").complete("leaf") }),
  });
  const middle = workflow({
    name: "topology-middle", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const sibling = workflow({
    name: "topology-sibling", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => ({ value: await ctx.stage("sibling-stage").complete("sibling") }),
  });
  const parent = workflow({
    name: "topology-parent", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const result = await run(parent, {}, {
    runId, store: liveStore, durableBackend: backend, adapters: { complete },
  });
  await backend.flush();
  ...
  assert.deepEqual(completedEntries.map((entry) => entry.workflowId), [runId]);
  ...
  const graphShape = (graph: typeof liveGraph) => graph.stages.map((stage) => ({
    id: stage.id,
    name: stage.name,
  ...
  assert.deepEqual(hydratedGraph.stages.map((stage) => stage.name), [
    "parent-before",
    "middle-before",
    "leaf-stage",
    "leaf-stage",
...
test("fresh DBOS replay caches a completed child before downstream parent work", async () => {
  ...
  const childComplete = new Promise<void>((resolve) => { reportChildComplete = resolve; });
  let releaseParent = (): void => {};
  const parentGate = new Promise<void>((resolve) => { releaseParent = resolve; });

  const child = workflow({
    name: "cached-before-downstream-child", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      childCalls += 1;
  ...
  const parent = workflow({
    name: "cached-before-downstream-parent", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      const nested = await ctx.workflow(child);
      if (nested.exited) throw new Error("child exited");
      if (holdAfterChild) {
  ...
  const firstController = new AbortController();
  const firstPromise = run(parent, {}, {
    runId, store: firstStore, durableBackend: firstBackend,
    signal: firstController.signal, adapters: { complete: { complete: async (text) => text } },
  });
  await childComplete;
  await firstBackend.flush();
  ...
  const freshStore = createStore();
  const resumed = await run(parent, {}, {
    runId, store: freshStore, durableBackend: fresh,
  ...
  const boundaryTerminals = fresh.listCheckpoints(runId).filter((checkpoint) =>
    checkpoint.kind === "stage" && checkpoint.checkpointId.includes("boundary-terminal:")
  );
...
test("durable boundary records preserve failed and exited child lifecycle without stale links", async () => {
  ...
  const failing = workflow({
    name: "status-failing-child", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => ({ value: await ctx.stage("child-fails").complete("fail-child") }),
  });
  const exited = workflow({
    name: "status-exited-child", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => ctx.exit({ status: "skipped", reason: "intentional", outputs: { value: "partial" } }),
  });
  const parent = workflow({
    name: "status-parent", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      try {
  ...
  await backend.flush();
  assert.equal(result.status, "completed");
  const terminals = backend.listCheckpoints(runId).filter((checkpoint): checkpoint is DurableStageCheckpoint =>
    checkpoint.kind === "stage" && checkpoint.topology?.boundary?.event === "terminal"
  );
  assert.deepEqual(terminals.map((checkpoint) => checkpoint.topology!.status), ["failed", "completed"]);
  const exitedOutput = terminals[1]?.output;
  const exitedRecord = typeof exitedOutput === "object" && exitedOutput !== null && !Array.isArray(exitedOutput)
    ? exitedOutput as Readonly<Record<string, import("../../packages/workflows/src/shared/types.js").WorkflowSerializableValue>>
    : undefined;
...
test("workflow exit persists one skipped child boundary without a child attachment", async () => {
  const backend = new DbosDurableBackend(createMockSdk());
  const child = workflow({
    name: "status-skipped-child", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => ({ value: await ctx.stage("never-runs").complete("never") }),
  });
  const parent = workflow({
    name: "status-skipped-parent", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      await Promise.all([
        ctx.workflow(child),
        Promise.resolve().then(() => ctx.exit({ status: "skipped", reason: "stop", outputs: { value: "skipped" } })),
      ]);
  ...
  const terminals = backend.listCheckpoints("wf-skipped-boundary").filter((checkpoint) =>
    checkpoint.kind === "stage" && checkpoint.topology?.boundary?.event === "terminal"
  );
...
test("fresh DBOS replay rejects stale nonreciprocal boundary ownership before child code", async () => {
  ...
  let childCalls = 0;
  const child = workflow({
    name: "stale-child", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async () => { childCalls += 1; return { value: "unsafe" }; },
  });
  const parent = workflow({
    name: "stale-parent", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      const nested = await ctx.workflow(child);
