...
test("disposal unsubscribes the delivery channel and stops the animation", () => {
  const { handle, emitDeliveryActivity, deliveryActivityListeners } = makeHandle(
    undefined,
    [],
    "completed",
  );
...
test("a delivery admitted after terminal cleanup still authorizes a retained turn", () => {
  ...
  try {
    ...
    assert.match(
      renderText(view),
      /Working/,
      "post-mortem conversation admitted after cleanup must keep Working",
    );
    assert.equal(view._hasAnimationTick, true);
...
test("remounting while a delivery is active replays it onto the new view", () => {
  ...
  try {
    assert.match(
      renderText(second),
      /Working/,
      "a view attached mid-delivery must replay the active delivery",
    );
