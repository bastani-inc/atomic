...
function stageContext(cwd: string, orchestrationContext: NonNullable<CreateAgentSessionOptions["orchestrationContext"]>): ExtensionContext {
...
test("a real foreground subagent inherits its workflow group and stays outside default chat", async () => {
  ...
  const workflowResult = await run(definition, {}, {
    store: createStore(),
  ...
  try {
    ...
    for (const params of [
      ...
      const result = await executor.execute("subagent-call", params, new AbortController().signal, undefined, context);
  ...
  const sessionInfo = (id: string, name: string, group?: string): SessionInfo => ({
    id, name, group, cwd: "/tmp", model: "test", pid: 1, startedAt: 1, lastActivity: 1,
  });
  ...
  assert.equal(writes.some((write) => write.socket === mainSocket && write.message.type === "message"), false);
  assert.equal(writes.some((write) => write.socket === childSocket && write.message.type === "delivery_failed"), true);
});
