...
test("reported absolute file paths are normalised to repository-relative posix paths", () => {
  ...
  const relative = JSON.stringify({
    numTotalTests: 1,
    testResults: [{ name: "test/unit/win.test.ts", assertionResults: [{ title: "a", status: "passed", duration: 1 }] }],
  });
...
test("an explicit budget still resolves from the file path the report names", () => {
  ...
  try {
    writeFileSync(
      ...
      ['test("declared", async () => {', "}, 60_000);"].join("\n"),
    );
    const scored = evaluateDurations(
      report([{ file: "suite.test.ts", tests: [{ title: "declared", status: "passed", duration: 100 }] }], directory),
      30_000,
      directory,
    );
...
test("repository declarations resolve to their real budgets", async () => {
  ...
  const installed = declaredTimeouts(await readText(join(root, "test/integration/installed-package-node-extensions.test.ts")));
  assert.equal(installed.get("installed @bastani/atomic loads builtin extensions under Node"), 240_000);
  const notification = declaredTimeouts(await readText(join(root, "test/unit/subagents-notification-content.test.ts")));
...
test("headroom is scored against the effective timeout, not a fixed ceiling", () => {
  ...
  try {
    ...
    assert.deepEqual(scored.failures.map((sample) => sample.name), ["heavy and undeclared"]);
    assert.deepEqual(scored.warnings.map((sample) => sample.name), ["heavy but declared"]);
