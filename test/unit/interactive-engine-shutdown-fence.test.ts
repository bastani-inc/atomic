...
test("shutdown stops the child, joins the in-flight attempt, and reports no failure", async () => {
	...
	const controller = new EngineHealthController({
		...
		restart: () => new Promise<void>((_, reject) => { failRestart = reject; }),
		clearActivity: () => {},
	});
	...
	const shutdown = controller.shutdown().then(() => { shutdownSettled = true; });
	await Bun.sleep(10);
...
test("a generation that dies after shutdown starts no replacement", async () => {
	...
	const controller = new EngineHealthController({
		stop: async () => {},
		restart: async () => { calls.restarts += 1; },
		clearActivity: () => {},
	});
...
serialTest("an explicit stop during a restart prevents the replacement from spawning", async () => {
	...
	try {
		...
		const restart = client.restart(undefined).then(() => undefined, (error: unknown) => error);
