...
test("concurrent completion claims share one promise-owned attempt", async () => {
	...
	const first = deliverClaimedCompletion("claim-concurrent", "signature", 60_000, {
		intercom,
		local: async () => true,
	});
	const second = deliverClaimedCompletion("claim-concurrent", "signature", 60_000, {
		intercom,
		local: async () => true,
	});
	...
	const third = deliverClaimedCompletion("claim-concurrent", "signature", 60_000, {
		intercom: async () => true,
		local: async () => true,
	});
...
test("new phase progress resets an earlier no-progress failure", async () => {
	...
	assert.equal(
		(await deliverClaimedCompletion("claim-progress-reset", "signature", 60_000, phases, 2)).noProgressFailures,
		1,
	);
	assert.equal(
		(await deliverClaimedCompletion("claim-progress-reset", "signature", 60_000, phases, 2)).noProgressFailures,
	...
	assert.equal(
		(await deliverClaimedCompletion("claim-progress-reset", "signature", 60_000, phases, 2)).status,
		"retry",
	);
	assert.equal(
		(await deliverClaimedCompletion("claim-progress-reset", "signature", 60_000, phases, 2)).status,
...
test("completion claims reject conflicting aliases in flight and after terminal states", async () => {
	...
	assert.deepEqual(
		await deliverClaimedCompletion("claim-conflict", "signature-b", 60_000, {
			local: async () => true,
		}),
...
test("completion claims preserve alias signatures after exhaustion", async () => {
	...
	assert.equal(
		(await deliverClaimedCompletion("claim-exhausted-conflict", "signature-a", 60_000, phases, 1)).status,
		"exhausted",
	);
	assert.equal(
		(await deliverClaimedCompletion("claim-exhausted-conflict", "signature-a", 60_000, phases, 1)).status,
	...
	assert.equal(
		(
			await deliverClaimedCompletion(
				"claim-exhausted-conflict",
				"signature-b",
				60_000,
				{
					local: async () => true,
