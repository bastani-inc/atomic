...
function testUi(onInput: () => void): WorkflowUIContext {
  return {
    async input() { onInput(); return "live-ui"; },
    async confirm() { throw new Error("unused"); },
    async select<T extends string>(_message: string, options: readonly T[]) { return options[0]!; },
...
async function runUnsafeCase(caseName: string, records: UnsafeRecordFactory) {
  ...
  const child = workflow({
    ...
    run: async (ctx) => {
      childCalls += 1;
      const tool = await ctx.tool("child-tool", {}, async () => { toolCalls += 1; return "live-tool"; });
  ...
  const result = await run(parent, {}, {
    ...
    ui: testUi(() => { uiCalls += 1; }),
    adapters: { complete: { complete: async (text) => { stageCalls += 1; return text; } } },
  });
...
test("one-sided and mixed scoped stage/tool/UI identities fail closed at the boundary", async () => {
  for (const [caseName, records] of cases) {
    ...
    assert.equal(observed.store.runs().some((run) => run.parentRunId === observed.runId), false, caseName);
  }
});
