...
test.sequential("real isolated child discovers and acquires engine-only custom OAuth", async () => {
	...
	try {
		...
		const callbacks: AtomicOAuthLoginCallbacks = {
			...
			onPrompt: async () => { callbackLog.push("prompt"); return "acme"; },
			onSelect: async () => { callbackLog.push("select"); return "primary"; },
			onManualCodeInput: async () => { callbackLog.push("manual"); return "code"; },
		};
		const result = await loginRpcOAuthProvider(client, "corp-oauth", callbacks);
		assert.equal(result.cancelled, false);
		assert.deepEqual(callbackLog, [
			"auth:https://corp.invalid/login", "device:CORP-CODE", "progress:waiting", "info:corp-info",
			"prompt", "select", "manual",
		]);
		assert.equal(result.cancelled ? false : result.models.some(({ provider, id }) => provider === "corp-oauth" && id === "corp-model"), true);
		...
		assert.equal(logout.models.some(({ provider }) => provider === "corp-oauth"), false);
