...
function childRun(rootId: string, childId: string, boundaryId: string): DurableStageRunTopology {
  return {
    runId: childId, runName: "catalog-child", parentRunId: rootId,
    parentStageId: boundaryId, rootRunId: rootId,
  };
}
...
function boundaryTopology(input: {
  ...
  return input.event === "start"
    ? {
        version: 1, stageId: input.boundaryId, parentIds: [], sourceOrder: input.order,
        status: "running", run: rootRun(input.rootId), boundary: { ...identity, event: "start", status: "running" },
      }
    : {
        version: 1, stageId: input.boundaryId, parentIds: [], sourceOrder: input.order,
        status: "completed", run: rootRun(input.rootId), boundary: { ...identity, event: "terminal", status: "completed" },
      };
}
...
function seedBoundary(input: {
  ...
  seedMockCheckpoint(sdk, rootId, {
    kind: "stage", workflowId: rootId, checkpointId: `start:${key}`,
    name: "workflow:catalog-child", replayKey: key, completedAt: 2 + order,
    topology: boundaryTopology({ rootId, childId, boundaryId, key, order, event: "start" }),
  });
  seedMockCheckpoint(sdk, rootId, {
    kind: "stage", workflowId: rootId, checkpointId: `${key}:child-stage`,
    name: "child-stage", replayKey: `${key}:stage:work:1`, output: "child-output", completedAt: 3 + order,
    topology: {
      version: 1, stageId: `child-stage-${order}`, parentIds: [], sourceOrder: 0,
      status: "completed", run: childRun(rootId, childId, boundaryId),
    },
  });
  seedMockCheckpoint(sdk, rootId, {
    kind: "stage", workflowId: rootId, checkpointId: `terminal:${key}`,
    name: "workflow:catalog-child", replayKey: key, completedAt: 4 + order, endedAt: 4 + order,
    output: {
      workflow: "catalog-child", runId: childId, status: "completed", exited: false,
...
async function assertHidden(
  ...
  const opened = openCompletedDurableWorkflow(rootId, {
    durableBackend: backend, store, stageControlRegistry: registry,
  });
...
test("fresh DBOS completed catalog rejects same-key output poisoning before merge", async () => {
  ...
  seedMockCheckpoint(sdk, rootId, {
    kind: "stage", workflowId: rootId, checkpointId: "poison", name: "ordinary",
    replayKey: key, completedAt: 9,
    output: {
      workflow: "catalog-child", runId: "poison-child", status: "completed",
      exited: false, outputs: { value: "poison" },
    },
    topology: {
      version: 1, stageId: "poison-stage", parentIds: [], sourceOrder: 9,
      status: "completed", run: rootRun(rootId),
    },
  });
...
test("fresh DBOS completed catalog hides legacy child output without a reciprocal child group", async () => {
  ...
  const sdk = completedSdk(rootId);
  seedMockCheckpoint(sdk, rootId, {
    kind: "stage", workflowId: rootId, checkpointId: "legacy-boundary",
    name: "workflow:catalog-child", replayKey: "workflow:catalog-child:1", completedAt: 2,
    output: {
      workflow: "catalog-child", runId: "missing-child", status: "completed",
      exited: false, outputs: { value: "verbatim" },
    },
    topology: {
      version: 1, stageId: "legacy-boundary", parentIds: [], sourceOrder: 0,
      status: "completed", run: rootRun(rootId),
    },
  });
...
test("fresh DBOS completed catalog rejects one prompt occurrence bound to different replay keys", async () => {
  ...
  for (const [index, replayKey] of ["prompt:input:first", "prompt:input:SECOND-DIFFERENT"].entries()) {
    seedMockCheckpoint(sdk, rootId, {
      kind: "stage", workflowId: rootId, checkpointId: `prompt-${index}`,
      name: "input", replayKey, completedAt: 2 + index, startedAt: 1, endedAt: 2 + index,
      topology: {
        version: 1, stageId: "same-prompt-stage", parentIds: [], sourceOrder: 0,
        occurrenceKey: "shared-prompt-occurrence", status: "completed", run: rootRun(rootId),
      },
    });
  }
...
test("fresh DBOS completed catalog rejects replay-key stage identity drift", async () => {
  ...
  seedMockCheckpoint(sdk, rootId, {
    kind: "stage", workflowId: rootId, checkpointId: "session", name: "work", replayKey,
    sessionId: "session", sessionFile: "/missing", startedAt: 1, completedAt: 2,
    topology: {
      version: 1, stageId: "original-id", parentIds: [], sourceOrder: 0,
      status: "running", run: rootRun(rootId),
    },
  });
  seedMockCheckpoint(sdk, rootId, {
    kind: "stage", workflowId: rootId, checkpointId: "output", name: "renamed-work", replayKey,
    output: "verbatim", endedAt: 3, completedAt: 3,
    topology: {
      version: 1, stageId: "drifted-id", parentIds: [], sourceOrder: 9,
      status: "completed", run: rootRun(rootId),
    },
  });
...
test("fresh DBOS completed catalog rejects empty and aliased owning-run boundary ids", async () => {
  ...
  seedBoundary({
    sdk: emptySdk, rootId: emptyRoot, key: "workflow:catalog-child:1",
    boundaryId: "", childId: "empty-child", order: 0,
  });
  ...
  seedBoundary({
    sdk: duplicateSdk, rootId: duplicateRoot, key: "workflow:catalog-child:1",
    boundaryId: "aliased-boundary", childId: "child-one", order: 0,
  });
  seedBoundary({
    sdk: duplicateSdk, rootId: duplicateRoot, key: "workflow:catalog-child:2",
    boundaryId: "aliased-boundary", childId: "child-two", order: 1,
  });
