...
function transcript(directory: string, name: string): string {
  ...
  writeFileSync(file, [
    ...
    JSON.stringify({ type: "message", id: `${name}-message`, parentId: null, timestamp: new Date().toISOString(), message: { role: "user", content: name } }),
  ].join("\n") + "\n");
  return file;
}
...
function boundaryTopology(
  ...
  return event === "start"
    ? {
        version: 1, stageId: boundaryId, parentIds: [], sourceOrder,
        status: "running", run: { runId: ROOT_ID, runName: "round2-routing-parent" },
        boundary: { ...identity, event: "start", status: "running" },
      }
    : {
        version: 1, stageId: boundaryId, parentIds: [], sourceOrder,
        status: "completed", run: { runId: ROOT_ID, runName: "round2-routing-parent" },
...
test.sequential("fresh completed DBOS graphs keep exact attach routing while terminal sends fail at the root", async () => {
  ...
  try {
    ...
    assert.deepEqual(catalog.completed.map((entry) => entry.workflowId), [ROOT_ID]);

    const attachedFiles: string[] = [];
    const prompts: string[] = [];
    const opened = openCompletedDurableWorkflow(ROOT_ID, {
