...
function activeReservationIds(sdk: ReturnType<typeof createMockSdk>): string[] {
  ...
  for (const value of sdk.state.steps.values()) {
    ...
    if (record["__atomicPromptReservation"] === true && record["operation"] === "reserve"
      && typeof record["reservationId"] === "string") ids.add(record["reservationId"]);
  }
  return [...ids];
}
...
async function prepareFixture(rootId: string) {
  ...
  const count = (name: string): void => { calls.set(name, (calls.get(name) ?? 0) + 1); };
  ...
  const twig = workflow({
    name: "cached-guard-twig", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      count("twig-body");
  ...
  const leaf = workflow({
    name: "cached-guard-leaf", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      count("leaf-body");
  ...
  const completedChild = workflow({
    name: "cached-guard-completed", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const liveSibling = workflow({
    name: "cached-guard-live", description: "", inputs: {}, outputs: { answer: Type.String() },
    run: async (ctx) => {
      count("live-body");
  ...
  const root = workflow({
    name: rootId, description: "", inputs: {}, outputs: { answer: Type.String() },
    run: async (ctx) => {
      count("root-body");
  ...
  const execution = run(root, {}, {
    runId: rootId, store, stageControlRegistry: controls, durableBackend: backend,
    signal: controller.signal, usePromptNodesForUi: true,
...
function resumeOptions(
  ...
  return {
    registry: createRegistry([root]), durableBackend: backend, jobs,
    baseRunOpts: {
      store: workflowStore, cancellation, stageControlRegistry: controls,
...
test.serial("cached completed boundary republishes a child that handled a failed stage and completed downstream", async () => {
  ...
  const resumed = await resumeDurableWorkflow(
    rootId, resumeOptions(fixture.root, fresh, jobs, controls, cancellation),
  );
  ...
  for (const original of fixture.originalRuns) {
    ...
    assert.deepEqual(restored.stages.map((stage) => stage.id), original.stages.map((stage) => stage.id));
    assert.deepEqual(restored.stages.map((stage) => stage.parentIds), original.stages.map((stage) => stage.parentIds));
    if (original.name !== rootId && original.name !== "cached-guard-live") {
      assert.deepEqual(restored.stages.map((stage) => stage.status), original.stages.map((stage) => stage.status));
...
test.serial("cached completed boundary accepts a fully timed skipped child stage without rerunning", async () => {
  ...
  assert.equal(fixture.calls.get("stage:after-handled"), 1);
  const answer = await workflowSendAction({
    runId: rootId, stageId: `${freshPending.runId}:${freshPending.stage.id}`,
    promptId: freshPending.stage.pendingPrompt!.id, response: "done", delivery: "answer",
  });
  assert.equal(answer.status, "ok");
  await jobs.get(rootId)!.promise;
  await fresh.flush();
