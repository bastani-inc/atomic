...
function terminals(backend: DbosDurableBackend, runId: string): DurableStageCheckpoint[] {
  return backend.listCheckpoints(runId).filter((checkpoint): checkpoint is DurableStageCheckpoint =>
...
test("failed, skipped, and exited child lifecycle survives fresh DBOS catalog hydration exactly once", async () => {
  ...
  const failing = workflow({
    name: "round2-lifecycle-failing", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => ({ value: await ctx.stage("failed-child-stage").complete("fail-child") }),
  });
  const exited = workflow({
    name: "round2-lifecycle-exited", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const parent = workflow({
    name: "round2-lifecycle-parent", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      try { await ctx.workflow(failing); } catch { /* handled failed child */ }
  ...
  const skippedChild = workflow({
    name: "round2-lifecycle-never", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => ({ value: await ctx.stage("must-not-attach").complete("never") }),
  });
  const skippedParent = workflow({
    name: "round2-lifecycle-skip-parent", description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      await Promise.all([
  ...
  const callbacks = {
    onStageStart: () => { stageStarts += 1; },
    onStageEnd: () => { stageEnds += 1; },
  };

  const completed = await run(parent, {}, {
  ...
  const skipped = await run(skippedParent, {}, {
    runId: skippedRootId,
    store: createStore(),
    durableBackend: writer,
    adapters: { complete: { complete } },
  ...
  assert.deepEqual(terminals(writer, completedRootId).map((checkpoint) => checkpoint.topology!.status), ["failed", "completed"]);
  assert.deepEqual(terminals(writer, skippedRootId).map((checkpoint) => checkpoint.topology!.status), ["skipped"]);
  ...
  assert.deepEqual(
    listOpenableCompletedWorkflows(fresh).map((entry) => entry.workflowId).sort(),
    [completedRootId, skippedRootId].sort(),
  );
  assert.equal(catalog.resumable.some((entry) => entry.workflowId === skippedRootId), false);
  ...
  assert.equal(reconstructed.some((candidate) => candidate.name === "round2-lifecycle-failing"), false);

  const freshStore = createStore();
  const registry = createStageControlRegistry();
  const opened = openCompletedDurableWorkflow(completedRootId, {
  ...
  assert.deepEqual(graph.stages.map((stage) => [stage.name, stage.status]), [
  ...
  const openedSkipped = openCompletedDurableWorkflow(skippedRootId, {
    durableBackend: fresh,
    store: skippedStore,
    stageControlRegistry: registry,
  }, catalog.completed);
  ...
  assert.equal(skippedStore.runs().some((candidate) => candidate.parentRunId === skippedRootId), false);
  ...
  assert.deepEqual({ stageStarts, stageEnds }, callbackCounts, "fresh hydration emits no historical lifecycle callbacks");
  assert.equal(persisted.state.steps.size, persistedStepCount, "catalog/open hydration must not write lifecycle records");
