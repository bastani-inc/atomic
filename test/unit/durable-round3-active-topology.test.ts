...
function childRun(runId: string, overrides: Partial<DurableStageRunTopology> = {}): DurableStageRunTopology {
  return {
    runId: `${runId}-child`, runName: CHILD, parentRunId: runId,
    parentStageId: BOUNDARY_ID, rootRunId: runId, ...overrides,
  };
}
...
function startTopology(
  ...
  return {
    version: 1, stageId, parentIds: [], sourceOrder: 0, status: "running", run: rootRun(runId),
    boundary: {
      version: 1, event: "start", replayScope, alias: CHILD, workflow: CHILD,
      invocationFingerprint: fingerprint(), status: "running",
      child: {
        runId: ownedChild.runId, runName: ownedChild.runName,
        parentRunId: runId, parentStageId: stageId, rootRunId: runId,
      },
    },
  };
}
...
function seedStage(sdk: ReturnType<typeof createMockSdk>, runId: string, stage: ChildStageSeed): void {
  seedMockCheckpoint(sdk, runId, {
    kind: "stage", workflowId: runId, checkpointId: `${stage.key}:checkpoint`,
    name: stage.name ?? stage.key, replayKey: stage.key, output: `cached:${stage.key}`, completedAt: stage.order + 2,
    topology: {
      version: 1, stageId: stage.id, parentIds: [...(stage.parents ?? [])], sourceOrder: stage.order,
      ...(stage.occurrenceKey !== undefined ? { occurrenceKey: stage.occurrenceKey } : {}),
      status: "completed", run: stage.run ?? childRun(runId),
    },
  });
}
...
function seedTerminal(
  ...
  seedMockCheckpoint(sdk, runId, {
    kind: "stage", workflowId: runId, checkpointId: `terminal:${suffix}`,
    name: `workflow:${CHILD}`, replayKey: SCOPE, completedAt: suffix === "completed" ? 9 : 10,
    ...(output !== undefined ? { output } : {}), topology: raw,
  } as never);
}

function completedChildOutput(runId: string, outputs: WorkflowSerializableValue): WorkflowSerializableValue {
  return {
    workflow: CHILD, runId: `${runId}-child`, status: "completed",
...
async function runCase(
  caseName: string,
  seed: (sdk: ReturnType<typeof createMockSdk>, runId: string) => void,
) {
  const runId = `round3-active-${caseName}`;
  const sdk = createMockSdk();
  seedMockWorkflow(sdk, { workflowId: runId, name: PARENT, status: "PENDING", createdAt: 1 });
  seedMockCheckpoint(sdk, runId, {
    kind: "stage", workflowId: runId, checkpointId: `boundary-start:${SCOPE}`,
    name: `workflow:${CHILD}`, replayKey: SCOPE, completedAt: 1, topology: startTopology(runId),
  });
  seed(sdk, runId);
  ...
  const child = workflow({
    name: CHILD, description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
      childCalls += 1;
  ...
  const parent = workflow({
    name: PARENT, description: "", inputs: {}, outputs: { value: Type.String() },
    run: async (ctx) => {
  ...
  const result = await run(parent, {}, {
    ...
    adapters: { complete: { complete: async (text) => { adapterCalls += 1; return text; } } },
    onStageStart: () => { lifecycleCalls += 1; },
    onStageEnd: () => { lifecycleCalls += 1; },
  });
...
const malformedCases: ReadonlyArray<readonly [string, (sdk: ReturnType<typeof createMockSdk>, runId: string) => void]> = [
  ["wrong-parent-outside-scope", (sdk, runId) => seedStage(sdk, runId, {
    key: "workflow:unrelated:stage:wrong:1", id: "wrong", order: 0,
    run: childRun(runId, { parentRunId: "wrong-parent" }),
  })],
  ["duplicate-id", (sdk, runId) => {
    seedStage(sdk, runId, { key: `${SCOPE}:stage:a:1`, id: "same", order: 0 });
    seedStage(sdk, runId, { key: `${SCOPE}:stage:b:1`, id: "same", order: 1 });
  }],
  ["prompt-occurrence-replay-poison", (sdk, runId) => {
    seedStage(sdk, runId, {
      key: `${SCOPE}:prompt:input:first`, name: "input", id: "same-prompt", order: 0,
      occurrenceKey: "shared-occurrence",
    });
    seedStage(sdk, runId, {
      key: `${SCOPE}:prompt:input:SECOND-DIFFERENT`, name: "input", id: "same-prompt", order: 0,
      occurrenceKey: "shared-occurrence",
    });
  }],
  ["duplicate-order", (sdk, runId) => {
  ...
  ["empty-id", (sdk, runId) => seedStage(sdk, runId, {
    key: `${SCOPE}:stage:empty:1`, id: "", order: 0,
  })],
  ["aliased-boundary-id", (sdk, runId) => {
    const otherScope = "workflow:workflow:other-child:1";
    seedMockCheckpoint(sdk, runId, {
      kind: "stage", workflowId: runId, checkpointId: `boundary-start:${otherScope}`,
      name: "workflow:other-child", replayKey: otherScope, completedAt: 2,
      topology: startTopology(runId, otherScope, BOUNDARY_ID, {
        runId: `${runId}-other`, runName: CHILD, parentRunId: runId,
        parentStageId: BOUNDARY_ID, rootRunId: runId,
      }),
    });
  }],
  ["start-non-running", (sdk) => {
...
test("fresh DBOS active child topology fails closed before cache, boundary dispatch, targets, or side effects", async () => {
  for (const [caseName, seed] of malformedCases) {
    ...
    assert.match(observed.result.error ?? "", /DurableNestedTopologyError|durable nested topology is non-resumable/, caseName);
    ...
    assert.equal(observed.store.runs().some((candidate) => candidate.parentRunId === observed.runId), false, caseName);
    assert.deepEqual(observed.store.runs().find((candidate) => candidate.id === observed.runId)?.stages, [], caseName);
  }
});
