...
function seedCompletedBoundary(
  ...
  backend.registerWorkflow({
    workflowId: runId, name: "completed-parent", inputs: {}, createdAt: 1,
    status: "completed", updatedAt: 5, resumable: false,
  });
  backend.recordCheckpoint({
    kind: "stage", workflowId: runId, checkpointId: `boundary-start:${replayKey}`,
    name: "workflow:child", replayKey, completedAt: 2,
    topology: {
  ...
  backend.recordCheckpoint({
    ...
    name: "child-stage", replayKey: `${replayKey}:stage:child:1`, output: "child", completedAt: 3,
    topology: {
  ...
  backend.recordCheckpoint({
    kind: "stage", workflowId: runId, checkpointId: `boundary-terminal:${replayKey}:completed`,
    name: "workflow:child", replayKey, output, result: "completed", completedAt: 4,
    topology: {
      version: 1, stageId: "boundary", parentIds: [], sourceOrder: 0,
      status: "completed", run: rootRun,
      boundary: {
        version: 1, event: "terminal", replayScope: replayKey, alias: "child", workflow: "child",
...
test("completed catalog uses the shared strict WorkflowChildResult parser", () => {
  ...
  seedCompletedBoundary(valid, "valid-completed", {
    workflow: "child", runId: "valid-completed-child", status: "completed", exited: false, outputs: {},
  });
  ...
  ] satisfies readonly WorkflowSerializableValue[]).entries()) {
