...
test("stale repair does not commit terminal status until its exact result payload is staged", () => {
	...
	const retried = reconcileAsyncRun(asyncDir, {
		resultsDir: blockedResults,
		now: () => 101,
		kill: () => {
			const error = new Error("dead") as NodeJS.ErrnoException;
...
test("legacy aliases without id dedupe by canonical runId", async () => {
	...
	for (const file of ["one.json", "two.json"]) {
		fs.writeFileSync(
			...
			JSON.stringify({ runId: "canonical-runid-alias", sessionId: "session", success: true }),
		);
	}
...
test("watcher revalidates session ownership after delayed intercom acknowledgement", async () => {
	...
	fs.writeFileSync(
		resultPath,
		JSON.stringify({ id: "owner-after-wait", sessionId: "session-a", intercomTarget: "parent" }),
	);
	const listeners = new Map<string, Set<(data: object) => void>>();
	let completions = 0;
	let acknowledgeIntercom: (() => void) | undefined;
	let markIntercomEmitted!: () => void;
	const intercomEmitted = new Promise<void>((resolve) => {
		markIntercomEmitted = resolve;
	});
...
test("stale repair recovers a failed atomic stage rename without replaying an armed stage", () => {
	...
	const payload = JSON.parse(fs.readFileSync(path.join(resultsDir, "rename-retry.json"), "utf-8")) as Record<
		string,
		unknown
	>;
...
test("acknowledged intercom remains idempotent while local notification retries", async () => {
	...
	fs.writeFileSync(
		resultPath,
		JSON.stringify({ id: "side-effects", sessionId: "session", asyncDir, intercomTarget: "parent" }),
	);
	const listeners = new Map<string, Set<(data: object) => void>>();
...
test("a delayed successful Intercom phase is checkpointed after watcher retirement", async () => {
	...
	fs.writeFileSync(
		resultPath,
		JSON.stringify({ id: "retired-phase", sessionId: "session", intercomTarget: "parent" }),
	);
	const listeners = new Map<string, Set<(data: object) => void>>();
	let intercomAttempts = 0;
	let localAttempts = 0;
