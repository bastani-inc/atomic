...
async function releasePromptOwners(
...
test.serial("fresh DBOS replay coalesces an omitted occurrenceKey prompt with current metadata", async () => {
  ...
  assert.equal(writerStore.resolveStagePendingPrompt(
  ...
  const decodedLegacy = fresh.listCheckpoints(ROOT_ID).find((checkpoint) =>
    checkpoint.kind === "stage" && checkpoint.checkpointId === legacy.checkpointId
  );
  ...
  const selectRecords = fresh.listCheckpoints(ROOT_ID).filter((checkpoint): checkpoint is DurableStageCheckpoint =>
  ...
  assert.deepEqual(new Set(selectRecords.map((checkpoint) => checkpoint.topology?.stageId)), new Set([legacy.stageId]));
  assert.deepEqual(
  ...
  assert.deepEqual(openable.map((entry) => entry.workflowId), [ROOT_ID]);
  const inspectedRuns = completedWorkflowRunSnapshots(catalogBackend, openable[0]!);
  assert.deepEqual(inspectedRuns.map((snapshot) => snapshot.id), [ROOT_ID]);
...
test("fresh DBOS active root rejects an occurrence bound to different replay keys before prompt replay", async () => {
  ...
  for (const [index, replayKey] of ["prompt:input:first", "prompt:input:SECOND-DIFFERENT"].entries()) {
    seedMockCheckpoint(sdk, rootId, {
      kind: "stage", workflowId: rootId, checkpointId: `active-prompt-poison-${index}`,
      name: "input", replayKey, completedAt: 2 + index, startedAt: 1, endedAt: 2 + index,
      topology: {
        version: 1, stageId: "active-poison-stage", parentIds: [], sourceOrder: 0,
        occurrenceKey: "active-shared-occurrence", status: "completed",
  ...
  const root = workflow({
    name: rootId, description: "", inputs: {}, outputs: { response: Type.String() },
    run: async (ctx) => ({ response: await ctx.ui.input("must never become pending") }),
  });
  try {
    const result = await run(root, {}, {
      runId: rootId, store, durableBackend: backend,
