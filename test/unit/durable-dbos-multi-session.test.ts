...
describe("shared-database visibility across sessions", () => {
	test("hides another session's live running workflow from resume, surfaces it once its heartbeat ages out", async () => {
		const state: SharedDbosState = { workflows: new Map(), steps: new Map() };
		state.workflows.set("wf-live-elsewhere", {
			workflowId: "wf-live-elsewhere",
			name: "multi-session-flow",
			status: "PENDING",
			createdAt: 1_000,
		});
		...
		crashed.workflows.set("wf-crashed-elsewhere", {
			workflowId: "wf-crashed-elsewhere",
			name: "multi-session-flow",
			status: "PENDING",
			createdAt: 1_000,
		});
		crashed.steps.set(
			...
			seededMetadata("wf-crashed-elsewhere", {
				workflowId: "wf-crashed-elsewhere",
				ownerExecutorId: "atomic-other-1",
...
describe("cross-process resume claim", () => {
	...
	test("a non-resumable paused reservation rolls back to blocked across hydration", async () => {
		...
		assert.equal(
			await claimant.transitionWorkflowStatus("wf-reservation", ["blocked"], "paused", undefined, false),
			true,
		);
		...
		assert.equal(
			await claimant.transitionWorkflowStatus("wf-reservation", ["paused"], "blocked", undefined, true),
			true,
		);
