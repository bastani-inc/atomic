...
async function waitForPending(
  ...
  throw new Error(`expected ${count} pending confirms: ${JSON.stringify(store.runs().map((snapshot) => ({
    id: snapshot.id,
    stages: snapshot.stages.map((stage) => ({ id: stage.id, status: stage.status, pending: stage.pendingPrompt?.kind })),
  })))}`);
}
...
function assertDurablePromptOccurrences(
  ...
  for (const checkpoint of backend.listCheckpoints(rootId)) {
    if (checkpoint.kind !== "stage" || checkpoint.name !== "confirm"
      || checkpoint.topology?.run?.runId !== childRunId
      || checkpoint.topology.occurrenceKey === undefined
      || checkpoint.topology.sourceOrder === undefined) continue;
  ...
  assert.deepEqual(occurrences.map((occurrence) => occurrence.stageId), expectedStageIds);
  assert.deepEqual(occurrences.map((occurrence) => occurrence.sourceOrder), [0, 1]);
...
function reservationIds(sdk: ReturnType<typeof createMockSdk>): string[] {
  ...
  for (const value of sdk.state.steps.values()) {
    ...
    if (event["__atomicPromptReservation"] === true && event["operation"] === "reserve"
      && typeof event["reservationId"] === "string") ids.add(event["reservationId"]);
  }
  return [...ids];
}
...
test.serial("fresh nested resume keeps sequential identical same-callsite prompt occurrences distinct", async () => {
  ...
  const child = workflow({
    name: "repeated-sequential-child", description: "", inputs: {},
  ...
  const root = workflow({
    name: "repeated-sequential-root", description: "", inputs: {},
    outputs: { first: Type.Boolean(), second: Type.Boolean() },
    run: async (ctx) => {
  ...
  const execution = run(root, {}, {
    runId: rootId, store: writerStore, stageControlRegistry: writerControls,
    durableBackend: writer, signal: controller.signal, usePromptNodesForUi: true,
  });
  ...
  writerStore.resolveStagePendingPrompt(
    firstPending.runId, firstPending.stages[0]!.id,
    firstPending.stages[0]!.pendingPrompt!.id, true,
  );
  ...
  assert.deepEqual(original.map((stage) => stage.parentIds), [[], [original[0]!.id]]);
  assert.equal(new Set(original.map((stage) => stage.replayKey)).size, 1);

  const persisted = await pauseAndCopy({
    rootId, sdk, backend: writer, store: writerStore, controls: writerControls,
    controller, execution, pendingCount: 1,
  });
  const fresh = new DbosDurableBackend(persisted, { executorId: "repeated-sequential-fresh" });
  await fresh.hydrateWorkflow(rootId);
  assertDurablePromptOccurrences(fresh, rootId, firstPending.runId, original.map((stage) => stage.id));
  ...
  assert.deepEqual(restored.map((stage) => stage.id), original.map((stage) => stage.id));
  assert.deepEqual(restored.map((stage) => stage.parentIds), original.map((stage) => stage.parentIds));
  assert.deepEqual(restored.map((stage) => stage.promptFootprint?.message), [QUESTION, QUESTION]);
  assert.deepEqual(restored.map((stage) => stage.status), ["completed", "awaiting_input"]);
  ...
  assert.deepEqual(openable.map((entry) => entry.workflowId), [rootId]);
  ...
  assert.deepEqual(inspectedPrompts.map((stage) => stage.id), original.map((stage) => stage.id));
...
test.serial("fresh nested resume restores and independently answers two concurrent identical prompts", async () => {
  ...
  const writerControls = createStageControlRegistry();
  const child = workflow({
    name: "repeated-concurrent-child", description: "", inputs: {},
  ...
  const root = workflow({
    name: "repeated-concurrent-root", description: "", inputs: {},
    outputs: { answers: Type.Array(Type.Boolean()) },
    run: async (ctx) => {
  ...
  assert.equal(new Set(original.map((stage) => stage.replayKey)).size, 1);
  assert.deepEqual(original.map((stage) => stage.parentIds), [[], []]);
  const persisted = await pauseAndCopy({
    rootId, sdk, backend: writer, store: writerStore, controls: writerControls,
    controller, execution, pendingCount: 2,
  });
  ...
  assertDurablePromptOccurrences(fresh, rootId, originalPending.runId, original.map((stage) => stage.id));
  assert.equal(freshPending.runId, originalPending.runId);
  const restored = promptStages(workflowStore, originalPending.runId);
  assert.deepEqual(restored.map((stage) => stage.id), original.map((stage) => stage.id));
  assert.deepEqual(restored.map((stage) => stage.parentIds), [[], []]);
  assert.deepEqual(restored.map((stage) => stage.promptFootprint?.message), [QUESTION, QUESTION]);
