...
describe("pattern workflow terminal artifacts stay references", () => {
    let tempCwd = "";
    beforeEach(() => { tempCwd = mkdtempSync(join(tmpdir(), "atomic-artifact-ref-")); });
        ...
        Object.defineProperty(ctx, "task", {
            value: async (name, options) => {
                ...
                return isTerminal(name)
                    ? { ...result, text: `${SENTINEL} ${String(options.output)}` }
                    : result;
            },
        });
        return ctx;
    ...
    test("classify-and-act returns the action artifact by reference", async () => {
        ...
        const ctx = stubTerminalText(
            Object.assign(
                makeMockCtx({ prompt: "Analyze the parser", categories: ["analysis", "implementation"], confidence_threshold: 0.5 }, {
    ...
    test("fan-out-and-synthesize returns the synthesis artifact by reference", async () => {
        ...
        const ctx = stubTerminalText(
            Object.assign(
                makeMockCtx({ prompt: "Map the runtime", max_branches: 2, max_concurrency: 2 }, {
                    task: (name) => name === "partition"
                        ? JSON.stringify({ partitions: [{ label: "one", objective: "first" }, { label: "two", objective: "second" }] })
                        : undefined,
                }),
                { cwd: tempCwd },
            ),
    ...
    test("tournament returns the reducer artifact by reference", async () => {
        ...
        const ctx = stubTerminalText(
            Object.assign(
                makeMockCtx({ prompt: "Design a safe migration", num_attempts: 2, max_concurrency: 2 }, {
    ...
    test("loop-until-done returns the completion artifact by reference", async () => {
        ...
        const ctx = stubTerminalText(
            Object.assign(
                makeMockCtx({ prompt: "Get the suite green", max_iterations: 2 }, {
                    task: (name) => name.startsWith("evaluate-")
                        ? JSON.stringify({
                            done: true,
