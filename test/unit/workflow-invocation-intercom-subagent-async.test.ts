  ...
  try {
    const executor = createSubagentExecutor({
      ...
      runtime: {
        ...
        executeAsyncSingle: (id, params) => executeAsyncSingle(id, {
          ...params,
          spawnRunner(config) {
        ...
        executeAsyncChain: (id, params) => executeAsyncChain(id, {
          ...params,
          spawnRunner(config) {
            configs.push(config as CapturedRunnerConfig);
            return { pid: 1234 };
    ...
    for (const params of [
      ...
      const result = await executor.execute("async-single", params, new AbortController().signal, undefined, context);
      assert.equal(result.isError, undefined);
    }
    const parallel = await executor.execute("async-parallel", {
      tasks: [
    ...
    const chain = await executor.execute("async-chain", {
      chain: [
    ...
    const foreground = await executor.execute("foreground-source", {
      agent: "worker",
      task: "persist source",
    }, new AbortController().signal, undefined, context);
    ...
    for (const group of [undefined, "default"] as const) {
      const revived = await executor.execute("revive", {
        action: "resume",
