...
function makeCompletionsModel(overrides?: Partial<Model<"openai-completions">>): Model<"openai-completions"> {
	return { ...(makeModel("openai-completions") as Model<"openai-completions">), ...overrides };
}
 ...
 * this adapter, and a pre-merge guard would miss `Model.samplingParams`, which is merged by
 * the same call. `top_p` and `top_k` are never named fields on this adapter, so
