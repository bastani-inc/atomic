...
function isModalityList(value: unknown, allowPdf = false): value is ("text" | "image" | "pdf")[] {
	return (
		Array.isArray(value) &&
		value.length > 0 &&
		value.every((entry) => entry === "text" || entry === "image" || (allowPdf && entry === "pdf"))
	);
}
...
function validateModelValue(
	...
	if (!isModalityList(value.input, true)) errors.push(`${label} has invalid input modalities`);
	if (value.type === "image") {
