...
function sanitizeRpcPath(value) {
  const raw = typeof value === "string" && value ? value.split("?")[0] : "/agent.v1.AgentService/Run";
  return raw.replace(/[\x00-\x1f\x7f]/g, " ").slice(0, 200);
}

function statusDiagnostic(status) {
  const path = sanitizeRpcPath(rpcPath);
  if (!Number.isInteger(status)) return `Cursor HTTP/2 request to ${path} failed before a valid response status was received.`;
  return `Cursor HTTP/2 request to ${path} failed with status ${status}.`;
}
...
h2Stream.on("response", (responseHeaders) => {
  ...
  responseStatus = typeof rawStatus === "number" ? rawStatus : Number(rawStatus);
  responseFailed = !Number.isInteger(responseStatus) || responseStatus < 200 || responseStatus >= 300;
});

// Forward H2 response data → stdout (length-prefixed)
h2Stream.on("data", (chunk) => {
  resetTimeout();
  if (!responseFailed) writeMessage(chunk);
});

h2Stream.on("end", () => {
  clearTimeout(timeout);
  client.close();
  if (responseFailed || !sawResponse) {
    process.stderr.write(`${statusDiagnostic(responseStatus)}\n`);
