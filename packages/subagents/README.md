...
Accepted levels are `auto`, `none`, `attested`, `checked`, `verified`, and `reviewed`. `acceptance: "auto"` is the default. Read-only reviewer/scout/analyzer tasks infer lightweight attestation, normal writer tasks infer checked evidence, and async/risky writer contexts infer a reviewed gate. Dynamic fanout and dynamic-group runs default to reviewed unless the task or agent is read-only/no-write. Prose report and summary requests such as "summarize the fix", "write a summary of README", or "write a report with proposed fixes" stay attested unless they explicitly ask for file or code edits. Objectful file edit requests such as "edit README and write a summary" or "modify README and report results" keep checked/reviewed write gates. Bare investigation or diagnostic prompts such as "Investigate the likely fix for the cache race" and read-only-agent tasks such as `codebase-analyzer` handling "Analyze each fix regression" also stay attested even when they include issue-title words like "fix" or "patch". Side-effectful operations such as "release the package and write a summary", objectless "release and write a summary", "commit the changes and write a report", or objectless "commit and write a report" keep checked/reviewed write gates; read-only phrases such as "summarize release notes" or "analyze the previous commit" stay attested. Prior-work summaries such as "report what changed in the patch", "summarize what changed in the diff", or "analyze tests added in the last patch" are also treated as read-only; actor-owned implementation summaries such as "report what you changed and tests added" still require write evidence. Explicit no-write/no-edit/no-file-write/no-source-change investigation, research, debugging, or inspection prompts stay attested even if the issue title contains words such as "fix"; boundary phrases may continue into commas, conjunctions, or constrained read-only `but` continuations, so "do not edit files, just identify the root cause", "do not edit files, but only report the root cause", "do not edit files, just list findings", "do not edit files and only report root cause", and "do not modify code, then summarize findings" are no-write. Scoped implementation constraints such as "do not change anything else", "do not change tests", "do not change snapshots", "do not change existing behavior", "do not modify CLI behavior", or "do not change database schema" preserve the write gate when paired with implementation intent, and write requests such as "do not edit files, but implement the migration" are not treated as no-write. To disable gates, prefer `{ level: "none", reason: "..." }`.
...
For `attested` or stricter levels, the child prompt includes a standardized acceptance section and asks for a fenced `acceptance-report` JSON block. Checked, verified, and reviewed write gates require non-empty `changedFiles`; a missing field and a present-but-empty list are reported differently so no-edit investigations are not confused with failed write evidence. Explicit failed gates fail the run. Inferred gates are persisted for observability without breaking older calls that omit `acceptance`. Status surfaces describe `rejected` as a completed run whose acceptance gate rejected the result and include the first actionable rejection reason when available, including the first review blocker or parent-decision finding, not a raw `acceptance: rejected` child state.
