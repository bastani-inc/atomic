...
export async function resumeAsyncRun(input: {
	...
	const revivedTarget = intercomBridge.active ? resolveSubagentIntercomTarget(runId, target.agent, 0) : undefined;
	...
	return {
		content: [{ type: "text", text: input.deps.runtime.formatAsyncStartedMessage(lines.join("\n")) }],
