...
interface GitResult { readonly stdout: string; readonly stderr: string; readonly status: number | null }
...
function prepareDestination(root: string, relativePath: string): string {
	...
	if (!isContained(fs.realpathSync.native(root), fs.realpathSync.native(ancestor))) throw new Error(`worktree setup path resolves outside its root: ${relativePath}`);
...
function hooksDirectory(mainRoot: string): string | undefined {
	...
	try { if (fs.statSync(husky).isDirectory()) return husky; } catch {}
...
function symlinkDirectories(mainRoot: string, worktreePath: string, configured: readonly string[]): string[] {
	...
	for (const raw of new Set(["node_modules", ...configured])) {
		...
		const source = path.join(mainRoot, relativePath);
		try { if (!fs.statSync(source).isDirectory()) continue; } catch { continue; }
		const destination = prepareDestination(worktreePath, relativePath);
...
export function performPostCreationSetup(mainRoot: string, worktreePath: string, configured: readonly string[], runGit: GitRunner): string[] {
	const syntheticPaths = symlinkDirectories(mainRoot, worktreePath, configured);
