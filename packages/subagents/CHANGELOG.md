...
- Fixed root single-agent `progress` calls so `progress: true` is schema-valid and maintains `progress.md` in the effective child working directory for foreground and background runs, while keeping `includeProgress` as the separate final-result telemetry option.
