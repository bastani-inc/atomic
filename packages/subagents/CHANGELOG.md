...
- Workflow stages now cancel every still-running foreground subagent they own when the stage completes, including single and parallel children detached for Intercom coordination. Cancelled children retain the existing terminal `interrupted` status with `cause: "abort"`, finish their normal artifact and listener cleanup, and do not send findings or completion notifications to the parent chat after the workflow has moved on. Direct `intercom.send` and `contact_supervisor` updates that were already in flight report cancellation rather than success, and their exact stage-owned run IDs cannot escape through the completed stage's main-chat route. Detached children still notify normally when their owning stage remains live, ordinary traffic not owned by that stage keeps its existing completed-stage route, and other stages' children are unaffected ([#2840](https://github.com/bastani-inc/atomic/issues/2840)).
