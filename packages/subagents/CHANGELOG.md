...
- Fixed detached subagent completions and terminal notifications that finish after an extension reload. Stale event-bus emits are now best-effort, and per-API notification ownership stays coherent when a completion subscription is retried after invalidation.
