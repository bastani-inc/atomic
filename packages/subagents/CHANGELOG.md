...
- Fixed root single-agent `progress` calls so `progress: true` is schema-valid and maintains a run-scoped `progress.md` under isolated artifact storage for foreground and background runs without overwriting a child-working-directory file, while keeping `includeProgress` as the separate final-result telemetry option.
