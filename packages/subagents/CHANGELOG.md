...
- Made the builtin debugger write-capable with the same tool list as `worker`, and updated its prompt to apply the smallest in-scope code or content fix after proving the root cause, rerun the failing scenario, and report the changes and validation rather than stop at recommendations.
