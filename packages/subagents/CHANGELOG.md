...
- Redesigned parallel worktree isolation to use filesystem-resolved canonical main roots, `.atomic/worktrees/` flattened locations, resettable `worktree-*` branches, origin-default/HEAD base selection, local settings and `.worktreeinclude` propagation, configurable directory symlinks, idempotent shared hooks-path setup, and forced idempotent worktree/branch cleanup.
