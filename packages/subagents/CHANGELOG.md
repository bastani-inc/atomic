...
- Fixed in-process foreground and async subagent result metadata to honor the workflow-stage Codex fast-mode scope. Eligible OpenAI and OpenAI Codex child models now expose `fast` consistently in live, continued, detached, and terminal result surfaces while main-chat children keep using `codexFastMode.chat` ([#2336](https://github.com/bastani-inc/atomic/issues/2336)).
