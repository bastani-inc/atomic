...
/**
 * Immediate response returned by runDetached().
 * Mirrors the accepted "run" action response shape used by the dispatcher.
 */
...
export function runDetached(
  ...
  // 4. Build executor opts — inject runId seam and signal.
  //    Destructure `jobs` out so it is not forwarded to RunOpts.
  const { jobs: _jobs, cancellation: _cancellation, ...restOpts } = opts;
