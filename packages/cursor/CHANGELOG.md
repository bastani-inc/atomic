...
- Made `GetUsableModels` the sole authority for runnable existence, exact IDs, display data, and Max state, preserving one model occurrence per returned row—including blank, whitespace-only, and duplicate IDs—in source order. Textual exact lookup uses the first occurrence, while selecting a later registered object retains that current occurrence's routing. Requests preserve the selected flat ID in both model structures, set its GetUsable Max in both, and leave `RequestedModel.parameters` empty; tuple expansion, backend-route construction, and route-rewriting reasoning selectors were removed ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Split `AvailableModels` into optional same-account image-only enrichment. One or more exact identity/variant matches enable images only when every match explicitly reports true, so duplicate or distinct agreeing evidence remains unambiguous while any false or omitted match is text-only. AvailableModels cannot add, remove, reorder, collapse, block, select, or parameterize GetUsable rows or provide display/Max data; model-family inference is not used ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
