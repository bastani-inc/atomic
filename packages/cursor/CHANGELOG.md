...
- Fenced stored-credential resolver invocations before discovery so a delayed superseded lookup—whether it returns an old account, no credential, or an error—cannot replace or revoke the newer account catalog, execution authority, cache, freshness state, or warnings, including after provider disposal ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
