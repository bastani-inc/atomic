...
The credential-free cache at `~/.atomic/agent/cursor-model-catalog.json` has a 30-minute TTL. Because it intentionally stores no account identity, the first authenticated use refreshes it; the TTL then suppresses repeat discovery only for the same in-memory credential identity. Credential changes refresh immediately, superseded requests cannot overwrite the latest account snapshot, and future-dated cache timestamps are not treated as fresh. Stale discovery is awaited for `atomic --list-models`; discovery failures are surfaced while retaining the previous catalog. Successful live registration does not depend on a successful cache write, and persistence failures remain available through refresh diagnostics. The bundled compatibility snapshot includes a conservative GPT-5.5 base ID but does not infer capabilities or parameter combinations from its names.
