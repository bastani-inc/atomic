...
describe("ModelRuntime image generation", () => {
	...
	it("inherits built-in image and classifier APIs for same-id and new extension models", async () => {
		const runtime = await createRuntime();
		const original = runtime.getModelOfType("image", "openrouter", "black-forest-labs/flux.2-flex")!;
		const { api: _api, ...definition } = original;
		runtime.registerProvider("openrouter", {
			models: [definition, { ...definition, id: "extension-image", name: "Extension image" }],
		});
		expect(runtime.getModelOfType("image", "openrouter", original.id)?.api).toBe("openrouter-images");
		expect(runtime.getModelOfType("image", "openrouter", "extension-image")?.api).toBe("openrouter-images");
		const jev = runtime.getModelOfType("classifier", "typesafe", "jev-latest")!;
		const { api: _classifierApi, ...classifierDefinition } = jev;
		runtime.registerProvider("typesafe", { models: [classifierDefinition] });
		expect(runtime.getModelOfType("classifier", "typesafe", "jev-latest")?.api).toBe(jev.api);
	});

	it("registers extension image and classifier models with caller headers winning over configured headers", async () => {
		const runtime = await createRuntime();
		...
		const caller = { headers: { "X-Operation": "caller", "X-Extra": "original" } };
		expect((await runtime.generateImages(image, context, caller)).stopReason).toBe("stop");
		expect((await runtime.classify(classifier, classifierContext, caller)).stopReason).toBe("stop");
		expect(observed.slice(2)).toEqual([
			{ apiKey: "extension-secret", headers: { "X-Operation": "caller", "X-Extra": "original" } },
			{ apiKey: "extension-secret", headers: { "X-Operation": "caller", "X-Extra": "original" } },
		]);
		const override = {
			...caller,
			transformHeaders: (headers: Record<string, string>) => ({ ...headers, "X-Operation": "transformed" }),
		};
		expect((await runtime.generateImages(image, context, override)).stopReason).toBe("stop");
		expect((await runtime.classify(classifier, classifierContext, override)).stopReason).toBe("stop");
		expect(observed.slice(4)).toEqual([
			{ apiKey: "extension-secret", headers: { "X-Operation": "transformed", "X-Extra": "original" } },
			{ apiKey: "extension-secret", headers: { "X-Operation": "transformed", "X-Extra": "original" } },
		]);
	});
