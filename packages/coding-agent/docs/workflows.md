...
The `/workflow resume` command mirrors `/resume` ergonomics. With no id, interactive sessions keep the live run picker when resumable live/paused runs exist **and also surface durable cross-session history** as a hint so the user can resume workflows from prior sessions without switching context; when no live runs exist it shows the durable-only picker. Headless sessions print the durable catalog. With a target id, Atomic first checks live runs in the current session; when the target id is not a live run, it falls back to the cross-session durable resume catalog and resumes by top-level workflow id. Resume re-dispatches the workflow with its cached inputs and the **original workflow id**, so completed `ctx.tool`, `ctx.ui`, stage/task/chain/parallel items, and child workflow boundaries return cached results instead of re-executing — DBOS-style replay without Postgres.
