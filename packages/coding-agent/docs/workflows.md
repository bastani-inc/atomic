...
- **Find in stage chat** - `ctrl+shift+f` in an attached stage chat opens a find box over *that* chat rather than the main transcript behind it. It searches the whole stage transcript, not the rows currently on screen, including every row of a long live answer still streaming and rows far above the window; the selected match is scrolled into view and highlighted with the same `searchMatchBg`/`searchMatchText` theme colors, underline, and inverse-and-bold current match the main transcript search uses. Live, paused, and read-only archived stage chats are all searchable; an archived question card has no transcript to search and declines the key. Opening, navigating, and closing route through the four `tui.altScreen.search*` actions alone — the bound `tui.altScreen.search` action outranks a scroll-shaped key, so a search remapped onto `pageUp` still opens the box — so remapping them moves the stage-chat search with them. `escape` (the default `searchClose`) closes the box only — it does not interrupt the stage or close the pane — and `ctrl+f` still queues a follow-up. Scrolling stays live underneath the box, and detaching with `ctrl+x`, a prompt, or a mounted question closes it.
...
The `ctx.tool(name, args, fn, options?)` primitive runs arbitrary TypeScript code as a first-class durable graph node and caches the result durably. The node is non-attachable and has no stage chat controls, and its graph card body is the constant `durable tool` in every state — status, timing, and dependency rows keep their own rows, and the card does not preview the result or error. In the graph viewer, focusing the node and pressing Enter, clicking it, or choosing it from the switcher opens one read-only agent-chat-style tool message block from the snapshot. It is collapsed by default: the block shows the tool name/call and a compact bounded result or error preview. The configured `app.tools.expand` action (`ctrl+o` by default) toggles the same block between the compact view and full bounded args, result or error, callback source captured from `fn.toString()` at registration, timing, and cached/replayed markers. Source capture never re-executes the callback and never reads a file. `↑`/`↓`, `PageUp`/`PageDown`, `Home`/`End`, the wheel, and the scrollbar all scroll the block, so a long payload stays readable in a keyboard-only session or a terminal without mouse reporting; Escape or `ctrl+x` returns to the graph. The message block is read-only and never offers chat attachment, steering, interrupt, or resume. Every field is bounded by one inspection cap and marks truncation explicitly with `… [truncated]`, timing wraps rather than being cut so `startedAt`, `endedAt`, and `duration` stay readable on a narrow terminal, callback source keeps its line structure with tabs expanded and control bytes shown as `\xNN`, and a cyclic payload, a throwing `toJSON`, or a throwing property getter renders `<cycle>`, `<unserializable>`, or `<unreadable>` instead of crashing the view. Hostile values are retained and displayed through typed payload boundaries with one own-key discovery call and bounded descriptor/value work, including collision-safe truncation markers for wide objects; the runtime's own key-list materialization remains outside this module's control. The same cap applies to what the live run snapshot retains for a tool node, so a multi-megabyte result cannot grow session, status, or picker surfaces, and a result holding a function, promise, proxy, symbol, or weakmap stays clonable for those surfaces. The durable checkpoint keeps the exact output, so a cached replay still returns the full original value and `argsHash` still hashes the raw args. `ToolNodeSnapshot.resultSummary` is unchanged and still ships in `workflow({ action: "status" })`. It is valid before, between, after, or without model stages, so a tool-only workflow completes normally; a workflow that returns normally without any stage, child, tool, or explicit exit remains invalid. On resume, if that ordinal tool call already completed (matched by call order plus content hash of `name` + `args`), the runtime returns the cached result without re-executing the function—ensuring completed side effects are not repeated while still preserving two intentional same-name/same-args calls as distinct ordered nodes. Legacy child checkpoints without topology keep that cached output authoritative even if the additive ownership-migration write is temporarily unavailable: current replay uses inferred child ownership, a later replay retries the metadata write, and fresh completed inspection falls back to root ownership with topology unavailable until a migration succeeds.
