...
- **Working animation lifecycle** - Ordinary attached-stage work keeps the same exact one-cell `∀` visible while pulsing regular → bold → regular every 80ms. Every agent and SDK turn resets to regular weight with a fresh lifecycle-relative cadence; turn, terminal, error, replacement, and disposal cleanup stop the active timer without stale repaint. Reduced motion uses a static un-emphasized `∀` without an animation timer; factual retry, fallback, compaction, cancellation, and error copy retains precedence.
