...
- **Working animation lifecycle** - Ordinary attached-stage work begins at the first `⠁` Atomic A frame on every agent and SDK turn start, advances from that lifecycle origin every 80ms, and stops at turn or terminal cleanup instead of inheriting wall-clock or prior-turn phase. Reduced motion uses the settled `⣵` frame without an animation timer; factual retry, fallback, compaction, cancellation, and error copy retains precedence.
