...
- **Fullscreen mouse routing** - In fullscreen TUI mode, a focused workflow graph or attached stage chat overlay receives wheel/trackpad and click input through the host's application-owned input route before the alternate-screen viewport. Events the overlay does not consume fall through to the viewport, while non-overlay focused components leave pi-tui's transcript scrolling, scrollbar interaction, and drag-selection path intact. Graph panning, stage-chat scrolling, and node click-to-attach therefore work without changing keyboard navigation.
...
- **Copy mode** - Press `ctrl+t` inside an attached stage chat to toggle **copy mode**. On regular hosts, copy mode disables workflow-chat mouse reporting so normal terminal/tmux text selection can work; press `ctrl+t` again to leave copy mode and restore transcript or prompt scrolling. In fullscreen, pi-tui owns the alternate-screen mouse baseline, so unconsumed copy-mode events use its viewport selection path. Archived read-only stage transcripts expose the same footer and copy-mode status, so their text can also be selected and copied; `esc` closes the transcript and `ctrl+x` returns to the graph. While copy mode is on, wheel/trackpad gestures are handled by the terminal/tmux in regular mode and may scroll terminal scrollback, so leave copy mode before using the wheel again.
