...
**Atomic repository `publish-release` chain.** The repository-local release workflow is a small sequential chain of capable model stages: prepare the versionless changelog branch; validate, commit, push, and open the release PR; inspect required CI once; merge the exact verified PR head and synchronize the selected base; run `scripts/cut-release.ts`; dispatch the protected publisher; inspect that publish run once; and summarize evidence. Inputs remain `target_version`, `release_kind`, and short `base_ref` (default `main`). Stable versions use `MAJOR.MINOR.PATCH`; prereleases use `MAJOR.MINOR.PATCH-alpha.REVISION`. The release PR changes changelogs only. Package manifests remain at the `0.0.0` placeholder on every supported base, and only the detached `Release <version>` tag commit receives the real version.

CI and publishing are one-shot, resumable human gates rather than wait loops. Each inspection returns a typed `passed`, `pending`, or `failed` outcome. `pending` suspends the same workflow run at a human-input node and tells the parent to end its turn; after GitHub advances, reconnect to or continue that same run and choose reinspection. `failed` presents the same structured choice instead of silently repairing, merging, tagging, redispatching, or publishing. A new inspection attempt runs only after that explicit answer. The workflow never uses watch mode, sleeps, polling loops, or a duplicate release workflow. Completed model stages and the accepted dispatch are durable checkpoints, so resuming the run does not repeat branch creation, merge, tag creation, or publisher dispatch.

After exact-head CI passes, the merge stage re-reads the captured PR identity and SHA and uses `gh pr merge <url> --match-head-commit <sha>` when a merge is still required. It then fast-forwards the selected base and proves local/remote equality and ancestry. The tag stage runs `bun run scripts/cut-release.ts <version> --base <base_ref> --push --yes`; that script creates a sole-parent detached release commit with canonical `Release-base-ref` and `Release-base-sha` trailers and pushes the immutable version tag without advancing the base. Before any model stage, the workflow checks for GitHub CLI 2.87.0 or newer and exits blocked when the prerequisite is missing. The dispatch stage explicitly runs `gh workflow run publish.yml --ref main -f version=<version>` exactly once. GitHub CLI returns the created run URL; the stage captures that exact URL and numeric ID, fails closed without redispatch if no identity is returned, and never scans run history. Publish inspection views only that exact run. Active runs are pending, identity mismatches or unsuccessful conclusions are failed, and success requires the protected integrity job to bind the remote tag to the expected release SHA.

`.github/workflows/publish.yml` has only a strict `workflow_dispatch` version input. It refuses any invocation not loaded and dispatched from current protected `main`, requires the exact workflow identity and revision, resolves the current remote tag, validates its sole parent and release-base trailers, checks current allowlisted-base containment, and deterministically regenerates the stamped tree before downstream jobs use the verified SHA. Repository permissions remain read-only by default. Only the final GitHub-hosted job, protected by the `npm-publish` environment, receives `contents: write` and `id-token: write`; npm OIDC publication uses no registry token. The workflow reconfirms the remote tag against the verified SHA immediately before each irreversible npm publish and immediately before GitHub Release creation.
