...
When exact evidence proves the PR is already `MERGED`, `publish-release` skips the merge command. The normal `OPEN` path uses an explicit selector plus `--match-head-commit`; both paths re-query required checks after the merge stage before final `MERGED` acceptance. Immutable evidence uses durable `ctx.tool` checkpoints, while mutable PR/check/branch/publish evidence refreshes on resume. Prior tags are reused only with `verified merge → tag parent → current base` ancestry; new tags require the exact current base parent. Pushing the tag is the sole publish signal: GitHub's native `create` event starts `publish.yml`, which is required to exist on the default branch; the integrity job verifies the protected-main workflow ref, pinned workflow SHA, event tag/SHA, and deterministic release tree before any privileged action. Final verification observes the matching `Publish <version>` run once and requires its protected `Verify release integrity` job log to name the exact verified tag SHA.
