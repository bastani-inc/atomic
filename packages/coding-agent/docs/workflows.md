...
Run-environment commands use an isolated configuration generated by `coder config-ssh` and one SSH control master per environment. Command calls reuse that master and cannot fall back to a fresh Coder connection if it disappears. On Windows control machines, Atomic selects Git for Windows' ControlMaster-capable OpenSSH instead of `GIT_SSH` variants such as Plink, requests Unix-style paths in Coder's generated config, and uses the master's proxy mode rather than unsupported direct multiplexed sessions. Windows workspace commands preserve empty and quoted argv entries when PowerShell 5.1 starts native executables. Execution streams stdout and stderr separately and distinguishes a remote exit from timeout, abort, or transport loss; bash-backed callers retain the built-in `timeout:<seconds>` and `aborted` error conventions.
