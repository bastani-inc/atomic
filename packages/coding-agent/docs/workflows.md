...
**How a review round is recorded.** Each `user-feedback-*` stage declares a structured output schema and finishes by returning a schema-validated answer: a `decision` of `approve` or `revise`, one entry per user note, one entry per accepted live change, and the annotated screenshot path when one exists. The stage's readable markdown report still appears in the transcript, but the structured answer is what the workflow reads, so a later turn cannot replace the review with a short wrap-up. Every round — approvals included — is written to `<artifact_dir>/feedback/iteration-N.json`, whose top level is that schema plus a nested `meta` block; the human-readable copy stays alongside it as `iteration-N.md`. The workflow reloads that file and drives the loop from it, and the next `generate-*` stage receives its path in `reads` and is told the file is the authoritative record of the round.

**The loop fails closed.** `revise` runs another `generate-*` round. `approve` exports only when the round captured no notes and no live changes — an approval that still carries captured work is read as `revise`, because exporting would discard what the user asked for. Anything else is *indeterminate*: the run stops with an error naming the stage and the persisted artifact rather than approving a preview whose review outcome is unknown. **Only the structured answer can approve.** The markdown report is a transcript record, so a `decision: approve` written only in prose is indeterminate, and so is a report that contradicts itself — two `decision`/`review_decision` labels, or any label the parser recognizes repeated with a different value (`display_method` and `preview_path` included, not only the fields the round captures), is never resolved by taking the first one. Prose that carries readable notes or accepted live changes is read as `revise`. A persisted round is restored only when the file is exactly what the workflow would have written for the round it holds — the record is re-serialized and compared against the file — and a round recorded as prose can never restore as an approval. Missing or malformed feedback never approves; only an explicit structured `decision: "approve"` with nothing captured, or the run-level `Skip remaining review rounds and export as-is` choice, approves an export.
