...
| `max_refinements` | number | no | `3` | Maximum fresh regenerations from the brief. The live review session itself is unbounded. |

The workflow establishes or loads project design context, extracts user-provided references, can browse curated galleries, writes a live `preview.html`, and keeps separate generator and review session lineages. It exports an HTML spec and implementation handoff once a review session says so. Browser-backed preview and review use the `playwright-cli` skill when available. Research context moves between stages as artifact files rather than inline prompt payloads: the composed project design context is written to `<artifact_dir>/design-context.md` and the curated references brief to `<artifact_dir>/references.md`; `reference-discovery` reads the design context, and the generate and exporter stages read both files via `reads` with explicit read instructions. Only small bounded payloads — the user's verbatim notes and the word-capped prior design summary — travel inline, so one oversized research result cannot become one oversized prompt message.

**Where the review gate appears.** The browser review inside a `user-feedback-*` stage is a long-poll, not an `awaiting_input` graph node, so the stage itself reports `running` while it waits. Each session therefore pauses first at a deterministic run-level prompt: the needs-attention badge fires, and the prompt names the preview path, the `file://` URL, and how many regenerations are left. Answer `Start live review` to begin the browser session — the stage prints the live `http://` review URL in its first lines of output, visible via `/workflow connect <run-id>` — or `Skip remaining review rounds and export as-is` to accept the current design and move to export. In headless runs the gate is skipped and the review degrades as before. A feedback stage that fails outright fails the run.

**One review session per generated design.** The `live` session is itself unbounded: the user picks an element, receives three on-brand variants, and accepts one — which is written into `preview.html` in place — steers the page, and keeps going until they leave. Nothing that happens inside the session needs a following `generate-*` round to apply it, so the session ends one of two ways. `export` sends the preview to the exporter as it stands. `regenerate` runs one more `generate-*` from the brief, design context, references, and the user's notes — a fresh pass rather than an in-place revision — and then opens the next review session. Starting over when the user rejects the direction rather than the details is the one thing the live session cannot do itself, which is the only reason the outer loop survives.

**How a session is recorded.** The `user-feedback-*` stage declares a structured output schema and finishes by returning a schema-validated answer: a `decision` of `export` or `regenerate`, one entry per user note, one entry per accepted live change, and the annotated screenshot path when one exists. Every session — exports included — is written to `<artifact_dir>/feedback/iteration-N.json`, whose top level is the schema fields plus `meta.iteration`, `meta.stage_name`, and `meta.captured_at`; the human-readable copy stays alongside it as `iteration-N.md`. The loop uses the structured answer in memory, while the next `generate-*` stage receives the JSON path in `reads` and is told the file is the authoritative record of the session. That structured answer is the only decision channel, and the stage runner guarantees every resolved schema-backed stage returns a valid one, so no alternate path exists.

**The workflow owns the poll loop.** In a chat session the agent polls the `impeccable live` helper itself and decides when the review is over. A workflow stage boundary is a commit point, and the live contract's `"timeout" -> LOOP` is an instruction to a model rather than a constraint, so a stage that concluded on a poll timeout would return a perfectly valid answer for a review the user was still in. Instead, the workflow drives the protocol: a `user-feedback-N-start` stage boots the session and prints the review URL, then durable `live-poll-N-M` tool nodes poll the helper, `live-generate-*` / `live-steer-*` / `live-manual_edit_apply-*` stages handle exactly the events that need a model, and `live-reply-N-M` tool nodes acknowledge each one. `accept`, `discard`, and `prefetch` are acknowledged by the poll script itself and mint no stage; `timeout` is absorbed inside the poll node, so an idle hour costs no graph nodes. The loop ends on the helper's own `exit` event, and only then does the `user-feedback-N` stage report the session as the structured deliverable. Termination is an event, not a judgment. The impeccable skill ships inside Atomic, so the live scripts always resolve — from `dist/builtin/workflows/skills/impeccable/scripts` in an installed build, or `packages/workflows/skills/impeccable/scripts` in the repository. A project that vendors its own copy under `.agents/skills/impeccable/scripts` takes precedence, so a fork can be driven without rebuilding Atomic. There is no model-driven fallback: a stage that owns the poll loop decides for itself when a review ended, which is the defect this design removes.

**`max_refinements` bounds regenerations, not review.** With the default of `3` a run performs `generate-1` plus at most three regenerations, while the review sessions between them stay unbounded. When a session asks to regenerate and the budget is already spent, the run exports what exists and states the dropped request in the returned design summary rather than discarding it silently; `approved_for_export` stays `false`, because nobody chose to export.
