...
Run-environment commands use an isolated configuration generated by `coder config-ssh` and one SSH control master per environment. Command calls reuse that master and cannot fall back to a fresh Coder connection if it disappears. On Windows control machines, Atomic selects Git for Windows' ControlMaster-capable OpenSSH instead of `GIT_SSH` variants such as Plink, requests Unix-style paths in Coder's generated config, escapes literal percent signs in control-socket and proxy-command paths, and executes through the master's proxy mode rather than unsupported direct multiplexed sessions. Windows workspace commands preserve explicitly empty environment-variable values as well as empty and quoted argv entries when PowerShell 5.1 starts native executables, and native children inherit the requested checkout as their working directory. Execution streams stdout and stderr separately and uses the SSH client's own exit status. Because SSH reserves status 255 for both remote exit 255 and transport errors, Atomic accepts a remote 255 only when the client records receipt of that status in a private per-command local log that the remote command cannot read or write. Abort, timeout, master-loss, and close paths use bounded termination waits and escalate from termination to forced kill, while bash-backed callers retain the built-in `timeout:<seconds>` and `aborted` error conventions.

Run-environment `read`, `write`, `edit`, and `ls` tools select POSIX or Windows commands from the ready workspace agent's reported operating system; no infrastructure-provider property participates. Every accepted public tool call crosses the execution transport exactly once. All four tools resolve paths with the workspace's syntax and leave `~` relative to its checkout instead of expanding it from the control machine's home directory. Reads and listings gather the complete response in that command, writes perform generated-file protection and executable-bit handling remotely, and edits apply one complete checkout-relative patch so nested paths remain intact. Write content and edit patches travel through standard input, and Windows edits retain CRLF and mixed line endings even when the final line is unterminated. Selectors that require control-machine-only URL, archive, SQLite, internal-resource, or notebook handling fail before transport.
