...
The `/workflow resume` command mirrors `/resume` ergonomics. With no id, interactive sessions show a **combined selector** that lists both live/paused runs and cross-session durable workflows together so the user can choose from all resumable workflows in one view; when no live runs exist it shows the durable-only picker. On a successful durable resume, the overlay connects to the resumed run automatically (matching live resume behavior). Headless sessions print the durable catalog. With a target id, Atomic first checks live runs in the current session; when the target id is not a live run, it falls back to the cross-session durable resume catalog and resumes by top-level workflow id, then opens the overlay. Resume re-dispatches the workflow with its cached inputs and the **original workflow id**, so completed `ctx.tool`, `ctx.ui`, stage/task/chain/parallel items, and child workflow boundaries return cached results instead of re-executing — DBOS-style replay without Postgres. Stale session-cache entries for workflows the durable backend knows are terminal (completed/cancelled/non-resumable) are suppressed during listing and resume, so terminal workflows cannot be resurrected from old JSONL cache metadata.
