...
- **Find in stage chat** - `ctrl+shift+f` in an attached stage chat opens a find box over *that* chat rather than the main transcript behind it. It searches the whole stage transcript, not the rows currently on screen, including the part of a live answer still streaming and rows far above the window; the selected match is scrolled into view and highlighted with the same `searchMatchBg`/`searchMatchText` theme colors the main transcript search uses. Live, paused, and read-only archived stage chats are all searchable; an archived question card has no transcript to search and keeps the key unbound. Navigation is the ordinary `tui.altScreen.searchNext`, `tui.altScreen.searchPrevious`, and `tui.altScreen.searchClose` bindings, so remapping them moves the stage-chat search with them. `escape` (the default `searchClose`) closes the box only — it does not interrupt the stage or close the pane — and `ctrl+f` still queues a follow-up. Scrolling stays live underneath the box, and detaching with `ctrl+x`, a prompt, or a mounted question closes it.
