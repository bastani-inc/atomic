...
- **Fullscreen mouse routing** - In fullscreen TUI mode, the focused workflow graph or attached stage chat receives wheel/trackpad and click input through the host's application-owned input route before the alternate-screen viewport. Events the component does not consume fall through to the viewport, so graph panning, stage-chat scrolling, and node click-to-attach work without changing keyboard navigation.
