...
When `/workflow resume` lists or resumes a DBOS-backed workflow in a fresh process, Atomic first hydrates its in-memory replay mirror from DBOS. Checkpoints are stored as structured DBOS outputs containing the checkpoint kind, id, tool argument hash, UI prompt hash, stage replay key, and completed output, so replay can skip completed `ctx.tool`, `ctx.ui`, and `ctx.stage` work without relying on prior in-process state. Atomic root metadata is mirrored as versioned DBOS records and the latest timestamp wins during hydration, so mutable status, checkpoint counts, pending prompt counts, and resumability update over time even though DBOS workflow ids are idempotent. Older DBOS records that contain only a raw output are still read as generic stage checkpoints.
