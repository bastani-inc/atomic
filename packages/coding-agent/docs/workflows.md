...
An admitted external message is an absolute nomination boundary for the prompt window: the runner persists the last non-empty assistant turn before the first admission. If no assistant text precedes the admission, it uses the first non-empty assistant turn after it as the only available fallback; with no admission, existing last-assistant behavior is unchanged. The admitted turn remains visible in model context and in the companion transcript, but is never selected as the curated artifact. Therefore, if a stage emits a short preamble, waits for an async subagent completion, and only then produces its real report, the preamble is persisted because the report belongs to the externally admitted turn. The receipt adds `WARNING: the stage's own turn was persisted; larger post-admission assistant content exists. Search the companion transcript at <path>.` when the discarded content is substantially larger; search that transcript with `rg` and structure the stage to return its deliverable before the admission when the warning appears.
...
1. **One owner per artifact.** The runner writes the stage's nominated final message to `output` after the stage ends and automatically writes the companion transcript outside the repository tree. Return the complete deliverable in the assistant turn before any admitted external message; if the receipt warns that larger post-admission content exists, search the companion transcript with `rg` instead of assuming the curated artifact contains it. Do not also ask that stage's prompt to author the same output path; return the content as the stage's final message and let the runner save it, or have the prompt write a path the stage does not declare as `output`.
