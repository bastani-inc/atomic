...
**How a review round is recorded.** Each `user-feedback-*` stage declares a structured output schema and finishes by returning a schema-validated answer: a `decision` of `approve` or `revise`, one entry per user note, one entry per accepted live change, and the annotated screenshot path when one exists. Every round — approvals included — is written to `<artifact_dir>/feedback/iteration-N.json`, whose top level is the schema fields plus `meta.iteration`, `meta.stage_name`, and `meta.captured_at`; the human-readable copy stays alongside it as `iteration-N.md`. The refinement loop uses the structured answer in memory, while the next `generate-*` stage receives the JSON path in `reads` and is told the file is the authoritative record of the round.

**The loop has two outcomes.** `revise` runs another `generate-*` round. `approve` exports only when the round captured no notes and no live changes — an approval that still carries captured work is treated as `revise`, because exporting would discard what the user asked for. The schema-backed result is the sole decision input; each round is persisted for the next stage and for durable inspection. The stage runner guarantees that every resolved schema-backed stage returns a valid structured answer, so no alternate decision path is needed.
