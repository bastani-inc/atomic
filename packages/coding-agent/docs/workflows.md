...
Run-environment commands use an isolated configuration generated by `coder config-ssh` and one SSH control master per environment. Command calls reuse that master and cannot fall back to a fresh Coder connection if it disappears. Execution streams stdout and stderr separately and distinguishes a remote exit from timeout, abort, or transport loss; bash-backed callers retain the built-in `timeout:<seconds>` and `aborted` error conventions.
