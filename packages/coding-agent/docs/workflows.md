...
Let `P` be the last non-empty assistant text before that first admission. When `P` exists, it remains the default unless the largest post-admission candidate is both at least 26 UTF-8 bytes and at least 3:2 larger than `P`; the 26-byte floor admits the shortest known genuine deliverable (`short but real deliverable`) while excluding normal terse acknowledgements, and the 50% ratio prevents marginal formatting growth from changing the result. When `P` does not exist, the runner selects the most recent post-admission candidate at or above the same floor, falling back to the latest non-empty candidate if every turn is terse. With no admission, existing last-assistant behavior is unchanged. This is deliberately heuristic: a terse genuine deliverable followed by a verbose acknowledgement can still be nominated incorrectly.

A receipt warning is added only when nomination actually passes over plausibly substantive assistant text, and every warning names the companion transcript. A pre-admission selection says `WARNING: the stage's own pre-admission turn was persisted; post-admission assistant content was discarded. Search the companion transcript at <path>.`; a no-pre-admission fallback says `WARNING: no stage-own assistant text existed before the admission; a post-admission assistant turn was selected and other post-admission assistant content was discarded. Search the companion transcript at <path>.`; and a size-qualified override says `WARNING: a substantially larger post-admission assistant turn was persisted; the pre-admission assistant turn was discarded. Search the companion transcript at <path>.` A terse trailing admission acknowledgement does not produce a warning. Search the transcript with `rg` and read only the narrow ranges you need.
...
1. **One owner per artifact.** The runner writes the stage's nominated final message to `output` after the stage ends and automatically writes the companion transcript outside the repository tree. Return the complete deliverable in the stage's own assistant turn before any admitted external message. If no assistant text can precede an admission, the runner selects the largest post-admission turn as its fallback; when it discards other post-admission text, the receipt names the transcript with a `WARNING:` and no size threshold. Search that transcript with `rg` instead of assuming the curated artifact contains every later turn. Do not also ask that stage's prompt to author the same output path; return the content in the stage's own turn and let the runner save it, or have the prompt write a path the stage does not declare as `output`.
