...
The runner writes the stage's **nominated final message** to `output` after the stage ends, so that path belongs to the runner. An admitted external turn (for example, an async subagent completion) remains visible to the model and in the companion transcript. Nomination considers assistant text on both sides of the first admission in the current prompt window; a stage that declares `output:` also automatically gets a full, rendered, line-oriented transcript of its session.

Let `P` be the last non-empty assistant text before that first admission, and let `L` be the latest post-admission candidate at or above 26 UTF-8 bytes. When `P` exists, it remains the default unless `L` exists and is at least 3:2 larger than `P`; the gate is evaluated against `L`, not an earlier, larger post-admission turn. The 26-byte floor admits the shortest known genuine deliverable (`short but real deliverable`) while excluding normal terse acknowledgements, and the 50% ratio prevents marginal formatting growth from changing the result. When `P` does not exist, the runner selects `L`, falling back to the latest non-empty post-admission candidate if every turn is below the floor. With no admission, existing last-assistant behavior is unchanged. This is deliberately heuristic: a genuine deliverable followed by a verbose acknowledgement can still nominate the acknowledgement.

Every warning names the companion transcript. When `P` is persisted, the receipt warns whenever any non-empty post-admission assistant turn was discarded, including turns below the 26-byte floor: `WARNING: the pre-admission assistant turn was persisted; all non-empty post-admission assistant turns were discarded. Search the companion transcript at <path>.` A no-pre-admission fallback warns only when another plausibly substantive post-admission turn was discarded: `WARNING: the latest plausibly substantive post-admission assistant turn was persisted; other plausibly substantive post-admission assistant turns were discarded. Search the companion transcript at <path>.` An override always warns because it discards `P`: `WARNING: the latest plausibly substantive post-admission assistant turn was persisted after meeting the 26-byte and 3:2 override gate; the pre-admission assistant turn and all other post-admission assistant turns were discarded. Search the companion transcript at <path>.` Search the transcript with `rg` and read only the narrow ranges you need.

The companion transcript is written once under the durable Atomic config root at `~/.atomic/workflows/runs/<runId>/transcripts/` (or the equivalent configured agent root; `ATOMIC_WORKFLOW_ARTIFACT_DIR` overrides that root). It is never placed inside the repository tree or OS temporary storage: a home-scoped durable location survives both worktree deletion and OS temp purges, and staying outside the repo keeps full tool output — which may contain secrets — from being committed accidentally. Run-scoped artifact directories are pruned only when their durable/live run record is terminal (or the directory is an unowned orphan) and older than the exported `WORKFLOW_ARTIFACT_RETENTION_MS` policy. Running, paused, quit, blocked, and awaiting-input runs are exempt indefinitely because their artifacts are live resume dependencies. When a terminal durable owner is aged out, the durable entry is deleted first; if authoritative deletion is unavailable or refuses, the artifact directory is preserved. Goal ledgers, Ralph implementation notes, and QA video paths share that same durable root and retention policy. The receipt names both absolute paths. Search the transcript with `rg`, then read only the narrow line ranges you need; do not read the whole transcript into a downstream prompt. The transcript is a secondary searchable record; the output artifact remains the curated handoff.

If the nominated artifact is empty or only points at its own output path—including terse forms such as `Wrote report.md` or `Saved: report.md`—the receipt adds an explicit `WARNING` directing the reader to the companion transcript instead of silently treating the degenerate write as valid. Substantive prose that merely mentions the path is not treated as a pointer.
...
1. **One owner per artifact.** The runner writes the stage's nominated final message to `output` after the stage ends and automatically writes the companion transcript outside the repository tree. Return the complete deliverable in the stage's own assistant turn before any admitted external message. Around an admission, the latest post-admission turn at or above 26 UTF-8 bytes is the only override candidate: it replaces the last pre-admission turn only when it is at least 3:2 larger. Without a pre-admission turn, that latest qualifying turn is selected, or the latest non-empty turn when all are below the floor. A receipt `WARNING:` names the transcript whenever an override discards the pre-admission turn, a pre-admission selection discards any non-empty post-admission turn, or a no-pre-admission fallback discards another qualifying turn. Search that transcript with `rg` instead of assuming the curated artifact contains every later turn. Do not also ask that stage's prompt to author the same output path; return the content in the stage's own turn and let the runner save it, or have the prompt write a path the stage does not declare as `output`.
...
await ctx.task("researcher", {
  task: "Map the subsystem and return the complete report in your own assistant turn before any admitted external completion; the workflow saves the nominated turn.",
  output: researchPath,
  outputMode: "file-only",
});
