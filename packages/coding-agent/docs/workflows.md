...
Ralph's orchestrator leaves the subagent tool's explicit `model` argument unset by default, so each named agent uses the model and fallback policy declared in its agent definition. The sole automatic exception applies when a delegated agent has no configured/default model policy: the orchestrator reads `packages/coding-agent/docs/models/model-selection.md`, follows its Pareto-efficient role recommendations, calls `workflow({ action: "models" })`, and may pin only an available returned `fullId`, adding a thinking suffix only when that exact level appears in the entry's `availableThinkingLevels`. If no recommendation intersects the catalog, it leaves the child unpinned, records the limitation, and asks the user only when the task truly needs an explicit model; an empty catalog stays unpinned and is recorded as returning no configured models. It does not inspect credentials or infer why models are absent. Outside that sole automatic path, a model override requires an exact user request or documented task-specific need, with the override and reason recorded before launch; Ralph never invents an ad hoc model ID merely for diversity. Before its first delegation, the orchestrator also creates one invocation-specific, non-`default` Intercom group name and passes that same explicit `group` value to every delegated subagent, including parallel and follow-up work. Delegates stay isolated from the shared `default` group while `contact_supervisor` keeps its broker-authorized cross-group path back to the orchestrator.
