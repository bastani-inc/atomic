...
Workflow run identifiers are shown in full everywhere they are presented to users: the `BACKGROUND` panel, workflow status and detail views, run pickers, control messages, and awaiting-input attribution banners. Every command and workflow-tool action that accepts `runId` also accepts a **unique 8-character hexadecimal prefix** of that UUID. A 32-character dashless value and every other truncated or malformed form are rejected. Prefixes are resolved only within the feature's authoritative visible live/durable/completed scope; if more than one full UUID shares the prefix, Atomic lists the matches and asks for the full UUID rather than choosing one by ordering.
...
workflow({ action: "status", runId: "<8-hex-prefix>" })          // same, when the UUID prefix is unique
...
- `runId` accepts either the full 36-character run UUID or a unique 8-character hexadecimal prefix for every lifecycle and inspection action, including `status`. User-facing surfaces still print the canonical full value. Other truncations are malformed, and collisions across the authoritative visible live/durable/completed namespace list every match and require the full UUID. Status lists and run pickers show top-level user-launched workflows; nested child runs are implementation details of the expanded parent graph.
- `status`, `stages`, `stage`, and `transcript` with an explicit `runId` first use the current session store, then inspect the durable catalog when no local run matches. This is inspection only: Atomic does not claim ownership, change status, run workflow code, or resume the workflow. A stale durable `running` root is shown as `crashed` with its resumability and an explicit `/workflow resume <id>` hint; fresh work owned by another Atomic process remains `running`, offers read-only status guidance, and stays protected from local control or resume. Deleted/tombstoned, absent, malformed, ambiguous, cyclic, orphaned, nonreciprocal, out-of-scope, and duplicate-node records report distinct failures instead of inventing a partial graph. `status` without `runId` remains current-session-only and never scans durable history.
...
- An explicit workflow-tool `resume` target that is absent from the current session store triggers targeted DBOS discovery before Atomic returns `Run not found`. A full run UUID or unique 8-character hexadecimal prefix resolves across the live, resumable, and completed root-run catalogs and resumes under the canonical full workflow ID. Ambiguous prefixes list every matching full UUID, while other malformed truncations are rejected before any durable lookup. Resource-loading and durable-backend failures remain visible. Ordinary workflow-tool `status` listing stays session-local and does not eagerly hydrate durable history.
- Targeted durable inspection is separate from resume. `status`, `stages`, `stage`, and `transcript` may resolve a full UUID or unique 8-character hexadecimal prefix and hydrate one missing-local root for read-only inspection, but they never claim it or execute replay. Only an explicit `resume` action enters the claim-and-dispatch path.
