...
- **Crash-safe execution ownership**: Default file-backed durability stores each root workflow in its own atomically lock-protected JSON file under `~/.atomic/workflow-durable`; workflow IDs are percent-encoded so filenames remain valid on Windows. File execution leases record host, PID, process-generation identity, and a per-dispatch token while refreshing a heartbeat. Same-host PID/generation liveness prevents debugger stops or event-loop stalls from creating split-brain; dead same-host owners are reclaimed immediately, process-generation identity is derived independently of locale/timezone so a same-host owner is never mistaken for a reused PID, and only the matching dispatch token can release ownership. A foreign-host file lease is treated conservatively as active and is **not** heartbeat-reclaimed, because a stale heartbeat from a long synchronous stage or a shared-filesystem stall is indistinguishable from a dead owner; file durability is therefore a same-host store, and cross-host coordination must use the DBOS/PostgreSQL backend, whose session advisory locks coordinate ownership across hosts and release automatically if the owning database connection/process crashes (clear a genuinely abandoned foreign-host file lease with `/workflow kill`). Both backends hide active owners, refuse targeted duplicate resume, retain ownership through paused metadata until the executor stops, and flush durable registration before reporting resume success. State-lock acquire/release uses atomic rename to prevent handoff races. Completed/cancelled/non-resumable files are pruned, and file writes use `0700` directories and `0600` files where supported. Durable replay identities use SHA-256 over canonical JSON.
