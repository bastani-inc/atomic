...
The runner writes the stage's **nominated final message** to `output` after the stage ends, so that path belongs to the runner. An admitted external turn (for example, an async subagent completion) remains visible to the model and in the companion transcript. When an assistant turn already exists before that admission, the boundary below keeps later assistant turns out of the curated artifact; when no such turn exists, the runner uses the documented fallback. A stage that declares `output:` also automatically gets a full, rendered, line-oriented transcript of its session.

An admitted external message is an absolute nomination boundary for the prompt window: the runner persists the last non-empty assistant turn before the first admission. If no assistant text precedes the admission, no stage-owned candidate exists, so it selects the largest non-empty post-admission assistant turn by UTF-8 byte length, preferring the later turn on a tie; with no admission, existing last-assistant behavior is unchanged. This fallback may select a post-admission turn because the prohibition cannot be satisfied in that window, but it prefers substantive report text over a short acknowledgement. Whenever any non-empty post-admission assistant text is discarded, the receipt adds a `WARNING:` with no size threshold, scoped to the same prompt window. For a pre-admission nomination the warning says `WARNING: the stage's own pre-admission turn was persisted; post-admission assistant content was discarded. Search the companion transcript at <path>.`; for the fallback it says `WARNING: no stage-own assistant text existed before the admission; a post-admission assistant turn was selected and other post-admission assistant content was discarded. Search the companion transcript at <path>.` If a stage emits a short preamble, waits for an async subagent completion, and only then produces its real report, the preamble is persisted and the receipt points to the transcript; structure the stage to return its complete deliverable before the admission when possible. Search the transcript with `rg` and read only the narrow ranges you need.
...
If the nominated artifact is empty or only points at its own output path, the receipt adds an explicit `WARNING` directing the reader to the companion transcript instead of silently treating the degenerate write as valid.
