...
Created by `/compact`, RPC `compact`, and automatic compaction. For model-driven compaction, `summary` is the mechanically reconstructed verbatim transcript string. Model-driven boundaries use the additive **full-collapse** format (`details.format: "full-collapse"`, `promptVersion: 4`), where the whole current context collapses into the string; for those, `firstKeptEntryId` is a self-anchor (the boundary's parent leaf) and reconstruction replays only entries appended after the boundary. A trusted extension override instead persists its arbitrary non-whitespace bytes exactly as a format-absent legacy hybrid (`promptVersion: 3`, `rung: "extension"`), and `firstKeptEntryId` points to the first structured kept-tail message.
...
On rebuild, Atomic emits the durable `summary` as a synthesized visible custom message. Reconstruction dispatches on `details.format`, not `details.rung`: full-collapse entries then emit only entries appended strictly after the boundary (no pre-boundary tail), while format-absent legacy hybrids emit original structured entries beginning at `firstKeptEntryId`. This exact string survives resume without rerunning a planner. `details.rung` is `"planned"` or `"extension"` (older sessions may carry `"standard"`, `"critical"`, or `"deterministic"`); `details.backupPath` is optional. `details.cache` is optional prompt-cache telemetry for the compaction request (`cacheReadTokens`, `cacheWriteTokens`, `cacheHit`, `provider`, `model`), present only for model-run full-collapse compactions and absent for extension-provided ones.

Compaction request fitting never rewrites these durable records. Provider admission uses exact counts when injected/available; otherwise API-aware request-bound prompt occupancy (Anthropic near-equal cache mirrors count once; OpenAI/Codex partitions sum, always excluding output) may be combined with one suffix only after complete captured-payload restoration. Warm construction deep-clones the captured JSON-wire body, preserves all captured outer fields and historical items, appends exactly one verified provider-shaped suffix, and fails closed to isolated planning on any ambiguity. Explicit Responses, Chat/Completions, Anthropic, Google, Bedrock, and Mistral projections provide confidence-typed whole-input fallback; unknown APIs are labeled generic, while opaque/cyclic or unbounded image/file projections remain permissive with `unavailable` confidence. Typed local input/headroom or warm-restoration failures advance through at most `warm → isolated-full → isolated-elided`; local output-budget failures remain terminal, while a retry-guard block skips its current projection without masquerading as provider overflow or replacing the latest real retry origin. After provider context rejection or conditional headroom-limited invalid `length` output, every later transport must have changed input/transport fingerprints and strictly fewer provider-visible input bytes. Guard state is updated atomically only from a proven final payload with both fingerprints and its byte count; incomplete thrown-overflow evidence fails closed, and there is never a fourth projection. The final request-local elided view omits only unprotected transcript lines while retaining original line numbers and all protected bytes; every result is still validated and reconstructed against the unchanged original region. Atomic caps each oversized protected tool result to its first 16,000 characters plus the deterministic omitted-character marker in the canonical planner/boundary text. That bounded text—never a raw structured replay—is what reopened active context and later compaction carry. The original `SessionMessageEntry`, including full raw content, optional fields, repeated/aliased argument values, tool IDs, and intentional unmatched partial tool calls, remains byte-semantically unchanged in append-only JSONL, backups, and pre-boundary branches. Failed local input-budget/output-limit attempts, observed provider overflows, and stale plans append neither a `CompactionEntry` nor a backup. If persistence of a fully planned compaction entry fails after its recovery backup was created, the active JSONL and live leaf/index roll back exactly while that standalone backup is intentionally retained as a recovery artifact. A successful boundary remains append-only, and reopen/branch reconstruction uses the durable summary plus post-boundary entries exactly as described above.
