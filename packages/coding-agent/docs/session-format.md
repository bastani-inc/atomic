...
Compaction request fitting never rewrites these durable records. Provider admission uses exact counts when injected/available; otherwise request-bound prompt occupancy (`input + cacheRead + cacheWrite`, excluding output) may be combined with one suffix only after proving the historical provider input and every other model-visible field unchanged. Explicit Responses, Chat/Completions, Anthropic, Google, Bedrock, and Mistral projections provide confidence-typed whole-input fallback; unknown APIs are labeled generic, while opaque/cyclic or unbounded image/file projections remain permissive with `unavailable` confidence. On fit failure or a recognized provider context rejection, Atomic enters at most `warm → isolated-full → isolated-elided`. Every retry must be below the smallest rejected/blocked provider-visible input size; separate input and exact post-hook/post-output-limit transport fingerprints prevent identical resends, and there is never a fourth attempt. The final request-local elided view omits only unprotected transcript lines while retaining original line numbers and all protected bytes; every result is still validated and reconstructed against the unchanged original region. Atomic may also cap an oversized tool result in the temporary provider request used to choose retained lines. The original `SessionMessageEntry`, including raw content, optional fields, repeated/aliased argument values, tool IDs, and intentional unmatched partial tool calls, remains byte-semantically unchanged. Failed local input-budget/output-limit attempts, observed provider overflows, and stale plans append neither a `CompactionEntry` nor a backup. If persistence of a fully planned compaction entry fails after its recovery backup was created, the active JSONL and live leaf/index roll back exactly while that standalone backup is intentionally retained as a recovery artifact. A successful boundary remains append-only, and reopen/branch reconstruction uses the durable summary plus post-boundary entries exactly as described above.
