...
Cursor advertises only reasoning levels carried by discovered parameter variants and preserves each complete preset. Fast service, thinking, effort, and Max/long-context modes remain separate; family names and fallback-ID suffixes do not synthesize levels, and explicitly unsupported levels fail instead of redirecting. Exact normal/Max context and output values are used only when supplied together for that corresponding model/mode. Missing fields degrade to clearly provenance-marked operational budgets and text-only input rather than family-name or display-label guesses. Schema-v2 cache files are scoped by a one-way digest of the stable authenticated account subject (never by storing or hashing an OAuth token), have a 30-minute TTL, and cannot be reused by another account. `--list-models` awaits a stale authenticated refresh; on refresh failure it warns and lists the retained scoped catalog. Cursor requests without exact discovered backend routing metadata fail instead of silently sending the picker/display ID.
