...
GetUsable is the sole authority for runnable existence, exact IDs, display data, and Max state. Atomic registers one row per exact returned ID and sends that ID unchanged in both Cursor request model structures. Max is copied to both structures, while `RequestedModel.parameters` remains empty. Atomic does not expand AvailableModels parameter tuples, synthesize concise/backend variants, expose a reasoning selector that rewrites the route, or use aliases, fuzzy matching, nearest-effort selection, another model/provider, or static rows to rescue an unavailable explicit Cursor ID.

AvailableModels is queried separately and only to enrich an exact GetUsable route with an unambiguous same-account image flag. It cannot change the executable set, route identity, display data, Max state, or parameters, and its failure cannot block text usage. Missing or ambiguous enrichment is text-only rather than a family-name guess. Schema-v3 cache files retain only exact GetUsable-derived routes and optional same-account image flags for 30 minutes. Schema-v1, schema-v2, unscoped, and parameterized caches are ignored; only a fresh same-account v3 snapshot may bridge a temporary GetUsable failure. Older experimental Cursor IDs are not migrated: reselect from `/model` when a saved setting, workflow, or session names an ID no longer returned. Strict workflow routes must complete authenticated discovery before listed rows are trusted, and an exact saved default is retried after deferred interactive extensions load without overriding an explicit or restored-session selection.
