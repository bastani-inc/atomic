...
## Cursor exact route occurrences

Cursor (experimental) uses the usual `provider/model` text, but the route part is the exact case-sensitive string returned by the authenticated Cursor catalog. Atomic does not normalize it, append a Max/thinking suffix, or derive aliases. Duplicate rows are preserved, so several selectable occurrences may intentionally share the same public `cursor/<route-id>` text.

The `/model` picker distinguishes Max state and occurrence in presentation labels without changing `model.id`. A provider-plus-ID reference succeeds only when it identifies one occurrence; duplicate text is explicitly ambiguous and must be selected in the picker. Settings and sessions persist a versioned provider-owned record for the exact account scope, route ID, absent/false/true Max state, and one-based occurrence. Restore resolves those named fields to the exact current occurrence or requires reselection. Same-version extra metadata is accepted but ignored; provider/ID-only records, unsupported versions, and missing or wrong named identity fields are not migrated, aliased, or sent through a fallback route/provider.

Every Cursor route is text-only. Current user images fail before the Run; live image tool results fail before that result content is transported. Historical image blocks are omitted while every adjacent text block remains verbatim and ordered without inserted separators; orphan text tool results are retained without synthetic calls. Every route also uses the same conservative Atomic accounting bounds of 200,000 context tokens and 64,000 output tokens. Those values are host budgeting policy, not Cursor-advertised limits, and do not affect catalog membership or route identity. `models.json` and model overrides cannot invent Cursor occurrences or replace the authenticated catalog.
