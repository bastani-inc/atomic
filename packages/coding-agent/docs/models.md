...
Built-in subscription providers such as Cursor (experimental) are selected with the same `provider/model` syntax, for example `cursor/composer-2.5`. Cursor model metadata remains text-only; current user-message images, including multiple images in the same current turn, are enabled by default through Atomic's experimental local private protobuf `selectedImages[].data` path. Cursor documents image support on other product/API surfaces, which do not validate this local private protobuf transport. Inline `@image.png` prompts, pasted absolute image paths such as `/tmp/atomic-clipboard-....png`, unescaped or shell-escaped macOS screenshot paths, `file://` image URLs from drag/drop, and pasted `<file name="/tmp/atomic-clipboard-....png"></file>` markers are attached to the current user message; historical user image blocks are tolerated on later turns but serialized as text-only history, while `read` tool image results are text-only for Cursor/text-only models and remain rejected at the Cursor transport boundary. Live private-API model metadata may fall back to estimated labels. Because Cursor support targets undocumented private endpoints with Cursor CLI-compatible headers, maintainers and users should explicitly accept the risk that it may conflict with Cursor's terms, break without notice, or affect the Cursor account used to authenticate.
