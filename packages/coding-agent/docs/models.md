...
The interactive `/model` selector renders the current authenticated snapshot immediately and refreshes network-backed catalogs in the background for up to 15 seconds. In isolated-engine sessions that deadline is enforced inside the engine as well as the terminal frontend, and model selection does not queue behind the refresh. A slow catalog therefore falls back to cached models without requiring an `auth.json` or `~/.atomic` reset.
