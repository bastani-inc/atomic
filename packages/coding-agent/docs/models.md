...
Built-in subscription providers such as Cursor (experimental) use the same `provider/model` syntax, for example `cursor/composer-2`. Cursor first discovers the authenticated account's private `AvailableModels` snapshot and uses `GetUsableModels` only as a compatibility fallback. Availability remains account-, plan-, region-, server-, and client-version-dependent.

Cursor treats AvailableModels parameter definitions and complete allowed variants as separate data: field 29 provides display labels, while field 30 provides the routable tuples. Atomic exposes only those complete tuples and groups rows only when effort is their sole difference. Each row has a concise effort-neutral ID and a name built from Cursor's actual display name plus necessary mode labels, such as `cursor/claude-fable-5-1m-max` with `Fable 5 (1M, Max)`; thinking, fast, context, Max, and other non-effort differences stay separate. Reasoning selection resolves to an exact advertised tuple, `reasoning=none` maps to Atomic `off`, and unsupported levels fail instead of redirecting. Cursor's marked/default tuple remains available as the provider-default route when no reasoning choice is supplied.

Cursor's raw server model ID, exact Max state, and ordered context/thinking/fast/reasoning parameters are retained separately from the selectable ID and sent without recombination. Deterministic suffixes disambiguate rare concise-ID collisions. Unambiguous synthetic Cursor IDs saved by earlier Atomic versions resolve to the new canonical row in `/model`, CLI patterns, defaults, and session restore; ambiguous aliases and unknown Cursor IDs are rejected instead of becoming fabricated remote-catalog models. Exact normal/Max context and output values are used only when supplied together for that corresponding model/mode. Missing fields degrade to clearly provenance-marked operational budgets and text-only input rather than family-name or display-label guesses. Schema-v2 cache files preserve definitions and variants, are scoped by a one-way digest of the stable authenticated account subject (never by storing or hashing an OAuth token), have a 30-minute TTL, and cannot be reused by another account. `--list-models` awaits a stale authenticated refresh; on refresh failure it warns and lists the retained scoped catalog. Cursor requests without exact discovered backend routing metadata fail instead of silently sending the picker/display ID.
