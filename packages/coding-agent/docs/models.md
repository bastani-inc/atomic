...
Built-in subscription providers such as Cursor (experimental) are selected with the same `provider/model` syntax, for example `cursor/composer-2.5`. Cursor model metadata remains text-only; current user-message images, including multiple images in the same current turn, are enabled by default through Atomic's experimental local private protobuf `selectedImages[].data` path rather than a Cursor-specific environment opt-in. Cursor documents image support on other product/API surfaces, which do not validate this local private protobuf transport. Explicit inline `@image.png` prompts, exact whole-message image paths or `file://` image URLs, Atomic clipboard temp image paths such as `/tmp/atomic-clipboard-....png` (including when surrounded by prompt text), and pasted `<file name="/tmp/atomic-clipboard-....png"></file>` markers are attached to the current user message; arbitrary bare absolute paths or `file://` URLs embedded in prose are left as text unless they are Atomic clipboard temp images. Historical user image blocks are tolerated on later turns but serialized as text-only history, while `read` tool image results are text-only for Cursor/text-only models and remain rejected at the Cursor transport boundary. Use the global `images.blockImages` setting if you need to block image attachments for all providers. Live private-API model metadata may fall back to estimated labels. Because Cursor support targets undocumented private endpoints with Cursor CLI-compatible headers, maintainers and users should explicitly accept the risk that it may conflict with Cursor's terms, break without notice, or affect the Cursor account used to authenticate.
