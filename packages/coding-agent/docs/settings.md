...
| `routerModel` | string | `""` | Inference model for workflow-stage and subagent `model: "auto"` selection only. Exact `provider/model`; empty selects Jev when its credentials are configured through `/login` or `TYPESAFE_API_KEY`, otherwise the current chat model. Does not change chat or `structured_output` tool inference. |
...
Use `/settings` → **Router model** to change the effective selection. If the project already defines `routerModel`, the picker edits that project override; otherwise it saves the global default in `~/.atomic/agent/settings.json`. The picker identifies which scope it will save. Choose **Automatic** to save `""` in that scope, **typesafe/jev-latest** for direct Jev, **openrouter/~typesafe/jev-latest** for Jev using your existing OpenRouter login or `OPENROUTER_API_KEY`, a Vercel AI Gateway or OpenCode Zen Jev ID (`vercel-ai-gateway/typesafe-ai/jev`, `opencode/jev-1.13`, `opencode/jev-1.13-free`) using that provider's credentials, or search available chat models by provider/model ID. Jev is a classifier in the unified registry, not a chat model; its decision route does not appear in `/model`. Run `/reload` or restart Atomic to apply the saved choice to an already-running isolated engine.

A nonempty explicit model takes precedence over automatic Jev selection. Use an exact chat catalog ID without a reasoning suffix, or a classifier decision ID returned by `getStructuredOutputProviders()`, such as `typesafe/jev-latest`, `openrouter/~typesafe/jev-latest`, `vercel-ai-gateway/typesafe-ai/jev`, or `opencode/jev-1.13`. Existing `typesafe-ai/jev-latest` values still work; replace the older `typesafe-ai/jev` with `typesafe/jev-latest`. An invalid explicit ID, image-generation model, `auto`, non-string value or surrounding whitespace fails the decision instead of silently changing providers. An explicit empty project value overrides a global selection and restores the configured-direct-Jev/current-chat precedence. OpenRouter, Vercel AI Gateway, and OpenCode Zen credentials alone do not select Jev automatically.

Any Jev failure — HTTP or connection errors, missing credentials, context-budget rejection, or exhausted structured-output repairs — switches routing to your current chat model, whether Jev was selected automatically or pinned explicitly. Transient failures (connection errors, HTTP 408, 429 and 5xx) are first retried up to three times with backoff, and each provider gets an initial attempt plus three corrective retries for invalid output, using the same context. Routing has no built-in wall-clock deadline; cancel the request to stop waiting. Independent provider, credential-preparation and enclosing tool-request limits still apply. Atomic reports the fallback; it can incur chat-provider charges. Cancellation and local input-validation failures do not trigger fallback.
...
If protected content or the remaining request still exceeds Jev's conservative context budget, routing switches to the current chat model, including for a pinned Jev selection. Large candidate sets are split into bounded comparisons without dropping candidates before evaluation.

This setting selects inference for [workflow-stage `model: "auto"`](/workflows/authoring#automatic-stage-model-selection) and [subagent `model: "auto"`](/subagents/reference#automatic-model-selection). It does not directly select the child execution model or change the selected chat model, `structured_output` tool, or general structured-output inference. The decision provider may be a classifier or chat model, never an image-generation model; execution `auto` selects only chat language models, including multimodal-input chat. The selected router provider receives the routing context, so choose a provider permitted to process that data. Authenticate direct Jev with `/login typesafe` (legacy `/login typesafe-ai` remains accepted) or `TYPESAFE_API_KEY`, and gateway Jev (OpenRouter, Vercel AI Gateway, OpenCode Zen) with that gateway's existing credentials. See [Structured decisions](/sdk/structured-decisions) and [TypeSafe Jev](/providers#typesafe-jev).
