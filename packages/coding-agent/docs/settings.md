...
| `showCacheMissNotices` | boolean | `false` | Show transcript notices for significant prompt-cache misses, billed compaction or branch-summary usage, and provider recovery diagnostics such as dropped Anthropic thinking blocks, including when a persisted transcript is resumed. Each miss notice names the first request segment that changed since the previous request: a tool addition or removal, the system prompt, a request parameter, a specific rewritten message by position, or a model switch. A miss after compaction is prefixed with `history compacted`, and a provider request format Atomic cannot segment is reported as `request shape unknown`. When there is no earlier request to compare with, such as a session started on an older version, the notice gives no cause. Atomic records a hash-only `cache_prefix` entry in the session file for every provider request whether or not this setting is on, so turning notices on later or resuming a session still shows the cause; the entry holds hashes, counts, tool names, and the cause label, never prompt text. |
