...
| `showCacheMissNotices` | boolean | `false` | Show transcript notices for significant prompt-cache misses, billed compaction or branch-summary usage, and provider recovery diagnostics such as dropped Anthropic thinking blocks, including when a persisted transcript is resumed. Each miss notice names the first request segment that changed since the previous request: a tool addition or removal, the system prompt, a request parameter, a specific rewritten message by position, or a model switch. A miss after compaction is prefixed with `history compacted`, and a provider request format Atomic cannot segment is reported as `request shape unknown`. |
