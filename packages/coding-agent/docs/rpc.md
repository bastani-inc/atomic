...
The built-in `RpcClient` checks each `message_start` before notifying listeners, and reusable interactive chat hosts apply the same check before creating transcript entries. Starts are ignored when the message is missing, its role is not a string, or fields needed by a built-in renderer are unsafe. Other string roles pass through unchanged because extensions can add `CustomAgentMessages` through TypeScript declaration merging. Unsafe starts can appear anywhere in the received lifecycle stream without stopping later well-formed events. Main and reusable-chat renderers also ignore assistant `message_update` and `message_end` snapshots whose `content` is absent, is not an array, or contains malformed blocks. If an assistant start is absent or rejected, interactive mode creates the streaming view from the next valid update or end, so the response still renders. Aborted end snapshots are normalized before that first render, including retry-count text. When buffering is active, a rejected start flushes pending updates before it is ignored; ordinary buffering may still coalesce rapid update frames. RPC producers must still emit the documented `AgentMessage` shape.

When an unsafe assistant end follows an announced tool call, interactive renderers detach that call from active routing. An immediately following `tool_execution_start` can reclaim the same row, which preserves normal message-end → tool-start event order. If another assistant message arrives first, the old row settles without added error text and later tool events for that retired call are ignored, so follow-on traffic cannot rewrite an earlier turn.
