...
- `workflows/` loads workflow SDK files (`.ts`, `.js`, `.mjs`, `.cjs`); `workflow/` is also accepted as a singular alias. Workflow files should `import { workflow } from "@bastani/atomic/workflows"`, import `Type` from `typebox`, and export the `workflow({ ... })` result. TypeScript resolves the compiled SDK and declarations directly from `@bastani/atomic`, without a hand-authored `.d.ts`, `declare module` shim, `tsconfig` `types` entry, or `paths` alias. Atomic's runtime loader retains the legacy `@bastani/workflows` alias for existing workflow files. See the workflow SDK typing guidance under Programmatic Usage in the workflows guide.
...
Workflow packages should author workflow files with `import { workflow } from "@bastani/atomic/workflows"`, `import { Type } from "typebox"`, and export definitions produced by `workflow({ ... })`. Do not use the removed `runWorkflow` object-form API, and do not hand-roll objects with `__piWorkflow: true`; discovery accepts only definitions minted by `workflow({ ... })`. `@bastani/atomic/workflows` is not a separate npm package: its types resolve through `@bastani/atomic`, so list `@bastani/atomic` and `typebox` in `peerDependencies`. No ambient declaration, `tsconfig` `types` entry, or `paths` alias is required.
