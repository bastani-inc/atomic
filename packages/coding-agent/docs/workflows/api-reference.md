...
Accepted answers are captured independently of the session's compactable context. Compaction cannot remove them from the handoff. If a continuation falls back to another model, earlier successful answers remain; only the failed attempt's provisional answers are discarded before the successful continuation is appended.
