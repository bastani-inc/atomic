...
Workflows that must never run without durable state can declare `durability: "required"` (see [the definition field](/workflows/api-reference#durability)). Atomic refuses to start or resume them on the in-memory fallback. SDK callers can also choose the backend and database for each `run()` call; see [Choosing the durable backend](/workflows/api-reference#choosing-the-durable-backend).
...
Picker rows use the workflow ID as the primary label for search and direct resume. The adjacent details show status, completed stages or checkpoints, then the workflow name; on narrow terminals, the picker may truncate the ID or name visually, but searching the full ID still works.
