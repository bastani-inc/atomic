...
Malformed or schema-invalid routing answers receive up to three corrective retries after the initial attempt, independently for Jev and any current-chat fallback, and transient provider failures (connection errors, HTTP 408, 429 and 5xx) are retried up to three times with backoff. Model selection has no built-in wall-clock deadline; cancellation and independent provider limits still apply. Any Jev failure triggers a reported switch to the current chat model, whether the router was selected automatically or pinned. If routing inference fails completely — Jev and the chat structured-output fallback both — the stage runs on the current chat model instead of failing, with a recorded warning. An empty eligible catalog, stale decisions, credential-bearing routing context and cancellation still prevent stage execution without fallback. Choose router and chat providers permitted to receive the stage task and reference context.
