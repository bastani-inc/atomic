...
## Provider-owned model runtime compatibility

- **Existing Atomic SDK and extensions remain compatible.** Atomic now adapts Pi's provider-owned authentication and asynchronous catalog APIs behind the existing `AuthStorage`, `ModelRegistry`, SDK session options, and extension OAuth contract. Complete credential auth — including headers and enterprise `baseUrl` overrides — reaches provider requests.
- **Catalog refresh is visible and controllable.** The model picker renders its cached snapshot immediately and reports concise success, partial-failure, or timeout status. Run `atomic update --models` to force-refresh authenticated dynamic provider catalogs.
- **Upstream interaction fixes are included.** Adjacent thinking blocks render as one section, Windows startup restores Atomic's terminal title after package checks, standalone Bun binaries include every interactive OAuth adapter, and early clone/fork attempts explain that the first assistant response must save the session.

See [SDK](/sdk) and [Custom providers](/custom-provider). The Reference section includes the complete Pi v0.80.10 port matrix.
