...
Inline image file references such as `@screenshot.png what is this?`, quoted `@"/path/with spaces.png"` references, exact whole-message image paths such as `/tmp/atomic-clipboard-....png`, shell-escaped macOS screenshot paths such as `/Users/me/Desktop/Screenshot\ 2026-06-22\ at\ 12.06.12 AM.png`, exact whole-message `file://` URLs from terminal drag/drop, pasted Atomic clipboard temp paths such as `/tmp/atomic-clipboard-....png` or `C:\Users\me\AppData\Local\Temp\atomic-clipboard-....png` even when surrounded by prompt text, and pasted `<file name="/tmp/atomic-clipboard-....png"></file>` markers are resolved before the turn starts only when the active model path can consume current-turn image attachments (including Cursor's default-enabled experimental path). Atomic replaces the reference with a file marker and attaches the image to the current user message. Atomic no longer scans arbitrary prose for bare absolute paths, so text like `please explain /tmp/a.png` stays textual unless the image is referenced explicitly with `@`, the submitted message is exactly the image path, or the path is an Atomic clipboard temp image. Existing `<file>...</file>` bodies are not recursively scanned for nested `@image` references, including markers formatted as `name = "..."`. Multiple images can be attached to the same current user message; Cursor's experimental path also tolerates historical user image blocks on later turns while serializing all images from the trailing current run of consecutive user messages. Missing paths and non-image mentions are left as text. This keeps image prompts on providers that support current-turn image attachments from falling back to a `read` tool-result image while avoiding broad filesystem probing. Cursor's image path uses an undocumented private protobuf transport that may fail or ignore images; use the global `images.blockImages` setting to skip inline image-reference resolution and block image attachments for all providers.
