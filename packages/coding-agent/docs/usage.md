...
Inline image file references such as `@screenshot.png what is this?`, pasted absolute image paths such as `/tmp/atomic-clipboard-....png`, unescaped or shell-escaped macOS screenshot paths such as `/Users/me/Desktop/Screenshot\ 2026-06-22\ at\ 12.06.12 AM.png`, `file://` URLs from terminal drag/drop, and pasted `<file name="/tmp/atomic-clipboard-....png"></file>` markers are resolved before the turn starts only when the active model path can consume current-turn image attachments (including Cursor's default-enabled experimental path). Atomic replaces the reference with a file marker and attaches the image to the current user message. Multiple images can be attached to the same current user message; Cursor's experimental path also tolerates historical user image blocks on later turns while serializing only current-turn images. Missing paths and non-image mentions are left as text. This keeps image prompts on providers that support current-turn image attachments from falling back to a `read` tool-result image while avoiding filesystem probing for text-only providers. Cursor's image path uses an undocumented private protobuf transport that may fail or ignore images; use the global `images.blockImages` setting to skip inline image-reference resolution and block image attachments for all providers.
