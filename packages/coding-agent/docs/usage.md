...
While ordinary agent work is active, a one-cell Braille A builds cumulatively dot by dot from top to bottom at pi-tui's canonical 80ms cadence, then reverses one dot at a time for a clean cycle. It occupies the same inline, one-row footprint as the standard spinner: one glyph immediately before the existing text. Main and workflow-stage chat preserve all 453 of Atomic's original randomized whimsical working verbs, selecting one message per turn; even the longest fits the tested 64-column surface. With `ATOMIC_REDUCED_MOTION=1`, the final A remains settled without animation. Factual retry, fallback, error, cancellation, and compaction status suppresses the thematic indicator, while blocker and human-approval/prompt surfaces hide ordinary work chrome and factual receipts remain verbatim. Extension-provided indicator frames and cadence remain unchanged.
