...
Both abort routes are cooperative: they ask the agent to stop and wait for it, and never terminate the engine that runs your tools. Ctrl+C additionally acts as an escape hatch when the interactive engine stops answering entirely — including a replacement engine that hangs before it finishes starting — and then stops that engine and starts one more. A message that could not be sent comes back to the editor rather than being lost: it is placed above anything you typed while the send was pending, separated by a blank line. See [Keybindings](/keybindings#application).
