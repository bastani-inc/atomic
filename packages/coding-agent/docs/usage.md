...
While ordinary agent work is active, the exact one-cell `∀` remains visible and pulses regular → bold → regular at an 80ms cadence. It occupies the same inline, one-row footprint as the standard spinner: one glyph immediately before the existing text. Main and workflow-stage chat preserve all 453 of Atomic's original randomized whimsical working verbs, selecting one message per turn; even the longest fits the tested 64-column surface. Every agent and SDK turn resets to regular weight with a fresh lifecycle-relative cadence, while turn, terminal, error, replacement, and disposal paths stop the active timer cleanly. Restoring the ordinary indicator after an extension override also begins at regular weight; extension-provided frames and cadence remain unchanged. With `ATOMIC_REDUCED_MOTION=1`, `∀` remains static and un-emphasized without an animation timer. Factual retry, fallback, error, cancellation, and compaction status suppresses the thematic indicator, while blocker and human-approval/prompt surfaces hide ordinary work chrome and factual receipts remain verbatim.
...
| `ATOMIC_REDUCED_MOTION` | Set to `1` to skip startup choreography and render the ordinary working identity as a static un-emphasized `∀` without a timer |
