...
Replace `typesafe-ai/jev-latest` or `typesafe-ai/jev` in router settings and SDK calls with `typesafe/jev-latest`. The old direct IDs and `/login typesafe-ai` alias are no longer supported. Keys saved under `typesafe-ai` are not migrated or used for `typesafe`; re-save your key with `/login typesafe`. `TYPESAFE_API_KEY` and the wire model `jev-latest` are unchanged. Gateway IDs such as `vercel-ai-gateway/typesafe-ai/jev` are provider-owned IDs, not legacy aliases, and remain unchanged.
