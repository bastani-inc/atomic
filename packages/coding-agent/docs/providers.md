...
- Cursor is experimental, and this catalog rewrite intentionally breaks older experimental IDs. Saved settings, CLI references, workflows, and restored sessions must use an exact current GetUsable ID. Old or unavailable IDs fail with reselection guidance; there is no alias, nearest-effort, static, AvailableModels, provider, or other-model fallback.
...
- Credential changes refresh immediately, superseded/out-of-order refreshes cannot overwrite newer state, and future timestamps are stale. A first-time `/login` completes only after the credential scope has a usable authenticated catalog; shutdown cancels pending discovery and errors are redacted. `atomic --list-models` awaits required discovery, while live registration remains usable if best-effort persistence fails.
- Current-turn user images and live mixed text/image MCP tool results retain strict serialization for image-capable routes. Payloads must be non-empty standard base64; MIME-style whitespace is accepted and stripped, and unsupported/malformed image input is rejected locally. Reconstructing images from earlier turns and structured clipboard attachments are deferred to [#1807](https://github.com/bastani-inc/atomic/issues/1807); assistant-generated images are out of scope.
