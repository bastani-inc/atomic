...
- Cursor is experimental, and this catalog rewrite intentionally breaks older experimental IDs. Saved settings, CLI references, `--models`/enabled-model scopes, workflows, and restored sessions must use an exact current GetUsable ID. Cursor-specific behavior also requires the raw provider identity to be exactly lowercase `cursor`: workflow strings use the literal prefix `cursor/<raw-id>`, and saved/session/default/object identities use provider `cursor`. Provider case or whitespace variants are ordinary non-Cursor identities and are never normalized into Cursor discovery, reselection errors, or execution authority. Every route byte—including blank IDs and suffix/context-looking text—is preserved without generic trimming or rewriting. Explicit lowercase `cursor/<id>` reserves authenticated discovery and is terminal when unavailable. Bare references reserve no Cursor discovery, but an exact already-published live lowercase Cursor row wins before generic matching; every other bare value follows ordinary non-Cursor resolution. Atomic retains no static list of historical Cursor IDs. Exact-lowercase Cursor definitions from `models.json` are ignored even if they copy the private routing shape; case/whitespace provider variants remain ordinary custom providers. A selectable lowercase Cursor row must come from authenticated GetUsable publication, use the Cursor API, and carry structurally valid exact per-ID live routing metadata. Static/generic or malformed rows are excluded from CLI, defaults, restored sessions, scopes, SDK sessions, and workflows. A selected duplicate occurrence must still exist in the current catalog at workflow validation and execution authorization; a stale ordinal is rejected instead of migrating to occurrence zero. Strict workflow preflight and stage resolution require a real participating `model_catalog_discover` handler before reading listed rows; a runner method that would emit to zero handlers and a list-only stale row are never sufficient. Strict Cursor scope failures are fatal during synchronous and deferred interactive startup, so Atomic never continues prompting on a default or current model outside the requested scope; enabled-model scope recovery keeps a current in-scope selection or the saved default (only an omitted default is absent) before falling back to the first scoped entry. A settings-only exact Cursor default is retried after deferred extensions load—the deferred interactive path is non-fatal until that retry—while an explicit selection or restored-session identity keeps precedence. Explicitly qualified non-Cursor IDs retain their normal meaning. There is no alias, nearest-effort, static, AvailableModels, provider, or other-model fallback.
- The exact lowercase `cursor` provider and `cursor-agent` stream handler are reserved for the loader-attested bundled extension. Canonical Cursor models retain their required top-level identity but receive isolated, recursively frozen nested metadata before publication, so captured aliases cannot mutate execution routing while shared ordinary-provider objects remain unfrozen. Untrusted OAuth model modifiers cannot inject, remove, reorder, clone, or alter canonical Cursor rows, and arbitrary extensions cannot replace the Cursor stream boundary to intercept account credentials. Cursor-like provider variants remain ordinary providers when they use ordinary APIs.
...
- Model metadata uses schema-v3 cache files named `~/.atomic/agent/cursor-model-catalog.json.account-<digest>` for 30 minutes. They preserve the ordered GetUsable row sequence—including blank, whitespace-only, and duplicate IDs—plus optional same-account image flags. Schema-v1, schema-v2, unscoped, and parameterized caches are ignored rather than migrated. The scope is a one-way digest of the stable JWT subject—not an OAuth-token hash—and no token or account claim is persisted. Rotated credentials may reuse only a fresh snapshot for the same account; only that fresh v3 snapshot may bridge a temporary GetUsable failure. A successful empty GetUsable response is authoritative: it revokes rows and execution leases, durably invalidates the scoped cache, leaves non-runnable `empty` status, and forces rediscovery after restart instead of reusing stale rows. A clear that throws or asynchronously rejects is awaited and falls back to a durable empty marker that replaces even a newer- or future-dated positive record and remains an ordering barrier against delayed older positive writes; if clear and marker persistence both fail, login fails closed and the cache instance remains tombstoned against stale loads until a positive replacement is confirmed to have landed. File-cache saves report applied versus timestamp-skipped writes; legacy custom caches with no outcome are verified through `load`, so skipped, rejected, or unverifiable positive saves retain the tombstone. Cache saves and invalidations are ordered independently per account: a pending invalidation blocks same-account loads, newer saves wait for earlier clears, and a later clear remains final without blocking another account. A later same-account positive save with an equal `fetchedAt` timestamp follows that invocation order and replaces the earlier positive record; only a strictly older positive save is rejected. Stale snapshots, AvailableModels data, and static rows are never executable fallback.
- Credential changes refresh immediately, superseded/out-of-order refreshes cannot overwrite newer state, and future timestamps are stale. Stored and execution credential lookups are epoch-fenced before discovery, cache, catalog, authority, status, or warning effects. OAuth login and refresh invocations share one monotonic owner fence from invocation through authentication, cancellation, discovery publication, authenticated-scope mutation, and return, so an older refresh cannot roll a newer login or refresh back. A stale login cannot revoke or replace a newer account, abort its active streams, or cancel a discovery producer shared by the current owner. A cancelled current login may cancel a login-owned same-token producer it adopted rather than created when no valid owner remains, while independently started stored/execution discovery survives. A superseded or post-disposal result is inert; a current missing or failed execution lookup revokes rows, the scoped cache, authority, and active leases with a fixed sanitized diagnostic. Caller cancellation and disposal detach pending first or second resolver waits; late outcomes remain observed and inert, and transport is never invoked once an abort is visible before Run. An authorization failure or caller abort that occurs before a paused tool turn is resumed surfaces an error but preserves that paused turn for a later authorized retry; only a failure once the resume has begun cleans up the resumed attempt. Execution is authorized against the host-selected account's current TTL-valid exact route and selected catalog occurrence: when the host credential scope changes, execution removes the previous rows and joins the selected account's cache/GetUsable refresh, while stale credentials cannot establish or roll back authority. A bounded lease-expiry timer actively aborts silent active and paused streams at TTL, unregisters expired rows, clears that scoped cache/freshness state, and requires rediscovery; inbound messages are rechecked before output, paused tool results recheck before every write, and aborted turns are removed. Account, published generation, route occurrence, image metadata, or Max changes revoke the same opaque lease. HTTP/2 Run streams release local codec/open-stream bookkeeping before awaiting remote close or cancellation; provider/session disposal detaches turn and cleanup bookkeeping immediately and waits only for a bounded grace, so never-settling remote cleanup cannot hang shutdown. A first-time `/login` completes after authenticated discovery establishes either a runnable catalog or authoritative empty account state; shutdown cancels pending discovery and errors are redacted. `atomic --list-models` awaits required discovery, while nonempty live registration remains usable if best-effort persistence fails.
- Only current-turn user images and live mixed text/image MCP tool results retain strict serialization for image-capable routes. Ordinary prior user/assistant text is reconstructed as ordered root-prompt blobs when a persisted source-CLI session resumes in a new process, while the structured turn tree remains available for Cursor state handling. Earlier-turn image blocks are omitted while their surrounding text/tool continuity remains. Current payloads must be non-empty canonical standard base64 after ASCII whitespace is stripped. The MIME string is forwarded without allowlist validation; known image MIME types receive conventional filename extensions and other MIME values use `.img`. Reconstructing earlier images and structured clipboard attachments are deferred to [#1807](https://github.com/bastani-inc/atomic/issues/1807); assistant-generated images are out of scope.
...
Prefer `cursor/<exact-getusable-model-id>` after login to request strict discovery and terminal validation; `/model` shows the current account's routes. An exact bare ID may also select an already-published live lowercase Cursor row without requesting discovery.
