...
For the shared ChatGPT Codex transport, an agent-session turn sends the fast routing contract only after the final request payload contains `service_tier: priority`: `originator: codex_cli_rs` plus `x-codex-routing-hint: model=<base-upstream-model>;tier=priority` on both HTTP/SSE and WebSocket transports. The same rule covers the stock provider, renamed providers, credential-resolved endpoints, and monitoring proxies that retain `api: "openai-codex-responses"`. WebSocket fallback, reconnect, and HTTP retry attempts reuse that identity; a final model or tier change drops a cached socket before reuse. Requests to the standard OpenAI API send only the tier. On a normal model, or when a request hook changes the final tier away from `priority`, Atomic keeps the normal `originator: pi` identity and sends no routing hint. A standalone `modelRuntime.complete()`/`completeSimple()` request still selects the fast model correctly — base upstream model ID plus `service_tier: priority` — but keeps `originator: pi` and sends no routing hint, because that identity is applied by the agent-session transport wrapper.
