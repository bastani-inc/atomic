...
- Text input and current user-message image input are enabled by default, including multiple images attached to the same current user message. This is intentionally default-on, not hidden behind a Cursor-specific environment opt-in. Cursor image input uses Atomic's experimental `selectedImages[].data` protobuf path while keeping model metadata text-only; later turns tolerate historical user image blocks in chat history but serialize history as text-only while sending only the latest user-message images. Explicit inline `@image.png` prompt references, exact whole-message image paths or `file://` image URLs, Atomic clipboard temp image paths such as `/tmp/atomic-clipboard-....png` even when surrounded by prompt text, and pasted `<file name="/tmp/atomic-clipboard-....png"></file>` markers are resolved into current user-message attachments before Cursor can request the `read` tool. Arbitrary bare absolute paths or `file://` URLs embedded in prose are left as text unless they are Atomic clipboard temp images. The `read` tool returns text-only placeholders for Cursor/text-only models so unsupported tool-result images are not sent. That protobuf path is undocumented and unvalidated against live Cursor remote behavior; Cursor documents image support on other product/API surfaces, which do not validate this local private protobuf transport. Remote behavior may fail or ignore images, and tool-result images remain rejected at the transport boundary. If you need to prevent image attachments entirely, use the global `images.blockImages` setting; it applies to all providers, not only Cursor, and skips inline image-reference resolution before filesystem probing.
...
Select models as `cursor/<model-id>` (default: `cursor/composer-2.5`).
