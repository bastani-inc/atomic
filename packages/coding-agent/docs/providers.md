...
- Model metadata uses schema-v2 cache files named `~/.atomic/agent/cursor-model-catalog.json.account-<digest>` for 30 minutes. The scope is a one-way digest of the stable JWT subject—not an OAuth-token hash—and no token or account claim is persisted. Rotated credentials can reuse only the same account's fresh cache; another account cannot load or overwrite it, and legacy unscoped caches are not trusted for authenticated startup. Credential changes refresh immediately, superseded in-process requests cannot overwrite the latest account snapshot, older same-account writes are ignored, and future-dated timestamps are stale. `atomic --list-models` awaits stale discovery; a failure prints a warning and lists the retained scoped catalog. Successful live registration is independent of best-effort persistence, whose failures remain diagnostic.
