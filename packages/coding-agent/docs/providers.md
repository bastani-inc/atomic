...
Atomic supports subscription-based providers via OAuth and API-key providers via environment variables or the auth file. Static providers ship model lists with Atomic releases. Authenticated dynamic providers such as Cursor and GitHub Copilot prepare their catalogs from the provider's current authority instead.
...
- Atomic rebuilds canonical user text, assistant text/thinking, tool calls, and text tool results from the current message context for continuation, persisted restart, and workflow stages. Historical and live-result text blocks remain verbatim and ordered without inserted separators when images are omitted; duplicate live tool-call IDs retain FIFO occurrence correlation. Orphan text tool results are preserved without synthetic tool calls. Process-local checkpoint/blob state is only an optimization and cannot replace, truncate, or reorder that history.
