...
Enabled fast mode sends the real first-party Codex routing contract for ChatGPT-backed `openai-codex/*` requests: `service_tier: priority` in the request body, plus `originator: codex_cli_rs` and `x-codex-routing-hint: model=<model>;tier=priority` on both HTTP/SSE and WebSocket transports. The backend gates priority routing on that harness identity, so the tier alone can be ignored. Requests to the standard OpenAI API send only the tier, and custom `openai-codex` base URLs, proxies, and every other provider keep their normal identity. Disabled fast mode changes nothing: Atomic keeps sending `originator: pi` with no routing hint.
