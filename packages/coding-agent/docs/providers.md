...
- Cursor uses private, undocumented APIs and CLI-compatible headers. `AgentService/GetUsableModels` is the sole authority for the authenticated account's runnable existence, exact route IDs, display data, and Max state. Atomic preserves one row per returned GetUsable row in source order, including blank, whitespace-only, and duplicate IDs; the result is an account snapshot, not a universal Cursor catalog. Textual exact lookup selects the first duplicate occurrence, while a selected later model object retains that current occurrence's routing metadata.
