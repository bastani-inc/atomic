...
- AvailableModels field 29 supplies parameter names and option labels, while field 30 supplies complete allowed tuples. Atomic caches both, exposes only field-30 tuples, and groups variants only when reasoning effort is their sole difference. Selectable rows use concise effort-neutral IDs and Cursor's actual display name plus necessary context, Max, thinking, fast, or other mode qualifiers; an arbitrary/default low tuple no longer labels a multi-effort row as `...max-mode-low`. Advertised `reasoning=none` maps to Atomic `off`, while Cursor's marked/default tuple remains the bare provider-default route.
- Selectable Cursor IDs are distinct from backend routing. Requests preserve the raw server model ID, exact Max flag, and ordered context/thinking/fast/reasoning tuple. Deterministic suffixes resolve concise-ID collisions, while unambiguous pre-correction synthetic IDs migrate to canonical rows during CLI/default/session resolution. Ambiguous aliases and unknown Cursor CLI/session IDs are rejected instead of becoming route-less synthetic models.
