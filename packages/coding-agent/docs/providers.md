...
- The implementation avoids a localhost proxy and keeps credentials OAuth-only. Cursor's HTTP/2 transport uses bundled `@bastani/atomic-natives`; request encoding can send generated `RequestedModel` parameters and Max-mode state from one discovered preset without recombining presets.
