...
- AvailableModels metadata keeps normal and Max context, Max-only, fast, thinking, reasoning-effort, image capability, and complete parameter combinations distinct. Atomic exposes only exact reasoning values carried by discovered parameter variants (including `extra-high` → Atomic `xhigh`); it does not infer levels from family names or fallback-ID suffixes, treat a standalone `-max` name as reasoning effort, or route an explicitly unsupported level to a nearby effort.
...
- Model metadata is cached without credentials in `~/.atomic/agent/cursor-model-catalog.json` for 30 minutes. Since the cache has no account identity, first authenticated use refreshes it; the TTL applies only to repeat discovery for the same in-memory credential identity. Credential changes refresh immediately, superseded requests cannot overwrite the latest account snapshot, and future-dated timestamps are stale. An authenticated stale refresh is awaited by `atomic --list-models`; discovery failures are surfaced while the previous cache/fallback remains usable. A successful live snapshot is registered immediately even if its best-effort cache write fails; persistence failures remain in refresh diagnostics.
...
- The implementation avoids a localhost proxy and keeps credentials OAuth-only. Cursor's HTTP/2 transport uses bundled `@bastani/atomic-natives`; request encoding preserves exact parameter presets with collision-free generated IDs and sends the discovered backend model ID through both legacy `ModelDetails` and generated `RequestedModel` fields, including parameters and Max-mode state without recombining presets.
