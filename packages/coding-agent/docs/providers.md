...
- Text input is supported by default; vision/image input is rejected with a clear error. A conservative experimental path can be enabled with `ATOMIC_CURSOR_EXPERIMENTAL_IMAGE_INPUT=1` to send final user-message images through Cursor's local private protobuf `selectedImages[].data` path while keeping model metadata text-only. That protobuf path is undocumented and unvalidated against live Cursor remote behavior; Cursor documents image support on other product/API surfaces, which do not validate this local private protobuf transport. Remote behavior may fail or ignore images, and tool-result images remain rejected.
