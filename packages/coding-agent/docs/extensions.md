...
"Never accepted" is decided at the transport boundary rather than by matching error text, so it covers the raw stream failures a dying engine actually produces (`EPIPE`, `ERR_STREAM_DESTROYED`, `ERR_STREAM_WRITE_AFTER_END`) as well as an explicit stop or a missing child. The rejected error keeps its own identity and its `code`, `errno`, and `syscall` fields; the classification is an additional non-enumerable marker, so extension and SDK callers that already branch on those fields are unaffected. Failures that are not transport failures — request timeouts after a successful write, RPC error responses, provider and model errors, anything after `agent_start` — are reported as before and never restore a draft.
