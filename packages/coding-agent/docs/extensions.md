...
`ReadOperations` must provide `resolvePath` to apply the target operating system's path syntax without probing the control filesystem. Select `pathStyle` from the target's reported operating system before calling `resolvePath`; do not infer it from the control machine. Explicit path syntax also controls `file://` URL decoding, including Windows drive and UNC paths. `ReadOperations` may provide `stat` and `listDir` to keep directory-tree reads on the injected filesystem. Document conversion uses bytes returned by the custom backend and never substitutes a same-named control-machine file. Archive, SQLite, notebook, internal-resource, and HTTP(S) URL inputs fail with `UnsupportedReadSelectorError` because those routes use local-only helpers; custom backends must not silently read or persist them on the control machine.
