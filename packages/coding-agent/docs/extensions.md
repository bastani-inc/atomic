...
The first is a remote custom UI. While an engine-owned `ctx.ui.custom()` component or overlay holds input, every key is forwarded to the engine child, so a component that never resolves would trap Ctrl+C too. The first press still reaches the component, so an extension that binds Ctrl+C itself keeps working; if the same component is still holding input on the next press, that press is handled by the host and replaces the engine with the calm status `Restarting interactive engine.`, even when the engine is perfectly healthy. Native host selectors, dialogs, input forms, session pickers, and unrelated native overlays are unaffected: they keep Ctrl+C as their own cancel.
