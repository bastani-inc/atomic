...
When an explicit `send`, `ask`, or `reply` returns `Client disconnected`, retry the identical call up to three times. Intercom keys the retry identity by the caller-issued target verbatim, independently from the freshly resolved transport session ID, so a name-addressed retry remains the same operation when its recipient reconnects while a name and an exact ID remain distinct calls. The client retains the original message ID for at most 11 minutes from the first attempt: the full 10-minute `ask` wait plus one explicit 1-minute retry opportunity, with no deadline extension and at most three reuses. Any settled result or non-recoverable failure releases it immediately, so a later intentional repeat is a new operation.

Accepted-operation authority is stored in `delivered-messages.sqlite` for 12 minutes, including the stable sender, logical target, payload, stable recipient, and question-route metadata needed to rebind replies. The broker durably reserves the identity before forwarding, then marks it accepted after the confirmed write and before acknowledging the sender. A crash after forwarding and acceptance can therefore return the retained success without a second delivery; if a crash leaves only the pre-forward reservation, the retry is refused as uncertain rather than redelivered. After a deduplicated ask retry, the recipient's public `intercom({ action: "reply", ... })` resolves the recorded sender name again and the broker authorizes only the exact pending question route; an ambiguous name, changed stable endpoint, changed groups, payload, or message ID is refused rather than claiming the route. Legacy frames without logical-target metadata keep using their transport target.

Both sides fail closed at memory or storage pressure instead of evicting authority that can still suppress a duplicate. A client holds at most 1,000 retained identities; overflow refuses all operation starts until the original possible-acceptance deadlines pass. The broker holds at most 10,000 live records and 64 MiB of retained UTF-8 record content; it refuses new delivery until TTL cleanup makes room. SQLite transactions serialize concurrent broker access, stale rows are removed by TTL, and malformed or truncated authority makes delivery refuse safely rather than starting from an empty cache.
...
- `delivered-messages.sqlite` — bounded 12-minute accepted-operation authority that survives broker replacement
