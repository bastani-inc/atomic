...
When an explicit `send`, `ask`, or `reply` returns `Client disconnected`, retry the identical call up to three times. Intercom retains that failed operation's message identity for up to nine minutes from the original attempt—shorter than the broker's ten-minute delivered-message record—and reuses it only for those three recoverable retries. If the broker accepted the first operation but its acknowledgement was lost, the retry receives success without delivering the operation twice; `ask` also keeps the same question correlation. Any settled result or non-recoverable failure releases the identity immediately, so a later intentional repeat of identical content is a distinct operation and is delivered normally. Across reconnects, the broker signs the delivery record with the client's stable registration return address rather than its newly generated broker session ID; a legacy registration without a return address falls back to that session ID, while a genuinely different sender or changed target or payload still fails with `message_id_conflict`.
