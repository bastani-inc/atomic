...
- [`packages/coding-agent/src/core/session-manager.ts`](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/src/core/session-manager.ts) - Public session-manager facade and entry persistence APIs
- [`packages/coding-agent/src/core/session-manager-history.ts`](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/src/core/session-manager-history.ts) - Active-branch reconstruction and monotonic persisted-deletion closure
- [`packages/coding-agent/src/core/session-manager-tool-dependencies.ts`](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/src/core/session-manager-tool-dependencies.ts) - Read-time tool-call/tool-result dependency closure
...
Tool-call/tool-result pairs are also treated as replay dependencies. Calls and results are paired as concrete chronological occurrences inside provider-visible structural turns rather than grouped globally by opaque ID, so separate exchanges may safely reuse an ID. Fresh plans are reconciled before the turn invariant is checked, so paired deletions cannot indirectly create a partial signed sequence. During active-context reconstruction, persisted deletion targets are authoritative: Atomic may add in-memory omissions to close an unsafe signed turn or tool dependency, but it never restores an entry or content block named by a `context_compaction` record. A content-block deletion inside a replay-sensitive signed assistant therefore promotes that assistant to a whole-entry omission, partial signed-turn omissions expand to the remaining signed assistants in that turn, and tool calls/results are omitted together. This closure is transient and deterministic; the append-only JSONL is not rewritten, and unrelated retained history remains unchanged. As a final provider-safety guard, orphaned `toolResult` messages are dropped before LLM serialization if their matching assistant `toolCall` is no longer the immediately preceding tool-use group. Raw `redacted_thinking` blocks are normalized in the transient, non-mutating LLM-compatible messages returned by `convertToLlm`; durable session messages remain byte-exact.
...
│   6. expand persisted filters to a monotonic signed/tool omission closure    │
│   7. never restore a durable deletion target during rebuild or resume       │
└─────────────────────────────────────────────────────────────────────┘
```
