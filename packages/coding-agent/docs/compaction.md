...
Atomic asks the active session model, through the normal session stream/provider wrapper, to rank every eligible line in one global pass and apply one threshold. Each planner request contains the entire compactable region; planning is never split into chunks. The first request omits reasoning for non-reasoning models and an unset or `off` level, preserves `minimal` and `low`, and caps `medium` or higher levels at `low`. Manual, threshold, and overflow compaction all calculate the line target directly from the prepared `compression_ratio`. Explicit protected lines form a hard keep floor.

The planner makes one targeted semantic retry only when its first response stops for length and contains no recoverable usable range. That second whole-region request uses `minimal` reasoning and the same prompt, context, authentication, token limit, and cancellation signal. A recoverable partial response succeeds immediately. Normal-stop malformed or unusable output, provider/API errors, overflow, stream errors, and cancellation do not enter this retry. If the second length-stopped response is still malformed or unusable, only that final response is diagnosed and the planner fails after two attempts.

This bounded internal planner retry is not a second compaction strategy, provider transport retry, model fallback, or call to `agent.continue()`. It does not control `compaction_end.willRetry`, which describes the surrounding compaction lifecycle. Planner failures write no compaction entry and schedule no continuation. During the post-tool preflight, final failure or cancellation stops the active loop before its follow-up provider request and is not admitted to ordinary provider retry or model fallback. There is no critical rung, deterministic fallback, or deterministic target correction.
