...
Prompt caching reduces repeated prefill work; it does **not** remove cached tokens from the context window. Atomic uses a bounded request-local projection sequence: `warm → isolated-full → isolated-elided`, entering each projection at most once. Local input fit failure advances only to a provider-visible input projection below the smallest prior rejected or blocked input size; a provider context rejection may do the same. Separate input and final-transport fingerprints prevent resending identical input even when only the output cap changes, and the transport fingerprint is computed after cache shaping and output-limit mutation over the exact returned object. There is no fourth attempt. The elided projection removes only unprotected temporary transcript lines, retains original line numbers and every protected source line byte-for-byte, includes deterministic edge candidates plus an omitted-range instruction the model must not echo, and still validates/reconstructs against the unchanged original canonical region. If protected-only content cannot fit or no useful content can be removed, compaction fails boundedly. Session entries, the original numbered region, and durable raw data are never modified.
...
Context thresholds and persisted token-reduction statistics use API-aware normalized usage. OpenAI Responses, Codex Responses, and OpenAI Completions sum uncached input plus cache-read/cache-write partitions. Anthropic Messages alone applies the mirrored-cache guard needed by compatible endpoints that duplicate the same prompt tokens across `input` and cache fields. That threshold/display compatibility rule is deliberately separate from request-bound warm occupancy: captured normal-request occupancy always sums all three prompt partitions exactly once and excludes output.
