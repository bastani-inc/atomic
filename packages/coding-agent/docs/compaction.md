...
Legacy (pre-full-collapse) boundaries kept a recent logical-turn tail outside the classifier request, preserving its structured images and tool messages. New full-collapse boundaries do not use that legacy tail: `preserve_recent` protects the final N context-visible messages as their canonical serialized physical lines inside the single region. Images therefore become `[image]`. When the canonical projection is created, oversized tool-result text is capped to its first 16,000 characters plus a deterministic remainder marker. If protected, that bounded representation becomes protected canonical content and persists in the full-collapse boundary and reopened active context. The complete raw result remains unchanged in the append-only JSONL, pre-boundary backup, and branches rooted before the boundary, but it is not replayed after the boundary.
...
Prompt caching reduces repeated prefill work; it does **not** remove cached tokens from the context window. Atomic uses a bounded request-local projection sequence: `warm → isolated-full → isolated-elided`, entering each projection at most once. Typed request-local fit/restoration failures survive adapters that convert an asynchronous payload-hook throw into a resolved assistant error: input/headroom or warm-restoration decline advances to the next bounded projection, output-budget failure remains terminal, and retry-guard failure is local rather than provider overflow. After an actual provider context rejection—or a recoverable headroom-limited length stop—the next projection must be changed and strictly smaller. Separate input and final-transport fingerprints prevent resending identical rejected input even when only the output cap changes, and the transport fingerprint is computed after suffix shaping and output-limit mutation over the exact returned object. There is no fourth attempt. The elided projection removes only unprotected temporary transcript lines, retains original line numbers and every protected source line byte-for-byte, includes deterministic edge candidates plus an omitted-range instruction the model must not echo, and still validates/reconstructs against the unchanged original canonical region. If protected-only content cannot fit or no useful content can be removed, compaction fails boundedly. Session entries, the original numbered region, and durable raw data are never modified.
