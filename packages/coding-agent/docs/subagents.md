...
Child output artifacts are fully opened, flushed, and closed before a foreground attempt returns or a background attempt publishes its terminal result. Normal completion and runtime-spawn failure share the same idempotent finalization, so immediate artifact cleanup cannot trigger a late stream error and successful output is not truncated at the lifecycle boundary.
