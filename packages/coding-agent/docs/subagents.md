...
Subagent runs attach structured acceptance evidence to the child result. Read-only investigation, research, analysis, and inspect-only prompts infer lightweight attestation; implementation prompts such as "fix", "edit", "update", or "implement" infer write gates that require concrete evidence. Prose-only report and summary requests (for example, "summarize the fix", "write a summary of README", or "write a report with proposed fixes") also stay attested unless they explicitly ask for file or code edits. Objectful file edit requests such as "edit README and write a summary" or "modify README and report results" keep checked/reviewed write gates. Bare diagnostic prompts such as "Investigate the likely fix for the cache race" and read-only-agent tasks such as `codebase-analyzer` handling "Analyze the fix regression" or dynamic "Analyze each fix regression" stay attested even when issue-title words include "fix" or "patch". Side-effectful operations such as "release the package and write a summary", objectless "release and write a summary", "publish the package and write a report", "deploy the app and write a summary", "commit the changes and write a report", or objectless "commit and write a report" keep checked/reviewed write gates; read-only phrases such as "summarize release notes" or "analyze the previous commit" stay attested. Read-only prior-work summaries such as "report what changed in the patch", "summarize what changed in the diff", or "review tests added by the previous change" stay attested; actor-owned implementation summaries such as "report what you changed and tests added" require write evidence. If a debugging or investigation prompt is explicitly no-write/no-edit/no-file-write/no-source-change, including forms followed by commas, conjunctions, or constrained read-only `but` continuations such as "do not edit files, just identify the root cause", "do not edit files, but only report the root cause", "do not edit files, just list findings", "do not edit files and only report root cause", or "do not modify code, then summarize findings", Atomic keeps it attested even when the issue title contains words such as "fix". Strong implementation wording still wins over report/summary prose, so prompts such as "please quickly fix the bug and report results", "after patching the bug, report results", "report what you changed and tests added", or "do not edit files, but implement the migration" require write evidence. Scoped constraints such as "do not change anything else", "do not change tests", "do not change snapshots", "do not change existing behavior", "do not modify CLI behavior", or "do not change database schema" do not suppress write gates when paired with implementation intent. Dynamic fanout and dynamic-group runs default to reviewed unless the selected agent or task wording is read-only/no-write.

For write gates, `changedFiles` must be present and non-empty. A status line that says the acceptance gate was rejected means the child execution completed, but the evidence gate did not accept the result; status output includes the first available rejection reason, such as a failed runtime check, verification command, review blocker, or parent-decision finding, instead of raw `acceptance: rejected` text.
