...
When a task has settled, the launch result, `action: "wait"`, and `action: "status"` show its output after the task record. Up to 16 KB is shown, led by the absolute path of a file that holds the full result. That file is the `output` path you requested (background runs included), otherwise the run's output artifact, otherwise a temporary file. With `outputMode: "file-only"` only the path is shown. A child that stops to ask a question through `contact_supervisor` settles with the question and a ready-to-run follow-up call in this output.
