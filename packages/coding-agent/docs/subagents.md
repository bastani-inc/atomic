...
Model fallback decisions use structured provider and attempt causes. There is no child wall-clock kill cap and no timeout-regex classification, and numeric process exit codes are not used as an outcome discriminator: a legitimately long turn — quiet on the socket while the provider composes a large response, or busy with in-flight tools — is allowed to finish, and only an explicit termination or provider failure supplies a retryable cause.

Unattended in-process children additionally apply a **content-progress liveness bound** (`control.streamStallMs`, default `300000` ms; `0` disables) to each assistant-response stream. It measures time since the last *content-bearing* event — any non-empty text, thinking, or tool-call delta, plus every structural and terminal event — so it bounds forward progress *within a single stream*, not socket idleness and not total turn or tool duration. A long reasoning pass or a slow in-flight tool keeps resetting the window and is never affected. If a stream stops producing content-bearing events for the whole window (the failure mode behind [#2446](https://github.com/bastani-inc/atomic/issues/2446): a flood of whitespace-only tool-call deltas that never reached a terminal event and blocked the parent for hours), it is ended with a *retryable* provider failure, so same-model retry and then configured `fallbackModels` advance within the same turn — or, with no fallback, `prompt()` resolves with an error and control returns instead of blocking. Interrupting the child ends the stream as a cancellation instead, so it does not fall back. The bound is off for the interactive main session, and with default timings it trips after the observational needs-attention (60000 ms) and long-running (240000 ms) notices, which surface a stalled child but never terminate it.
