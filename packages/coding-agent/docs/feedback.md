...
The initial debugger handoff contains only a bounded, privacy-scrubbed diagnostic snapshot, not the parent transcript, file contents, or raw artifacts. The debugger may investigate normally without implementing a fix or resetting, cleaning, stashing, or overwriting your pre-existing changes. It reports supported evidence and unknowns. If it is unavailable, interrupted, or fails, the draft says `Investigation unavailable`; an inconclusive investigation leaves the cause unknown.
