...
- Documented and integrated Cursor's GetUsable-authoritative catalog as an ordered sequence with one model occurrence per returned row, including blank/whitespace and duplicate IDs. Raw textual exact lookup uses the first occurrence without trimming, suffix/context parsing, ambiguity rejection, or rewriting; only the literal lowercase `cursor/` prefix identifies provider-qualified Cursor text, and a lowercase `cursor/<id>` with no live route is a terminal reselection failure rather than a fall-through to a same-named custom row. Saved/session/default/object identities likewise require provider exactly lowercase `cursor`; case or whitespace variants retain ordinary non-Cursor behavior. Bare references reserve no Cursor discovery, but an exact already-published live lowercase Cursor row wins before generic non-Cursor resolution; otherwise the bare reference remains ordinary. Atomic keeps no static list of historical Cursor IDs. Explicit SDK, session, and workflow Cursor objects require current live-registry membership plus exact valid occurrence identity; routing-shaped clones and missing, malformed, stale, or out-of-range occurrences fail without canonicalization or migration. Active sessions refresh to the same occurrence's current full metadata and fail clearly if it disappeared. Persisted provider/ID-only references recover only an unambiguous singleton and require reselection when current rows duplicate the ID because no occurrence identity was persisted. The selected flat ID and GetUsable Max state are sent in both request model structures, `RequestedModel.parameters` is empty, and tuple expansion, backend-route synthesis, and route-rewriting reasoning selectors remain removed ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
...
- Ordered Cursor cache saves and invalidations independently per account so delayed persistence cannot reload revoked state, erase a newer catalog, or recreate data after a later clear, and a later same-account save with an equal timestamp still replaces the earlier record. File-cache saves now distinguish applied from timestamp-skipped writes; after clear and marker persistence both fail, skipped, rejected, or unverifiable custom-cache positive saves retain the fail-closed tombstone until a replacement is confirmed to have landed. Persistence failures remain best-effort with fixed sanitized diagnostics. Caller cancellation and provider disposal now detach pending credential resolver waits, leave late outcomes inert, and prevent transport invocation when cancellation becomes visible after authorization. An authorization failure or caller abort before a paused tool turn is resumed preserves that paused turn for a later authorized retry ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Treated a successful empty GetUsable response as authoritative account state rather than a discovery/login error: login succeeds, registry rows and execution leases are revoked, the scoped cache is durably invalidated (including caches without `clear`), status is non-runnable `empty`, and restart rediscovery cannot resurrect stale routes. OAuth login invocations are monotonically owner-fenced from pre-authentication through cancellation, publication, authenticated-scope mutation, and return, so a stale login cannot revoke, abort, or replace a newer account or a producer shared by the current login ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
...
- Excluded static, generic-API, malformed, and non-current exact-lowercase Cursor objects from registry, CLI, default, restored-session, enabled-scope, SDK-session, session mutation, and workflow selection. Exact-lowercase Cursor model definitions loaded from `models.json` are ignored even when they copy structurally valid private routing metadata; case/whitespace provider variants remain ordinary custom providers. Executable lowercase Cursor rows must instead be current objects from authenticated GetUsable publication with the Cursor API and a structurally valid exact per-ID occurrence ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Reserved dynamic registration and unregistration of the exact lowercase `cursor` provider for loader-attested first-party Cursor module identities. Startup-queued, event-time, and direct registry attempts from generic extensions now fail before mutation even when they copy valid-looking private routing metadata, while genuine builtin GetUsable publication and teardown retain their source capability. Every canonical Cursor publication preserves its required top-level identity while replacing reachable nested metadata with isolated copies and recursively freezing the full graph before exposure, preventing captured aliases from changing routing, Max state, occurrence, headers, costs, or input metadata without freezing objects shared by ordinary providers. Untrusted OAuth model modifiers additionally receive isolated Cursor copies and cannot add, remove, reorder, clone, or alter canonical current Cursor rows; arbitrary providers also cannot replace the reserved `cursor-agent` stream handler and intercept Cursor execution credentials. Exact Cursor provider/model overrides, request credentials, and headers from `models.json` are stripped so static configuration cannot mutate live Max, image, routing, or transport metadata; model-header storage uses structured provider/model identity so arbitrary colon-bearing names cannot collide, and request configuration for distinct ordinary providers such as `cursor:proxy` remains intact across layered config files ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Exposed extension-context model discovery only when a real `model_catalog_discover` handler participates, preventing a no-handler runner method from attesting stale listed Cursor rows during strict workflow preflight ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Restored ordinary text continuity across source-CLI Cursor process restarts by reconstructing ordered prior user/assistant root-prompt blobs, while keeping earlier-image reconstruction deferred to [#1807](https://github.com/bastani-inc/atomic/issues/1807). Workflow validation and execution authorization now reject missing, malformed, removed, and out-of-range duplicate occurrences instead of silently migrating to occurrence zero, and active sessions adopt the same occurrence's current full metadata on catalog refresh. SDK startup, direct context-window selection, replay, model switching, and tree resume preserve the canonical current Cursor registry object rather than cloning away its execution provenance ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
