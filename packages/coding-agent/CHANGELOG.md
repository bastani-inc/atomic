...
- Fixed Windows release archives to avoid Bun's `--bytecode` standalone executable startup crash and the split-launcher sidecar bundle resolving a macOS build-machine `import.meta.url` at runtime on Windows. Windows binaries remain compiled standalone launchers but now ship the source payload instead of embedded bytecode until Bun's Windows bytecode-alignment fix is available, and sidecar `app.js` detects the split launcher via `ATOMIC_CODING_AGENT` so package assets resolve next to `atomic.exe`.
