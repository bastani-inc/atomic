...
- Added the exported typed `ProviderModelReference` seam and exact-reference helpers, including separate transient transport and persistable selection records, plus settings/session/picker support for persisting and distinguishing duplicate provider occurrences without rewriting public model IDs ([#1830](https://github.com/bastani-inc/atomic/issues/1830)).
...
- Fixed explicit Cursor startup selection (`--provider cursor --model <exact-route-id>`) retaining a catalog route whose generation had been invalidated during duplicate session initialization or a later isolated-TUI remote catalog refresh, causing requests to fail locally as stale before transport. Cwd-bound services now register and prepare required authoritative providers before option resolution; session construction refreshes ordinary providers without re-registering or re-preparing required providers; provider-scoped refresh generations keep skipped exact routes current; and RPC catalog refresh rebinds the engine child's active model to the refreshed exact identity. Direct SDK initialization, ordinary-provider and late runtime-key refresh, and missing-authentication behavior remain intact ([#1830](https://github.com/bastani-inc/atomic/issues/1830)).
...
- Fixed RPC and isolated interactive model selection losing provider-owned exact identity during JSON serialization, including `model_changed` events and runtime-only catalogs without a persistable account scope. Model-bearing RPC records now expose an opaque `providerSelection`, `set_model` revalidates it against the current available catalog, and duplicate Cursor route occurrences can be selected without persisting transient identity or allowing client-supplied routing metadata ([#1830](https://github.com/bastani-inc/atomic/issues/1830)).
- Fixed custom-provider SDK streaming to invoke the exact provider handler owned by each session's model registry, preventing parallel workflow stages or providers that share an API from displacing one another while preserving standard-provider and legacy global dispatch.
