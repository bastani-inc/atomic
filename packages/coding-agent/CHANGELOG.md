...
- Documented and integrated Cursor's GetUsable-authoritative catalog as an ordered sequence with one model occurrence per returned row, including blank/whitespace and duplicate IDs. Textual exact lookup uses the first occurrence, while a selected later model object retains current occurrence-specific routing. The selected flat ID and GetUsable Max state are sent in both request model structures, `RequestedModel.parameters` is empty, and tuple expansion, backend-route synthesis, and route-rewriting reasoning selectors remain removed ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Limited the separate best-effort AvailableModels operation to same-account image enrichment that requires every matching exact identity/variant record to report true. Duplicate or distinct agreeing matches remain image-capable; any false or omitted match is text-only. AvailableModels cannot add, remove, reorder, collapse, or change GetUsable membership, route identity, display/Max data, parameters, or text availability ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
...
- Required an authenticated Cursor discovery capability before strict workflow preflight or stage model resolution can read the model registry, so list-only stale rows cannot create a Run, job, body, stage session, prompt, or transport. Interactive deferred startup now retries a settings-only exact Cursor default after extensions load, while explicit and restored-session identity keep precedence and failed exact recovery still requires reselection ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
