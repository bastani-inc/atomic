...
- Fixed Atomic's bundled workflow graph flattening for nested `ctx.workflow()` composition. Valid child graphs now recursively replace import boundaries with faithful parent-to-root and terminal-to-downstream edges even when durable snapshots are not topologically ordered; repeated, sibling, sequential, and parallel children preserve distinct virtual identities and exact owning run/stage control targets; and boundaries with missing, empty, stale, mismatched, or recursively invalid child graphs, plus failed or skipped boundaries, retain a summary instead of exposing an unrelated run. Nested stages remain controllable through attach, send, pause, interrupt, resume, inspection, selection, and post-mortem chat without leaking implementation child runs into top-level workflow listings.
