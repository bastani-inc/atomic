...
- Added the configurable `createCodingAgentHarness()` factory, with Atomic's hashline coding tools, live prompt metadata, Atomic and `PI_*` bash session variables, injectable pi-agent-core execution environments, and runtime rejection of environments that omit `renameFile()`.
