...
- Fixed workflow-stage file output nomination around admitted external turns. An `output:` write now always persists the last non-empty assistant turn before the first admitted external message in the prompt window, regardless of admission provenance or candidate size; only when no assistant turn precedes that admission does it use the first non-empty assistant turn after it. Admissions and their persisted provenance remain visible in model context and companion transcripts. If discarded post-admission assistant content is at least 3:2 larger in UTF-8 bytes than the persisted turn, the receipt now emits an explicit warning and names the companion transcript to search.
