...
- Removed broad prompt-wide bare image path scanning in favor of explicit `@image`/`<file name="...">` references, cross-platform Atomic clipboard temp image paths, and Codex-style exact whole-message image paths, avoiding accidental filesystem fanout from prose such as `please explain /tmp/a.png` while preserving current-turn image attachments for supported providers.
- Fixed prompt image-reference resolution so existing `<file>...</file>` bodies, including unnamed, nested, malformed, and whitespace-formatted `name = "..."` tags, are not recursively scanned for nested `@image` references while preserving body text on pasted image file markers; CLI-generated file markers now also escape file-name attributes plus literal opening/closing file tags from text-file bodies so quoted content cannot prematurely expose image references to scanning.
- Fixed prompt image attachment merging so CLI `@image` file arguments that already carry structured images are not re-attached from their generated `<file>` marker, and repeated inline references to the same image do not duplicate attachments.
- Fixed extension-origin queued user messages during streaming so fire-and-forget `sendUserMessage()` calls without input handlers queue synchronously instead of being dropped behind async image-reference resolution, while extension-sourced prompts that still use prompt-template expansion queue the expanded text.
