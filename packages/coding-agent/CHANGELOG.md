...
- Changed the interactive status-bar dollar figure **and** token/cache badges (`↑`, `↓`, `R`, `W`) to report full transitive session spend — the current session plus subagents, sub-subagents, and workflow-stage descendants — updating live as descendants spend. Token badges going transitive keeps delegated spend visible on subscription/OAuth models where the per-token cost is `$0.000`. The context-window percentage remains current-session only, and `/cost` shows a reconciled self/descendant breakdown. Dropped the `~` lower-bound prefix on the cost figure because the footer total is always slightly behind the provider's own accounting ([#1636](https://github.com/bastani-inc/atomic/issues/1636)).
...
- Fixed live transitive spend reporting for background `/workflow` runs and subagent-heavy workflows such as `deep-research-codebase`: stale persisted-session walks can no longer discard newer live descendant reports, foreground subagent tool updates emit live usage rollups while child agents are still running, workflow stages forward nested descendant updates immediately to the launching footer, synthesized Copilot catalog models such as `gpt-5.6-sol` inherit known OpenAI-family pricing/tiers instead of zero-cost placeholders, and zero-priced token usage now renders an explicit `$0.000` segment rather than making the dollar field disappear ([#1636](https://github.com/bastani-inc/atomic/issues/1636)).
