...
- Fixed compaction stats mixing an authoritative token count with a heuristic one, which produced negative or nonsensical `percentReduction` values whenever a large `preserve_recent` tail was kept, and made the boundary's "Compacted from N tokens" line show a small heuristic figure instead of the authoritative whole-context total. Both sides of the reported reduction now come from one symmetric heuristic estimator — the protected-tail estimate is added to `tokensBefore` and, when the tail is kept, to `tokensAfter`, so it cancels out of a genuine ratio; when the tail is dropped it stays only in `tokensBefore`, so the reduction reflects the context actually shed. The durable boundary and its resumed rendering show the authoritative `tokensBefore` from the compaction entry, falling back to the heuristic stat only for legacy entries that lack it ([#2052](https://github.com/bastani-inc/atomic/issues/2052)).
