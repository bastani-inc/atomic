...
- Changed the built-in xAI default and the shipped xAI and OpenRouter workflow and subagent fallback chains from Grok 4.5 to Grok 4.6. Model guidance now uses DeepSWE v1.1 results and reporting corrections through August 14, 2026, including Grok 4.6's measured 67% `pass@1` at `xhigh`, the new DeepSeek budget-frontier points, corrected GPT-5.6 Luna/Terra costs, and Claude Opus 5's 74% ceiling; Artificial Analysis v4.1.1 agentic results provide the cross-benchmark basis for retaining Grok 4.6 as a provider-diverse fallback.
...
- Transcript-search highlighting no longer stalls on long rows containing malformed or repeated ANSI escape sequences; matching and ANSI-preserving styling now scan those rows linearly.
