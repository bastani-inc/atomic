...
- Fixed malformed or missing interactive RPC message payloads—including unsafe starts encountered between out-of-order lifecycle frames and assistant update/end snapshots with absent, non-array, or malformed content—crashing the main TUI or a workflow chat while a follow-up began. The RPC and reusable-chat event boundaries validate render-sensitive built-in message shapes, and both renderers reject unsafe assistant recovery snapshots, while preserving the open, declaration-merged `CustomAgentMessages` contract, including bash-execution, branch-summary, tool-result, and extension-defined roles. When an assistant start is dropped, its next valid update or end restores the main streaming view so the response still renders; aborted ends are normalized before that first render, preserving retry-count errors such as `Aborted after 2 retry attempts`. Malformed ends detach assistant-owned pending tools from active routing: the normal immediate tool start can reclaim its row, while a follow-up assistant first settles unresolved tools and fences late events from rewriting prior entries ([#1985](https://github.com/bastani-inc/atomic/issues/1985)).
