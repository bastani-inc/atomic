...
- Fixed tool-only workflows being reported failed after durable `ctx.tool()` side effects had completed. The workflow graph and completed-run inspector now show non-attachable tool nodes, including cached replay state, while preserving live declared outputs and preventing duplicate side effects ([#1991](https://github.com/bastani-inc/atomic/issues/1991)).
