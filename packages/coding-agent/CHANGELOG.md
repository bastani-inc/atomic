...
- Preserved extension Ctrl+C bindings under the remote-UI escape hatch. A focused remote `ctx.ui.custom()` component now receives the first Ctrl+C, so the workflows prompt card's `ctrl+c Skip` and the stage chat's `ctrl+c Close` keep working; if the same component is still holding input on the next press, the host takes that press and replaces the engine. Nothing is timed — only a second deliberate press escalates — and an unresponsive engine still escalates on the first press, since a wedged child cannot run a local cancel anyway.
