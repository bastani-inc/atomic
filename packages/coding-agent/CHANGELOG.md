...
- Added a `handlesCtrlC` option to `ctx.ui.custom()`. A component that declares it receives Ctrl+C while it owns input and keeps its own Skip, Close, or cancel binding; a component that does not is closed by the host on the first press, so a remote component that never resolves can no longer trap the keyboard. The bundled workflow graph, stage chat, prompt cards, inline form, and inputs picker declare it.
