...
- Added a clickable "Jump to latest message" label with the `tui.altScreen.bottom` shortcut to the fullscreen transcript while it is scrolled up ([#9080](https://github.com/rwachtler/pi/pull/9080) by [@rwachtler](https://github.com/rwachtler)).
...
- Added `/feedback` and a bundled skill for conversational drafts matching Atomic's current bug and enhancement issue forms, including required-field validation and exactly one foreground debugger investigation for bugs only. Diagnostics are bounded and scrubbed; every revision and the final pre-post check replace private data while disclosing category and count only. Clear conversational approval gates submission to `bastani-inc/atomic` with the matching label, duplicate protection, and honest failures that preserve the editable draft ([#2799](https://github.com/bastani-inc/atomic/issues/2799)).
