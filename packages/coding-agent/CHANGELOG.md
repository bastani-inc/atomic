...
- Fixed workflow-stage file output nomination around admitted external turns. An `output:` write now always persists the last non-empty assistant turn before the first admitted external message in the prompt window, regardless of admission provenance or candidate size. When no assistant text precedes that admission and the prohibition is therefore unsatisfiable, the fallback selects the largest non-empty post-admission turn by UTF-8 bytes, preferring the later turn on a tie so an acknowledgement cannot hide a subsequent report. Admissions and their persisted provenance remain visible in model context and companion transcripts. Whenever nomination discards any non-empty post-admission assistant content, the receipt warns without a size threshold, states whether a pre-admission stage turn or a post-admission fallback was persisted, and names the companion transcript to search.
