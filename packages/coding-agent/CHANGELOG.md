...
- Fixed workflow-stage file output nomination around admitted external turns. An `output:` write now treats the last non-empty assistant turn before the first admitted external message in the prompt window as the default, but a post-admission candidate at least 26 UTF-8 bytes and at least 3:2 larger can replace it so a short preamble cannot hide a report-scale deliverable. When no assistant text precedes the admission, nomination takes the most recent post-admission candidate at or above the 26-byte floor, falling back to the latest non-empty turn. Admissions and provenance remain visible in companion transcripts. Receipts warn, with the transcript path, only when plausibly substantive content was actually passed over; terse admission acknowledgements no longer add warning noise.
