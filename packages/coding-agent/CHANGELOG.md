...
- **Herdr reports top-level workflow runs.** A background workflow keeps the pane `working` while any nonterminal top-level run remains, changes it to `blocked` with a short workflow/stage label while a run awaits input, pauses, blocks, or fails, and returns to `idle` only after the last tracked run completes or quits. Concurrent runs are refcounted; user dialogs still take precedence. Nested workflow and subagent runs are excluded, and run ids, prompt bodies, stage prompts, tool arguments, model output, and provider error text never cross the Herdr socket. The state survives `/reload`, `/new`, resume, and fork through lifecycle snapshot seeding ([#2210](https://github.com/bastani-inc/atomic/issues/2210)).
