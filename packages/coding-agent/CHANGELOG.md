...
- Added the configurable `createCodingAgentHarness()` factory, with Atomic's hashline coding tools, live prompt metadata, Atomic and `PI_*` bash session variables, injectable pi-agent-core execution environments for read, bash, edit, write, and find, and runtime rejection of environments that omit `renameFile()`. The default `search` tool remains local until its filesystem and matching pipeline gains a complete remote operations seam.
