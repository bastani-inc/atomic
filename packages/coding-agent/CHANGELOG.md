...
- A session that was named and then cleared now reads as cleared rather than never-named. `getLatestSessionName` and the session-list reducer preserve the distinction the session file already records, and the new exported `SessionNameState` type and `getSessionNameState()` expose it to SDK consumers.
