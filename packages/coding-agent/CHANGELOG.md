...
- Fixed workflow stage file output nomination around admitted external turns. Admissions remain visible in model context and companion transcripts, and their provenance survives custom-message persistence; when provenance leaves a pre-admission deliverable versus a post-admission acknowledgement genuinely ambiguous, a documented 3:2 UTF-8-size heuristic prefers substantive text over a fragment. This remains a heuristic, so a short genuine deliverable paired with a longer acknowledgement can still be nominated incorrectly.
