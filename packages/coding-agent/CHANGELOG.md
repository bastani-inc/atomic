...
- Added an **experimental** fullscreen TUI layout. Start it with `--tui-mode fullscreen` or choose it in `/settings`; `regular` remains the default, and `/settings` can switch a live session without a restart. The fullscreen transcript scrolls above a sticky dock containing the editor, status line, usage meter, extension widgets, and footer, while wheel and trackpad input stays with the transcript rather than engine-hosted overlays such as workflow graphs ([#2269](https://github.com/bastani-inc/atomic/pull/2269), [#2274](https://github.com/bastani-inc/atomic/pull/2274)).
