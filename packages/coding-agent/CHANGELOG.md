...
- Kept an explicit escape hatch for a genuinely wedged engine, matching the watchdog's own `Esc interrupt · Ctrl+C terminate` copy. Once the engine is provably not answering, Ctrl+C stops the engine child and starts one replacement. That covers a watchdog-confirmed unresponsive engine, a cooperative abort left unanswered past the same one-second threshold, and — with no heartbeat or watchdog coverage of its own — a replacement engine still waiting for readiness past it, which was previously unrecoverable. A repeat Ctrl+C can stop an overdue replacement and start another, while a replacement that is still fresh is never stopped by a stray press. That Ctrl+C now reaches the host even while an engine-owned custom UI or overlay holds input, and host-native selectors, dialogs, and forms keep Ctrl+C-as-cancel whenever the engine is healthy.
