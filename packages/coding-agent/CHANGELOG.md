...
- Fixed being unable to tell which background workflow raised an `ask_user_question` or `ctx.ui` prompt when several runs are active. A run waiting on human input now carries the existing blue awaiting-input `？` indicator instead of the ordinary running dot, on the BACKGROUND widget, the `/workflow connect` picker, and the `/workflow status` list, and reverts to the running dot (or whatever state then applies) as soon as the prompt is answered, cancelled, or the stage moves on. A prompt raised inside a hidden nested `ctx.workflow(...)` child marks its visible top-level ancestor and leaves unrelated runs alone. The awaiting indicator never outranks a terminal, blocked, or quit state, so a completed, failed, killed, cancelled, or skipped run carrying stale pending-prompt state keeps its own glyph and colour. The prompt surfaces themselves — the graph-viewer prompt card and the attached stage chat, including brokered `ask_user_question` widgets — now name the originating run with the same short run id and workflow name the widget shows, but only while more than one top-level run is active; single-run output is unchanged. Attribution is applied at render time only, so stored prompt text stays verbatim for `workflow status` and `workflow send`. The `/workflow connect` picker now reports effective run status, keeping its indicators aligned with the BACKGROUND widget and `/workflow status`.
