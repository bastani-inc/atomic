...
- Fixed compaction retry evidence, extension compatibility, and duplicate occurrence protection. Provider rejections now update retry guards atomically only from a proven final transport and fail closed on incomplete thrown-overflow evidence; later identical/non-smaller local candidates are skipped without replacing real provider diagnostics. Trusted `session_before_compact` replacement text now writes an exact format-absent legacy hybrid with its structured tail (and needs no provider auth), while model-driven compaction remains validated full-collapse and incompatible no-legacy overrides fail before persistence. Isolated subsequence validation now finds the earliest exact monotone assignment that includes every protected duplicate or blank source occurrence, with an O(n+m) protected-suffix fast path and O(m)-memory general reconstruction.
- Fixed silent-overflow compaction so a completed assistant response whose reported input plus cache-read usage exceeds the selected context window is retained exactly once in the full-collapse boundary and rebuilt/reopened active context without a retry. Only a retryable overflow assistant artifact at the current leaf is excluded from compaction and retry context; its append-only JSONL entry remains archived.
...
- Fixed full-collapse overflow recovery near and beyond the model context limit. Warm construction now deep-clones the complete captured JSON-wire provider body, preserves every captured outer field and historical item, appends exactly one verified provider suffix without replaying the originating hook, and fails closed to isolated non-`KEEP` planning on any sequence/media/JSON ambiguity with no warm telemetry. Public Responses' single known leading system/developer item, installed Codex/Anthropic bodies, and deliberate simple Chat bodies are handled conservatively. Oversized protected tool results stay bounded to the canonical 16,000-character prefix plus marker across boundary/reopen/repeated compaction, while append-only JSONL, backups, and pre-boundary branches retain full raw bytes and tool IDs.
- Fixed full-collapse admission and retry correctness for provider-shaped payloads. A typed request-local side channel now preserves asynchronous final-payload fit/restoration failures even when a provider adapter resolves the hook throw as `stopReason:"error"`; local input/restoration advances boundedly, retry-guard blocks skip only the current projection, output-budget failures remain terminal, and only actual provider context rejection sets `providerOverflow`. Invalid `length` output advances only when an exact/projected count plus the transported numeric cap proves total-context headroom reduced it and the next input is changed and strictly smaller; later local blockage preserves the latest chronological real provider/eligible-length origin, its exact raw response and cap, while attempts count every entered buildable projection. Prompt occupancy now shares Anthropic's near-equal cache-mirror predicate across thresholds and warm capture while OpenAI/Codex keep disjoint sums, and the detached normal-stream result observer is rejection-neutral and generation-safe.
