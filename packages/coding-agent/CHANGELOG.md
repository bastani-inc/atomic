...
- `withCodexFastRouteHeaders` no longer takes an `enabled` flag, and `withChatGptCodexTransportRouting` no longer derives identity from a caller flag or the final payload tier. Both derive the first-party Codex identity from the model's own `fastRoute` — added only when that route declares the priority tier and the model uses first-party Codex routing — and the routing hint names the route's upstream model ID instead of the canonical `-fast` ID. A caller or payload hook therefore cannot grant fast identity to a normal model.
