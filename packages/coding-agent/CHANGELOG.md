...
- **Herdr reports top-level workflow runs.** A background workflow keeps the pane `working` while any nonterminal top-level run remains, changes it to `blocked` with a short workflow/stage label while a run awaits input, pauses, blocks, or fails, and returns to `idle` only after the last tracked run ends. Concurrent runs are refcounted; user dialogs still take precedence. Nested workflow and subagent runs are excluded, and run ids, prompt bodies, stage prompts, tool arguments, model output, and provider error text never cross the Herdr socket. A run that is killed, cancelled, or skipped ends its contribution the same way a completed one does, and the pane is never told it completed. Across `/reload`, `/new`, `/resume`, and fork the replacement session reconstructs what it reports from the workflow store it can observe: a run still live keeps the pane's state, and one the new session cannot see has its contribution dropped rather than left behind ([#2210](https://github.com/bastani-inc/atomic/issues/2210)).
