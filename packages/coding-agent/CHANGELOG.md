...
- A non-terminating in-process subagent turn no longer blocks its parent indefinitely ([#2446](https://github.com/bastani-inc/atomic/issues/2446)). `createAgentSession` accepts an optional `streamStallMs` content-progress liveness window; when set, each assistant-response stream is wrapped so a turn that stops producing content-bearing events — the reported failure mode was a flood of whitespace-only tool-call deltas that never reached a terminal event, leaving a foreground child pending and the parent blocked for hours with fallback never consulted — is ended with a *retryable* provider failure. Same-model retry and configured `fallbackModels` then advance within the same `prompt()` call, or, with no fallback, `prompt()` resolves with an error and control returns instead of hanging. The window measures time since the last content-bearing event, so long reasoning passes and slow in-flight tools keep resetting it and are unaffected; an interrupt still ends the stream as a cancellation that does not fall back; and the guard is inert (`0`/unset) for the interactive main session, which is byte-for-byte unchanged.
