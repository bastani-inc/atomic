...
- Fixed full-collapse admission and retry correctness for provider-shaped payloads. Atomic no longer treats serialized UTF-8 bytes as a hard token count: it prefers injected exact counts, then request-bound prompt occupancy plus a suffix only when the full historical visible prefix is unchanged, then explicit Responses/Chat/Anthropic/Google/Bedrock/Mistral projections with honest heuristic or unavailable confidence for opaque/media shapes. Prompt occupancy sums uncached input and both cache partitions while excluding output. Local fit failures no longer masquerade as observed provider overflow, output budgets/diagnostics cannot become negative, and request-local recovery is bounded to `warm → isolated-full → isolated-elided` inputs below the smallest rejected/blocked visible-input size; separate input and exact post-mutation transport fingerprints suppress identical resends.
