...
- Fixed an extension provider registration's offline catalog refresh overwriting a newer credential-resolved refresh under load, which could lose configured `$ENV` or `!command` API keys.
