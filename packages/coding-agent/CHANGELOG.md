...
- `WorkflowPendingStageDelivery` now requires a `fail(reason: Error): void` member. A delivery owner that has run out of recovery calls it to settle the stage's pending delivery terminally; `ready()` has no timeout, so a delivery nobody can settle leaves its stage parked forever. Implementers of the interface must add `fail`.
...
- Gemini 3.8 Flash is now selectable in Atomic on Google (`google/gemini-3.8-flash`), Google Vertex, GitHub Copilot, opencode zen, OpenRouter (`openrouter/google/gemini-3.8-flash` and its `:batch` variant), and the Vercel AI Gateway. It carries a 1,048,576-token context window and 65,536 maximum output tokens on Google, Google Vertex, opencode zen, OpenRouter, and GitHub Copilot alike, and 1,000,000 on the Vercel AI Gateway. Thinking is always on, so the selectable levels are exactly low, medium, and high on those five mirrors: off is unavailable, and minimal is unavailable because Google publishes only those three efforts for this model — unlike Gemini 3.5 and 3.6 Flash, which keep offering minimal. The Vercel AI Gateway is the exception: it publishes no per-model thinking levels for any of its `anthropic-messages` models, so its entry also offers off and minimal, exactly as its Gemini 3.6 and 3.7 Flash entries already did. On GitHub Copilot the model runs through Copilot's own OpenAI-compatible endpoint and sends the selected effort as the reasoning_effort request field. Run `--list-models` for the models your account actually has access to; Copilot availability follows GitHub's rollout and administrator policy ([#9076](https://github.com/earendil-works/pi/issues/9076)).
...
- Compaction reports a consistent token-reduction ratio and shows the authoritative "Compacted from N tokens" count. The before/after pair previously mixed a provider-authoritative `tokensBefore` with a `char/4` heuristic `tokensAfter` — and estimated the protected tail by subtracting one from the other — which could render a negative reduction percentage and a headline count that disagreed with the reported stats. Both sides of the ratio now come from the same estimator (a region term plus an explicit tail estimate) in one shared `computeWholeContextStats` path used by the planned, fresh, and extension results; the protected tail is counted on both sides and cancels when it survives, but is correctly not cancelled when the fresh rung drops it. The "Compacted from N tokens" line uses the provider-aware count while the internal heuristic stats stay self-consistent ([#2052](https://github.com/bastani-inc/atomic/issues/2052)).
