...
- **Builtin Herdr pane reporter.** Running Atomic inside a [Herdr](https://github.com/herdrdev/herdr) pane now makes that pane reflect what Atomic is doing — `working` during a turn, `idle` once the turn has fully settled, `blocked` while a dialog waits on you or when a turn ended in a provider error. There is nothing to install and nothing to configure. Reports carry only the pane id, the fixed `herdr:atomic` / `atomic` identity, the state, a short dialog title capped at 120 characters, a sequence number, and a session reference; prompt text, tool arguments, model output, and provider error text never cross the socket. A title within the cap is sent exactly as given, whitespace included, and a longer one is sent as its first 119 characters plus an ellipsis. A turn that ended in a provider error reports the fixed string `Agent turn failed` rather than the provider's own message, which routinely carries authorization headers and echoed prompt content. Idle is decided at `agent_settled`, so a pane does not flicker between a failed request and its retry. The reporter is a complete no-op outside a Herdr pane: the builtin is withheld before its factory ever runs in a host that presents no terminal pane, so a headless RPC, JSON, or print run registers no event handler even with the Herdr environment set, and without `HERDR_ENV=1`, `HERDR_PANE_ID`, and `HERDR_SOCKET_PATH` it opens no socket and starts no timer. Inside a pane the builtin is the pane's only writer: the resource loader skips Herdr's installed file-based `herdr-agent-state` integration (`herdr-agent-state.ts`/`.js`) at load time, so the pane never gets two writers and an installed integration that loads but silently fails can no longer stand the builtin down and leave the pane unreported; outside a Herdr pane that file loads exactly as before. A dead, refusing, or hung socket degrades to silence after one 500 ms attempt and one 1500 ms retry. Ordinary lifecycle callbacks never wait for socket I/O and a non-quit shutdown returns at once; quitting deliberately drains the queue and then attempts the release, which against a socket that accepts and never answers is bounded by the transport budgets at roughly six seconds. State, session identity, and the release share one writer, so exactly one request is ever in flight and state queued behind it coalesces to the newest value without ever collapsing across a session or release entry. `/reload`, `/new`, `/resume`, and forking hand over to a successor whose sequence numbers continue above the predecessor's. Deferred extension loading is handled too: a reporter that loads mid-turn or mid-dialog seeds both from the host and reports the wait it arrived into. One wait is deliberately not reported: the startup project-trust prompt is shown by the terminal host before the interactive engine child exists, so reporting begins once trust resolves. See [Herdr](/herdr) ([#2210](https://github.com/bastani-inc/atomic/issues/2210)).
- **Herdr reports top-level workflow runs.** A background workflow keeps the pane `working` while any nonterminal top-level run remains, changes it to `blocked` with a short workflow/stage label while a run awaits input, pauses, blocks, or fails, and returns to `idle` only after the last tracked run ends. Concurrent runs are refcounted; user dialogs still take precedence. Nested workflow and subagent runs are excluded, and run ids, prompt bodies, stage prompts, tool arguments, model output, and provider error text never cross the Herdr socket. A run that is killed, cancelled, or skipped ends its contribution the same way a completed one does, and the pane is never told it completed. `/reload`, `/new`, `/resume`, and fork all replace the session inside one process that keeps running the workflows, so the replacement re-adopts a run that is still live and keeps reporting it, reconstructed from the workflow store it can observe rather than from events it may never have seen; a terminal or unobservable run has its contribution dropped rather than left behind ([#2210](https://github.com/bastani-inc/atomic/issues/2210)).
- **`pi.awaitUserDecision(label, reason)` on `ExtensionAPI`.** Extensions can declare that the agent is waiting on a person, for a wait Atomic cannot otherwise see — a component the extension mounts and drives itself. It returns a `UserBlock`, the only handle that can end the block; `release()` is idempotent and safe in a `finally`. Reasons are `"dialog"`, `"project_trust"`, `"workflow_prompt"`, and `"supervisor_ask"` (the last two are reserved). Blocks are reference counted, so nesting one dialog inside another holds two and the agent leaves the blocked state only when the last releases; the oldest open block's label is the one reported as the current wait. There is deliberately no release-by-id, release-by-label, or release-all call, so one extension can never end another's wait. See [Extensions](/extensions#pi-awaituserdecision-label-reason) ([#2210](https://github.com/bastani-inc/atomic/issues/2210)).
- **`agent_blocked` and `agent_unblocked` extension events.** Fired when the agent starts and stops waiting on a person, carrying the block id, short label, reason, the number of blocks still open, and the label of the oldest open block. Every blocking `ctx.ui` dialog — `select`, `confirm`, `input`, `custom`, `editor` — now opens and releases a block automatically, so these events fire without any extension having to declare anything. A `custom()` overlay whose caller holds an `OverlayHandle` (through `options.onHandle`) tracks visibility: hiding the overlay with `setHidden(true)` or `hide()` releases the block even while the dialog's promise stays pending, and re-showing it opens a new one — so a mounted-but-hidden overlay, such as the workflow graph viewer after Ctrl+X, never pins the agent (or a Herdr pane) at `blocked`. They are session-scoped: Atomic's startup project-trust prompt opens a `project_trust` block, but it runs before extensions are bound, so no handler is registered yet to receive it; blocks opened once the session is running are delivered normally. Wrapping is applied at the single `ctx.ui` accessor and is transparent: every member, blocking or not, is invoked on the host's own object, so a UI context whose methods read private fields or per-instance state keyed by receiver identity behaves exactly as it did before, and a frozen or individually sealed UI context keeps working. Return values, cancellation, thrown errors, `ctx.ui` identity, own keys, and repeated member lookup are all unchanged; members are forwarders rather than the host's own function objects, which is the one way a wrapped context differs from an unwrapped one ([#2210](https://github.com/bastani-inc/atomic/issues/2210)).
- **`sessionScopedExtensionState(scope, key, create)` on the host API.** `/reload` re-evaluates a file extension's whole module graph, so module-scoped singletons restart empty while the session — and any work the extension started — keeps running. An extension can now register keyed state against the host session scope it serves (`pi.events`; every per-extension facade resolves to the one shared bus) and re-bind to that state on each load. Two in-process sessions with distinct buses stay isolated, and entries live exactly as long as the bus they serve. The workflows extension uses this to keep in-flight runs observable and controllable across `/reload` ([#2247](https://github.com/bastani-inc/atomic/issues/2247)).
