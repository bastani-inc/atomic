...
- Added shared Pier/Harbor eval sandbox provisioning for every external runtime declared by Atomic's default shipped skills: pinned Playwright CLI with a preinstalled, launch-verified Chromium browser; pinned LiteParse and uv with Python/search dependencies; tmux; LibreOffice; ImageMagick; and Python/PyYAML. The exhaustive inventory explicitly excludes project-local developer skills, preserves Pier's apk/apt/yum and Harbor's Debian installation paths, and fails setup early when a required CLI is unusable.
