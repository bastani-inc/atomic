...
- Fenced delayed stored Cursor credential lookups so an obsolete account, missing credential, or lookup error cannot overwrite or revoke a newer account catalog, cache, live execution authority, freshness state, or UI status, including after disposal ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
