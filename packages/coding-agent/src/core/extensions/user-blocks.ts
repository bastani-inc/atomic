 ...
 *
 * Every path that stops the agent to wait on a person — an extension dialog,
 * the project-trust prompt — opens a block here and releases it through the
 * handle it was given. There is deliberately no release-by-id, release-by-label,
 * or clear-all entry point: a block can only be ended by the code that opened
 * it, so one caller can never end another caller's wait.
 *
 * The registry is module scope on purpose. The project-trust prompt runs before
 * any `ExtensionRunner` exists, and successor sessions inside one process must
 * see the same open-block set, so a per-runner registry could not refcount the
 * pane's real state.
 */
