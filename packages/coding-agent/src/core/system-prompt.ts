...
function buildRules(
	...
	if (hasBash || hasPowerShell) {
		...
		addGuideline(
			[
				"**Execution history**: Leave a mineable record of how each task was executed and what the user prefers, and mine those records to calibrate later work. The records only accumulate value if every task writes them.",
				"  - When you author a commit, PR, issue, or issue/review comment for a task, record how it ran: `Assistant-workflow: <workflow name> (run <runId>)` for a registered or custom workflow, or `Assistant-workflow: inline` when no workflow was used; and `Assistant-duration: <elapsed> <converged|blocked|abandoned>, estimated <estimate>` (for example `Assistant-duration: 42m converged, estimated 30m`), measuring wall-clock time from launch or task start until acceptance was proven or work stopped, including resumes and repair rounds. Take elapsed time from workflow status timing or timestamps you recorded (for example `date` at task start); write `unmeasured` rather than guess. Put these as git trailers alongside `Assistant-model` in commits and as the same `Key: value` lines in PR, issue, and comment bodies. For multi-commit work, record the workflow on each commit and the final duration once, in the last commit or the PR body.",
				"  - When the user states or reveals a durable preference about how they work (a correction, a rejected approach, scope, review, tooling, or communication style), record it in the same artifacts as `User-preference: <one line, in the user's terms>`. Record only what the user expressed or their own edits show; never record secrets, credentials, or personal details unrelated to the work, and skip it if the user asks you not to. If repository rules restrict commit or PR formats, fit the records into what they allow.",
				"  - Before choosing between a workflow and inline work, estimating a duration, or interpreting an ambiguous request, mine these records with the rest of the history, for example `git log --format='%h %s%n%(trailers:key=Assistant-workflow,key=Assistant-duration,key=User-preference)'` and `gh pr list --state all --search 'Assistant-workflow in:body'`. Compare tasks of similar kind and size, and state the sample size with the median and range. Treat fewer than five comparable records as anecdotal, and prefer the more recent record when two conflict. Apply mined preferences unless the current request contradicts them. This history is a guide, not the decision: the current request, the task itself, and explicit user instructions decide between a workflow and inline work.",
			].join("\n"),
		);
	}
