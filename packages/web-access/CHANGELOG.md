...
- Added an optional You.com search provider for `web_search`. Configure it with `youcomApiKey` in `web-search.json` or the `YDC_API_KEY` environment variable, and select it with `provider: "youcom"` (or per call). Under automatic provider selection it sits after Exa and Perplexity and before the Gemini fallbacks. `domainFilter` includes are sent to the API as `include_domains` and a purely exclude filter as `exclude_domains`; a filter that mixes both sends `include_domains` and enforces the exclusions client-side because the API rejects the combination. Every result is re-checked client-side against the filter, so a filtered search can return fewer results than `numResults`. The filter must be an array, and every entry must be a hostname, optionally prefixed with `-`; a non-array filter and blank, non-string, or malformed entries are rejected with an error rather than ignored. When automatic provider selection reaches You.com, a rejected domain filter stops the search rather than falling back to another provider. The You.com answer is assembled from result snippets with source citations ([#2949](https://github.com/bastani-inc/atomic/pull/2949) by [@mouse-value-add](https://github.com/mouse-value-add)).
