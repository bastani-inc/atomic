...
An explicit `send`, `ask`, or `reply` that reports `Client disconnected` should be retried identically up to three times. The tool retains the failed operation's identity for nine minutes and reuses it only for those recoverable retries, while the broker retains successful delivery identities for ten minutes. This closes the acknowledgement-loss window: a retry of an operation the broker already accepted reports success without delivering it twice, and a retried `ask` keeps its question correlation. A settled result or non-recoverable failure releases the identity immediately, so intentionally repeating the same message after success still creates and delivers a distinct operation.
