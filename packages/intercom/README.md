...
An explicit `send`, `ask`, or `reply` that reports `Client disconnected` should be retried identically up to three times. Retry identity uses the caller-issued target verbatim rather than the resolved broker session ID, so recipient reconnects do not change a name-addressed operation and different caller aliases remain different operations. The client retains an identity for at most 11 minutes from the original attempt—the full 10-minute `ask` wait plus a 1-minute retry opportunity—with no deadline extension and at most three reuses. Settled and non-recoverable outcomes release it immediately.

The broker keeps accepted-operation authority for 12 minutes in `delivered-messages.sqlite`, including stable sender and recipient identities plus question-route metadata. It reserves an identity durably before forwarding, then marks it accepted after the confirmed write and before acknowledging the sender. This survives broker-process replacement: after acceptance and acknowledgement loss, the identical retry reports success without redelivery; a crash that leaves only an uncertain reservation is refused instead of retried unsafely. A deduplicated ask remains answerable through the recipient's public `reply` action after the asker reconnects. Reply routing resolves the stored sender name again but still requires the broker's exact pending thread and group authorization; ambiguous names, changed stable endpoints, target aliases, payloads, and message IDs cannot claim an accepted operation. Legacy sends without the optional logical target retain transport-target behavior.

Retry state is independently bounded and fails closed at pressure. The client holds at most 1,000 retained identities and, on overflow, refuses operation starts until all possibly accepted identities reach their original 11-minute deadlines. The broker retains at most 10,000 live authority records and 64 MiB of UTF-8 record content; it refuses new delivery rather than evicting a live record, then accepts again after 12-minute TTL cleanup. SQLite transactions serialize replacement-broker access, and malformed or truncated storage refuses delivery rather than authorizing a duplicate.
...
- `delivered-messages.sqlite` — bounded durable accepted-operation authority
