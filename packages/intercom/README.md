...
An explicit `send`, `ask`, or `reply` that reports `Client disconnected` should be retried identically up to three times. The tool retains the failed operation's identity for up to nine minutes from the original attempt and reuses it only for those recoverable retries, while the broker retains successful delivery identities for ten minutes. This closes the acknowledgement-loss window: a retry of an operation the broker already accepted reports success without delivering it twice, and a retried `ask` keeps its question correlation. A settled delivery result or a non-recoverable failure releases the identity immediately, so sending the same content again later is still a distinct operation. Across reconnects, the broker signs the delivery record with the client's stable registration return address rather than its newly generated broker session ID; a legacy registration without a return address falls back to that session ID, while a genuinely different sender or changed target or payload still fails with `message_id_conflict`.
