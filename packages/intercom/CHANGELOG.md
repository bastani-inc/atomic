...
- The startup log is capped at 8 KiB rather than growing without limit. The parent truncates it on every spawn, but the parent exits while the detached broker keeps running, so the broker applies the cap itself before it starts listening. External truncation would not work — a POSIX append descriptor writes at the new end of file, and the inherited Windows append handle has no truncate right ([#2208](https://github.com/bastani-inc/atomic/issues/2208)).
