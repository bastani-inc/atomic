...
- A recoverable `Client disconnected` error and Intercom's model-visible tool guidance now tell agents to retry the same call up to three times, so a stale broker socket triggers the existing lazy reconnection instead of being misdiagnosed as a permanently broken coordination channel.
- Retrying an identical `send`, `ask`, or `reply` after a recoverable disconnect now reuses the failed operation's bounded message identity. If the broker accepted the operation but its acknowledgement was lost, the retry reports success without delivering a duplicate; settled and non-recoverable outcomes release the identity immediately, so intentional repeated messages remain distinct.
- A stable-message retry no longer fails with `message_id_conflict` solely because its sender reconnected. The broker previously included its newly generated session ID in the delivered-message signature, so the same sender's unchanged target and payload appeared to come from a conflicting operation after every registration. Delivery records now use the client's reconnect-stable registration return address when available, with the transient session ID retained as the fallback for legacy registrations; a genuinely different sender or changed target or payload still conflicts.
- Retry identity now preserves the model-issued target verbatim while the resolved transport target changes independently. A name-addressed retry therefore remains stable across recipient reconnection, while different raw aliases such as a name and its exact session ID remain distinct operations; legacy wire frames that omit the new logical target retain their existing transport-target behavior.
- A recipient can now use the public `reply` action after an accepted ask loses its acknowledgement and the asker reconnects. The retry rebinds the broker's exact pending route, while the recipient resolves the recorded sender name again; ambiguous aliases, changed stable peers or groups, and mismatched message IDs or payloads are refused without redelivery or route theft.
- Broker acceptance records now persist in bounded SQLite authority below the configured agent Intercom directory for 12 minutes, surviving actual broker-process replacement with stable sender, recipient, payload, logical-target, and question-route checks. An operation is durably reserved before forwarding and marked accepted after the confirmed write but before sender acknowledgement; a surviving uncertain reservation is refused rather than redelivered. The client retry identity lasts at most 11 minutes—the full 10-minute ask window plus one minute to retry—without deadline extension and with three reuses. Client retention is bounded at 1,000 identities; broker authority is bounded at 10,000 records and 64 MiB of UTF-8 content. Capacity, corrupt storage, and malformed records fail closed instead of evicting a live acceptance proof and risking duplicate delivery.
