...
- Intercom's model-visible tool guidance now tells agents to retry the same call once after the exact recoverable `Client disconnected` result, so a stale broker socket triggers the existing lazy reconnection instead of being misdiagnosed as a permanently broken coordination channel.
