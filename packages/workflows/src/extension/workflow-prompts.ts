...
export const DEFAULT_PROMPT_GUIDANCE: string[] = [
  ...
  - Prefer using the \`goal\` workflow for small fixes/quick fixes, and the \`ralph\` workflow for tasks that are non-trivial (over 2K LoC estimated diff). Adjust the \`max_loops\` as well based on task complexity (estimated LoC and number of unique files that are touched).
