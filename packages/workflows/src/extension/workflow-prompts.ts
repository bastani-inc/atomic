...
export const DEFAULT_PROMPT_GUIDANCE: string[] = [
  ...
  - Named workflows may be builtin, project, user, or package supplied. Direct \`task\`, \`tasks\`, and \`chain\` modes provide one-off tracked shapes. For richer branches, loops, gates, child workflows, or human-in-the-loop behavior, author a custom TypeScript \`workflow({...})\` with normal coding tools, reload workflow resources, then run it. The workflow tool does not have a create action; do not force-fit a builtin such as \`goal\` or \`ralph\` when a direct shape or custom workflow better matches the job.`,
