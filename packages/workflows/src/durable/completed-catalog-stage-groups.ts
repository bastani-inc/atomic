...
export function validBoundaryRecordSet(checkpoints: readonly DurableCheckpoint[], strict: boolean): boolean {
  ...
  if (boundaries.some((checkpoint) => checkpoint.topology!.boundary!.event === "terminal"
    && startsByReplay.get(checkpoint.replayKey)?.length !== 1)) return false;
  return [...startsByReplay.values()].every((starts) =>
    boundaryTransitionError(starts[0]!, stages, strict) === undefined
  );
}
...
export function validStageGroup(
  ...
  const first = runs[0];
  if (runs.some((run) => run.runId !== runId || (first !== undefined
    && (run.runName !== first.runName || run.parentRunId !== first.parentRunId
      || run.parentStageId !== first.parentStageId || run.rootRunId !== first.rootRunId)))) return false;
...
export function runTopologyFor(
  ...
  return drafts.find((draft) => draft.topology?.run !== undefined)?.topology?.run
...
export function validateRunGroups(
  ...
  const toolsFor = (runId: string) => tools.filter((checkpoint) =>
  ...
  for (const [parentRunId, drafts] of grouped) {
    for (const draft of drafts) {
      ...
      if (childRun?.parentRunId !== parentRunId || childRun.parentStageId !== draft.topology?.stageId
        || (childRun.rootRunId !== undefined && childRun.rootRunId !== rootRunId)) return false;
      owners.set(childRunId, draft);
    }
  }
  for (const [runId, drafts] of grouped) {
    if (runId !== rootRunId
      && (runTopologyFor(drafts, toolsFor(runId)) === undefined || owners.get(runId) === undefined)) return false;
  }
...
export function mergeStageGroup(
  ...
  void _output; void _result; void _topology;
...
export function mergeStageDraft(
  ...
  return {
    ...
    sourceIds: [...new Set([...(existing?.sourceIds ?? []),
    ...
    ...(checkpoint.topology !== undefined ? { topology: checkpoint.topology }
...
export function workflowChildFromDraft(draft: StageDraft): StageSnapshot["workflowChild"] | undefined {
  ...
  return {
    alias: child.workflow, workflow: child.workflow, runId: child.runId,
    status: child.status, ...(child.exited !== undefined ? { exited: child.exited } : {}),
...
function hasCurrentStageIdentity(draft: StageDraft): boolean {
  return draft.topology?.sourceOrder !== undefined || draft.topology?.status !== undefined
    || draft.topology?.occurrenceKey !== undefined || draft.topology?.boundary !== undefined;
}
