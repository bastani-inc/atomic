...
- Stopped the `/workflow resume` and `/workflows` picker from full-scanning the durable directory after a normal register/checkpoint/status/prompt/delete write. Catalog freshness is now decoupled from the durable-directory mtime: a monotonic `generation` watermark plus a persistent dirty-id journal is the self-write change token, so the synchronous serve gate checks only index integrity (schema version, `complete`, empty dirty journal) and a known write leaves the catalog immediately servable — the seeded 100,000-run benchmark performs zero directory scans after mutating one known workflow and still lists in well under a second. A crash between marking a workflow dirty and syncing its row is repaired by re-reading only the journaled ids (O(dirty)), never by enumerating the directory. Out-of-band additions/deletions/in-place edits still reconcile, but asynchronously through a single coalesced background pass (at most one reconcile in flight; a burst of drift observations collapses to one scan) that never runs on the picker/render turn, and selecting a row authoritatively revalidates it — stale/vanished rows are rejected and repaired out of the index. The catalog schema is bumped `1 → 2` (the reconciled directory signature is now written only by a full/background reconcile, never by an incremental row write, preserving the invariant that a self-write cannot bless an unrelated external deletion); a v1 catalog cold-rebuilds once on first open. Debug-gated phase timings (`ATOMIC_WORKFLOW_DEBUG=1`) cover resource discovery, index open/freshness, targeted dirty repair, background reconcile, SQL query, and selected hydration.
