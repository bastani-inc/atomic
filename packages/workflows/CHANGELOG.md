...
- Restructured the builtin `open-claude-design` workflow around the accessible `impeccable` skill (`/skill:impeccable …`). The new phase order is: (1) **discovery** — a `discovery` stage runs `/skill:impeccable shape` to interview the user (via the structured question tool) for a confirmed design brief, the output type (`prototype`/`wireframe`/`page`/`component`/`theme`/`tokens`), and the references to emulate (URLs/files/screenshots/design docs), returning a structured `{ brief, output_type, references }`; (2) **init (always runs)** — `ensureProjectDesignContext` invokes `/skill:impeccable init` on every run, reusing the discovery answers; it detects `PRODUCT.md`/`DESIGN.md` (root, `.agents/context/`, or `docs/`, case-insensitive), creates whichever is missing, and reconciles existing files against the brief without clobbering them; (3) **a single combined context phase** — one concurrent `ctx.parallel` fan-out runs design-system onboarding (`ds-locator`/`ds-analyzer`/`ds-patterns`), the gated gallery **reference-discovery** (Awwwards, recent.design, Dribbble, Monet, Motionsites via `playwright-cli` — clicking into standout work to record a scroll-through video of each real design page so its animations are captured, with a full-page screenshot as a supplement/fallback and the real destination URL, not just the gallery thumbnails — with a web-search fallback, persisted to `<artifact_dir>/references.md`), and per-reference **import** (`web-capture-<n>` / `file-parser-<n>`) together, after which `design-system-builder` synthesizes the design system; (4) **generation**; (5) **live-driven refinement**; (6) export. User-provided references take **precedence over `DESIGN.md`/`PRODUCT.md`** (a shared `REFERENCE_PRECEDENCE` note is threaded into the import, generator, and apply-changes prompts; `DESIGN.md` fills gaps the references don't cover, `PRODUCT.md` still governs strategic register/voice). The `preview-display-*` stages drive `/skill:impeccable live` against the static `preview.html` for in-browser element-level variant QA, capturing accepted variants via a new `live_changes` field threaded through the refinement feedback; live degrades to `playwright-cli show --annotate` then a manual file path. New logic lives in `open-claude-design-setup.ts` (`runDiscovery`, `detectDesignContextFiles`, `ensureProjectDesignContext`, `buildReferenceDiscoveryPrompt`, `persistReferencesBrief`, `buildLivePreviewDisplayPrompt`) and `open-claude-design-utils.ts` (`discoveryDecisionSchema`, `discoveryDecisionFromResult`, `REFERENCE_PRECEDENCE`) to keep the runner and phases files under the 500-line file-length gate. Added unit coverage for the discovery schema/parse, discovery prompt, always-run init + reconcile + discovery-context threading, the combined context fan-out, reference-discovery prompt/persist, the live-QA prompt, the trimmed input contract, a full-run `init` path, and `live_changes` capture/threading/persistence.
...
- Fixed the builtin `open-claude-design` workflow dropping interactive Playwright annotation feedback between the `preview-display-*` stages and the downstream `user-feedback-*` / `apply-changes-*` refinement stages, so a user could annotate a preview and watch the next iteration apply only internal critique while the design stayed visually unchanged ([#1464](https://github.com/bastani-inc/atomic/issues/1464)). The initial and per-iteration `preview-display-*` results are now captured instead of awaited-and-discarded, parsed for the user's `user_notes` / `annotated_snapshot` (tolerant of heading/bullet/bold/backtick label styles and placeholder values), and carried through the refinement loop as a `previewFeedbackHistory` so every later iteration honors earlier annotations. `user-feedback-*` now receives the captured annotations as the PRIMARY signal in a dedicated `<user_annotations>` block (and is told not to approve export while honest user feedback is unaddressed); `apply-changes-*` now consumes a single merged refinement brief built by the new `open-claude-design-feedback.ts` helper that orders **user annotations above** the reviewer decision, impeccable critique, and screenshot/visual-QA findings, with a prompt contract requiring each annotation to be visibly addressed or explicitly explained as a DESIGN.md conflict. Captured annotations are also persisted as durable workflow artifacts under `<artifact_dir>/feedback/iteration-<n>.md` / `.json` (and a best-effort copy of the annotated snapshot `.png`/`.yaml`). A guardrail runs before each apply stage: if a `preview-display-*` stage captured `user_notes` but they did not thread into the apply prompt, the workflow now fails loudly instead of silently refining without user feedback. Added regression coverage that a `preview-display-*` annotation result threads verbatim into both `user-feedback-*` and `apply-changes-*` (ordered above internal critique) plus unit coverage for the parser, merged-brief ordering, guardrail, fallback, and artifact persistence.
...
- Hardened the builtin `open-claude-design` refinement/export flow after code review: (1) the export gate no longer drops user feedback — an immediate `ready_for_export` is refused while the latest preview captured unaddressed annotations (forcing one apply pass via `assertUserAnnotationsThreaded`), the **final** refinement iteration and the post-export `final-display` are now read-only (they no longer solicit `user_notes`/`live_changes` they cannot apply and instead point the user at re-running), so terminal annotations are never captured-then-orphaned; (2) `assertUserAnnotationsThreaded` now enforces accepted `live_changes` threading in addition to `user_notes`; (3) added a deterministic, browser-centric early exit — when the `playwright-cli` browser is unavailable the run calls `ctx.exit()` up front (surfacing the would-be artifact paths and install instructions) instead of generating a design no one can review, gated off under `NODE_ENV=test` and runtimes without `ctx.exit`; (4) `copyAnnotationArtifacts` now refuses to copy a model-supplied snapshot path that resolves outside the project/artifact dir; (5) the parallel onboarding prompts (`ds-analyzer`/`ds-patterns`) were reworded to make clear each does its own independent scan rather than depending on `ds-locator`; (6) dropped `a`/`n` from the placeholder-token set so a one-character real note survives, and fixed a `staff design enginer` prompt typo. Added unit coverage for the export-gate guard, the `live_changes` guardrail, the read-only final prompt, the snapshot-containment check, and the early-exit predicate.
