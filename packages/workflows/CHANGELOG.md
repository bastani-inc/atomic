...
- Fixed cross-session `/workflow resume` so exact `--session` restoration cannot let an ended local snapshot shadow the authoritative paused durable root, added heartbeat-backed crash-reclaimable execution leases for both file and DBOS durability so active roots are hidden from discovery and refused on targeted resume instead of being double-dispatched while malformed, foreign-host, and reused-PID stale leases recover, made stale write locks recover after interrupted owner publication, encoded Windows-illegal workflow-id characters in durable filenames, refuse definition-incompatible resume before stage execution instead of replaying persisted progress through changed workflow code, and order multiple resumable roots deterministically by newest update with an id tie-break.
