...
- Added durable `ctx.stage` / `ctx.task` checkpointing: completed stage/task outputs are recorded with stable ordinal replay keys and replayed on resume, skipping re-execution while avoiding checkpoint id collisions after redispatch. ([#1498](https://github.com/bastani-inc/atomic/issues/1498))
