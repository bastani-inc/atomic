...
- Changed workflow model validation so provider-qualified Cursor strings and `Model` objects must match the authenticated exact live catalog before stage session creation. Only the literal lowercase `cursor/` prefix and exact lowercase object provider `cursor` activate Cursor discovery; case or whitespace variants retain ordinary non-Cursor behavior. Bare references reserve no Cursor discovery, but an exact already-published live lowercase Cursor row wins before ordinary generic resolution; otherwise bare pass-through is unchanged. Provider-qualified text—including `cursor/` for a blank route—selects the first ordered occurrence byte-for-byte; generic trimming, context tokens, reasoning suffixes, and fuzzy/bare lookup never rewrite another string into a Cursor route. An explicit Cursor object is accepted only when it is the current live catalog object with a valid exact per-ID occurrence. Routing-shaped clones and missing, malformed, stale, or out-of-range occurrences fail with reselection guidance without canonicalization, migration, configured/current/provider/model fallback, or stage side effects; generic non-Cursor behavior remains unchanged, and builtins no longer hard-code account-specific Cursor routes ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
- Awaited the shared `model_catalog_discover` lifecycle before reading the registry for explicit lowercase `cursor/` strings and exact-lowercase-provider Cursor objects; bare references reserve no Cursor discovery, though an exact already-published live lowercase Cursor row may win during ordinary registry matching. Atomic retains no static list of historical Cursor IDs. Named `workflow({ action: "run" })` launches now perform the same preflight before creating a Run/store/stage entry; concurrent launches share discovery, cancellation or an unavailable exact route creates no Run and cannot fall through, and non-Cursor launches are not delayed ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
...
- Required strict Cursor catalog rows to carry structurally valid exact GetUsable routing metadata, and required the host extension context to expose discovery only when a real `model_catalog_discover` handler participates. Static/generic or malformed lowercase Cursor rows and no-handler runners now fail before catalog listing or Run/stage side effects ([#1702](https://github.com/bastani-inc/atomic/issues/1702)).
