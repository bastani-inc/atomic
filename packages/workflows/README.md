...
Ctrl+Shift+F searches the attached stage chat itself: a find box opens over that chat — never the main transcript behind it — and searches every row of the whole stage transcript, including a long streaming answer and rows far above the window. Navigation follows the ordinary `tui.altScreen.search*` keybindings; Escape closes the box without interrupting the stage.
