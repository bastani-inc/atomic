...
Cursor is a deliberate exception to generic workflow fallback resolution because its authenticated live catalog is the only executable authority. Before validating an explicit lowercase `cursor/<id>` string or exact-provider Cursor `Model` object, workflows await the shared authenticated catalog-discovery lifecycle. Named `workflow({ action: "run" })` dispatch performs this preflight before creating any Run, store entry, or stage session; concurrent named Cursor launches share discovery, while failure or cancellation creates no Run. A matching object resolves to the exact live catalog occurrence, so executable api, base URL, and routing never come from the caller; an unavailable or stale explicit reference fails with reselection guidance and does not advance to configured, current, provider, or model fallbacks. Bare references reserve no Cursor discovery, but an exact already-registered live lowercase Cursor row wins before ordinary non-Cursor matching; every other bare ID follows generic resolution. Non-Cursor IDs and fallback behavior are otherwise unchanged and do not wait for Cursor discovery. Builtins do not hard-code account-specific Cursor routes.
