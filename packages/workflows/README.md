...
Ralph's orchestrator omits the subagent tool's explicit `model` argument by default so named agents use their configured model and fallback policy. An override requires an exact user request or a documented task-specific need, plus a reason recorded before launch; ad hoc model selection for diversity is forbidden. The orchestrator also gives every delegated subagent in the run one shared invocation-specific, non-`default` Intercom group while preserving cross-group `contact_supervisor` escalation.
