...
- If the path is missing, the parent directory is created and Git runs `git worktree add --detach <path> <baseBranch>` from the filesystem-resolved canonical main checkout. `baseBranch` defaults to the invoking checkout's `HEAD`. Missing targets whose existing parent resolves through a symlink beneath the invoking checkout are rejected.
...
`worktree: true` is different: it creates branch-backed temporary worktrees under `<mainRoot>/.atomic/worktrees/` using flattened names and `git worktree add -B worktree-<name>`, then force-removes each worktree and branch after execution. Linked-checkout invocations remain main-root anchored. Setup propagates untracked local Atomic settings, `.worktreeinclude` matches, configured directory symlinks, and Husky/native hook configuration. Relative direct outputs are persisted to runner-owned artifact directories before cleanup.

Reusable setup remains cached by canonical repository and target identity within a run, revalidates checkout identity before reuse, and retries transient read-only Git probe timeouts with full command/cwd/timing diagnostics. Durable resume records the original invocation cwd and reusable-worktree metadata so it replays from the original repository context. Temporary outputs remain readable after cleanup because relative outputs are copied to distinct, containment-checked runner artifact roots; `file-only`, absolute outputs, and nonblank explicit `chainDir` behavior are unchanged.
