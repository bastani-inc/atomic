...
Cursor is a deliberate exception to generic workflow fallback resolution because its authenticated live catalog is the only executable authority. Before validating an explicit Cursor `provider/id` string, Cursor `Model` object, or rejection-only bare historical direct ID, workflows await the shared authenticated catalog-discovery lifecycle. Named `workflow({ action: "run" })` dispatch performs this preflight before creating any Run, store entry, or stage session; concurrent named Cursor launches share discovery, while failure or cancellation creates no Run. A matching route is replaced with the exact live row; an unavailable or stale reference fails with reselection guidance and does not advance to configured, current, provider, or model fallbacks. Bare historical IDs such as `composer-2` are tombstones only: a currently returned exact Cursor route wins, otherwise the bare value fails instead of resolving to another provider; explicitly qualify a non-Cursor collision. Non-Cursor IDs and fallback behavior are unchanged and do not wait for Cursor discovery. Builtins do not hard-code account-specific Cursor routes.
