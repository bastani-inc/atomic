...
3. Poll with the default long timeout (600000 ms). Run `live-poll.mjs` again immediately after every event or `--reply`; Codex runs this one-shot poll in the foreground. Never pass a short `--timeout=`. The global bar's **Impeccable mark** dims with a pulsing amber dot when nothing is polling `/poll`; restart `live-poll.mjs` to reconnect.
4. On `generate`: reuse `event.scaffold` when present; read the screenshot if present; load the action's reference; deliver variants; `--reply EVENT_ID done`; poll again. Generate in this thread: you already hold the project's tokens and layout. The overlay preview IS the verification channel; do not screenshot, re-render, or QA variants between generate and accept. Apply craft-floor's contrast, spacing, and type floors by construction as you write; full verification runs once at accept on the chosen variant.
5. On `steer`: read the message and `pageUrl`; do the work; `--reply EVENT_ID steer_done`; poll again. No pickup ack.
...
LOOP:
  ...
  "generate"  → Handle Generate; `--reply EVENT_ID done`; LOOP
  "steer"     → Handle Steer; `--reply EVENT_ID steer_done`; LOOP
  "accept"    → Handle Accept; complete carbonize cleanup if required; LOOP
  "discard"   → Handle Discard; LOOP
  "prefetch"  → Handle Prefetch; LOOP
  "manual_edit_apply" → Handle Manual Edit Apply; `--reply EVENT_ID done|partial|error`; LOOP
  "variant_mount_failed" → Fix the variant files; `--reply EVENT_ID done --file <path>`; LOOP
  "timeout"   → LOOP
  "exit"      → break → Cleanup
```
