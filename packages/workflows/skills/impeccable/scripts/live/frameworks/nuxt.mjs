...
import { findConfigFile, resolveProjectPath, writeProjectFileAtomic } from './detect-utils.mjs';
...
export function applyNuxtLiveAdapter({ cwd = process.cwd(), port, token, project = detectNuxtProject(cwd) }) {
  ...
  let absFile;
  try {
    absFile = resolveProjectPath(cwd, project.pluginFile);
  } catch (error) {
    return { file: project.pluginFile, error: 'unsafe_nuxt_plugin_path', hint: error.message };
  }
  ...
  if (content !== existing) writeProjectFileAtomic(cwd, project.pluginFile, content);
  return {
    file: project.pluginFile,
    inserted: true,
...
export function removeNuxtLiveAdapter({ cwd = process.cwd(), project = detectNuxtProject(cwd) }) {
  ...
  try {
    ...
    return { file: project.pluginFile, removed: false, error: 'unsafe_nuxt_plugin_path', hint: error.message };
  }
