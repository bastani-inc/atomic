...
import { resolveProjectPath, writeProjectFileAtomic } from './detect-utils.mjs';
...
export function injectJournalPath(cwd = process.cwd()) {
  // Validate the path against the canonical root, but retain the caller's
  // lexical root in output so /tmp aliases do not leak into user-facing paths.
  resolveProjectPath(cwd, INJECT_JOURNAL_RELPATH);
...
function writeInjectJournal(cwd, journal) {
  return writeProjectFileAtomic(cwd, INJECT_JOURNAL_RELPATH, JSON.stringify(journal, null, 2) + '\n');
}
...
function normalizeRel(cwd, rel) {
  return resolveProjectPath(cwd, String(rel || '')).split(path.sep).join('/');
}
...
function healArtifact(cwd, artifact, undoers) {
  let abs;
  try {
    abs = resolveProjectPath(cwd, artifact.path, { kind: 'file' });
  } catch {
  ...
  if (artifact.kind === 'created') {
    ...
    if (artifact.pruneTo !== undefined) {
      let pruneRoot;
      try {
        pruneRoot = artifact.pruneTo
          ? resolveProjectPath(cwd, artifact.pruneTo, { kind: 'directory' })
          : fs.realpathSync(cwd);
      } catch {
        pruneRoot = null;
      }
      if (pruneRoot) pruneEmptyDirs(path.dirname(abs), pruneRoot);
    }
  ...
  if (artifact.kind === 'patched') {
    ...
    try { writeProjectFileAtomic(cwd, artifact.path, next); } catch { return null; }
...
export function healInjectJournal(cwd = process.cwd(), { keep = [], undoers = PATCH_UNDOERS } = {}) {
  ...
  for (const rel of keep) {
    try { keepSet.add(normalizeRel(cwd, rel)); } catch { /* unsafe paths are never kept */ }
  }
  const healed = [];
  const kept = [];
  ...
  for (const artifact of journal.artifacts) {
    ...
    try { normalizedArtifact = normalizeRel(cwd, artifact.path); } catch { continue; }
    if (keepSet.has(normalizedArtifact)) {
      kept.push(artifact);
      continue;
    }
