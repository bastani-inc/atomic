...
rem Atomic ships this skill inside its own release and cannot self-update, so
rem the engine's daily update check and its anonymous concept-roll ping stay off.
if not defined IMPECCABLE_NO_TELEMETRY set "IMPECCABLE_NO_TELEMETRY=1"
if not defined IMPECCABLE_NO_UPDATE_CHECK set "IMPECCABLE_NO_UPDATE_CHECK=1"
set "arch=x64"
