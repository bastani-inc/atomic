...
// The directory is created up front so `agent-browser record start <path>` can
...
export function renderQaE2eVideoGuidance(qaVideoPath: string): string {
  return [
    E2E_VERIFICATION_GUIDANCE,
    `For a browser UI scenario, record the QA pass using agent-browser record start ${qaVideoPath} after opening the browser, exercise the complete scenario, then agent-browser record stop. For terminal or desktop/simulator scenarios, use the domain-appropriate tool from the guidance above, not a browser substitute. Save compatible video to exactly ${qaVideoPath} when produced; do not rename an incompatible format to look like a video.`,
