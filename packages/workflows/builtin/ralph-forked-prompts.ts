...
export function renderForkedResearchPrompt(args: {
  ...
  return taggedPrompt([
    ...
    [
      "research_artifact",
      [
        "Return the rewritten research report for this iteration as your final message in this stage's own assistant turn, before any admitted async completion. The runner's output boundary persists the last non-empty assistant turn before the first admitted external message; if no assistant text precedes that admission, it selects the largest non-empty post-admission assistant turn by UTF-8 bytes, preferring the later turn on a tie, because no stage-owned candidate exists. The companion transcript preserves the full stage, including admitted async results and later turns; downstream implementation and review stages read the artifact first and search the transcript when they need omitted detail.",
        `Do not write ${args.researchPath} yourself. The runner owns that artifact path and saves the nominated turn there; return complete findings in this stage's own assistant turn before any admitted async completion, while the companion transcript retains tool output and admitted async results. Skill-owned notes under research/docs/ and research/web/ are unaffected.`,
