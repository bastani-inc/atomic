...
export function renderForkedResearchPrompt(args: {
  ...
  return taggedPrompt([
    ...
    [
      "research_artifact",
      [
        "Return the rewritten research report for this iteration as your final message in this stage's own assistant turn. Around an admitted async completion, the runner defaults to the last pre-admission assistant text but replaces it with a post-admission candidate that is at least 26 UTF-8 bytes and 3:2 larger; without pre-admission text it uses the most recent post-admission candidate at that floor. Do not leave a short preamble as the only substantial candidate. The companion transcript preserves the full stage, including admitted async results and later turns; downstream implementation and review stages read the artifact first and search the transcript when they need omitted detail.",
        `Do not write ${args.researchPath} yourself. The runner owns that artifact path and saves the nominated turn there; return complete findings in this stage's own final assistant message, while the companion transcript retains tool output and admitted async results. Skill-owned notes under research/docs/ and research/web/ are unaffected.`,
