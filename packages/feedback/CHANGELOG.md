...
- Fixed repeated home-directory segments within one path retaining a second account name after privacy scrubbing ([#2799](https://github.com/bastani-inc/atomic/issues/2799)).
- Fixed numeric feedback titles being accepted through implicit conversion in npm-installed Atomic running under Node. That distribution now reports `title: must be string`. Downloaded binaries and source-checkout runs can still convert numeric titles to text; pass text for every field and review the prepared draft ([#2799](https://github.com/bastani-inc/atomic/issues/2799)).
