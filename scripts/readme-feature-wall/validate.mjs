...
//
// Checks, in order:
//   1  required set, count, and order: 27 core lessons, three workflow-use paths, and A.1-A.10
//   2  README table shape: one wall, 40 rows, order, links, picture markup, alt
...
const REQUIRED = [
	...
	["W.1", "Launch a workflow in plain English"],
	["W.2", "Run a workflow with typed inputs"],
	["W.3", "Inspect and control workflows"],
...
// ---------------------------------------------------------------- 1. lessons
{
	...
	if (!failures.some((f) => f.startsWith("lessons")))
		ok(
			"lessons",
			"40 rows in exact order: 27 core lessons, three workflow-use paths, and Extras A.1 through A.10, with unique capture and media mappings",
		);
...
// ------------------------------------------------------------- 2. README wall
{
	...
	if (startCount !== 1 || endCount !== 1) {
		...
		MANIFEST.lessons.forEach((l, i) => {
			const row = rows[i];
			if (!row) return;
			const link = `${COURSE_URL}${l.anchor}`;
			if (!row.includes(link)) fail("readme", `${l.id}: row is missing its crash-course link ${link}`);
		...
		if (!failures.some((f) => f.startsWith("readme"))) {
			ok("readme", "one wall, 40 two-column rows, in order, each with link, <picture>, GIF, poster, alt text");
		}
	}
}
...
const gifPaths = [];
{
	...
	for (const l of MANIFEST.lessons) {
		...
		const longestHold = timing.reduce(
			(longest, frame) => (frame.duration > longest.duration ? frame : longest),
			{ start: 0, duration: 0 },
		);
