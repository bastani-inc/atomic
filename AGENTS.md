...
- `scripts/run-flaky-test-suite.ts` scores every duration Bun prints against that test's effective timeout: warn at 40 % of budget, fail the step at 70 %. It always writes the per-test duration table to `.ci-diagnostics/<suite>-durations.md`, on green runs too. If it fails your test, make the test faster or justify a structural explicit timeout — do not raise the shared default.
