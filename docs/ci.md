...
The test matrix checks out with `lfs: true` (two coding-agent test fixtures are LFS objects) and the default shallow `fetch-depth`; no test-job step reads git history. [Git checkout caching](https://docs.blacksmith.sh/blacksmith-caching/git-checkout-caching) keeps a full mirror server-side while the workspace respects the shallow depth, so the shallow fetch speeds up both the cached path and the standard-clone fallback. A `Blacksmith cache setup failed, using standard checkout: ... failed to expose and mount sticky disk` warning is the beta git-mirror feature degrading gracefully on Blacksmith's side — the run continues on a normal clone; it is not caused by (or fixable from) workflow YAML, and recurring instances belong in a Blacksmith support report.
