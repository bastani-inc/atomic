...
Protected publisher run `29694686010` used the workflow snapshot that existed when it started. GitHub documents that a rerun keeps the original event's [`GITHUB_SHA` and `GITHUB_REF`](https://docs.github.com/en/actions/how-tos/manage-workflow-runs/re-run-workflows-and-jobs), and that each run uses the [workflow version present at the event-associated ref/SHA](https://docs.github.com/en/actions/concepts/workflows-and-actions/workflows#workflow-triggers). A normal rerun therefore cannot adopt a later fix from `main`. GitHub also documents that [`GITHUB_RUN_ID` stays fixed while `GITHUB_RUN_ATTEMPT` increments](https://docs.github.com/en/actions/reference/workflows-and-actions/variables#default-environment-variables); attempt 2 cannot be replayed, and another rerun of historical run `29529182569` would create the forbidden attempt 3 with the old tag-era YAML.

Merging the publisher-fix PR does **not** activate recovery: the exact marker path is intentionally absent from that PR. After the fix is merged and reviewed, publication requires a separate protected-main PR that adds only `.github/recovery/0.9.10-alpha.1.json` with these exact bytes:
...
That protected-main marker addition is the explicit post-merge publication authorization. It starts the existing `publish-release.yml` identity without inputs. The validator requires repository `bastani-inc/atomic` (`1081638046`), `refs/heads/main`, the exact protected workflow ref, `github.workflow_sha == github.sha`, and run attempt exactly 1. It also requires both the push's `before` and event commits to be locally available, proves the former is an ancestor of the latter, requires marker absence at `before`, and compares the marker's event-commit bytes exactly before accepting the compiled tag `0.9.10-alpha.1` / release SHA `88c11adcdddcf5245b7b04dd3d2912c7531906fe`. It then runs every ordinary tag, release-base, deterministic-tree, artifact, checksum, package, OIDC, and least-privilege check. There are no tag/SHA inputs, no `workflow_dispatch`/`repository_dispatch` recovery trigger, and no route for another workflow, tag, release SHA, or rerun attempt.

Do not rerun either historical workflow, delete/move/recreate the tag, dispatch a publisher, or change npm settings as part of the fix implementation. After successful publication, immediately request a cleanup PR that removes the recovery `push` arm, recovery constants/branch in `scripts/verify-publish-context.ts`, its tests, and the marker file in the same change. Because the pushed cleanup version no longer contains the recovery event arm, deleting the marker cannot start another recovery run. Keep the normal inert signal and protected `workflow_run` publisher intact.
