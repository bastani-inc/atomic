...
Approving a fork run lets the pull request's own workflow files run on Namespace. Before approving:

1. Inspect the pull request's diff for any change under `.github/`, especially `runs-on`, new or renamed workflows, changed triggers (`on:`), and `permissions:`.
...
A further mitigation is a workspace-level workload-permission policy in Namespace, which would limit workload tokens whatever runner label a job names. It has not been adopted; whether to set one is a decision for the workspace owner, recorded under [Follow-ups](#follow-ups).
...
4. **Workspace-level workload policy.** Decide whether to set a workspace-level workload-permission policy in Namespace, so that a fork job on an inline `nscloud-*` label would not get Permissive access either. Until then, approval is the only barrier against a fork pull request that edits `.github/workflows` ([Approving fork workflow runs](#approving-fork-workflow-runs)).
