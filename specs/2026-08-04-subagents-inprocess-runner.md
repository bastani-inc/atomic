...
- **Integration — runner parity:** existing foreground suites (single/parallel, fail-fast, interrupt, model fallback, artifacts, history) rewritten against typed statuses; `ps` probe asserts **zero** child processes in all modes, including `async: true`.
