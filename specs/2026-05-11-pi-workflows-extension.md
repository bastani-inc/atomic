...
| Slash commands                  | `/run`, `/parallel`, `/subagents-doctor`                              | `/workflow`, `/workflow:<name>`, `/workflow list/quit/status/resume/inputs`, `/workflows-doctor` |
| Clarify UI                      | None                                                                  | `ctx.ui.custom({overlay:true})` preview before run                                               |
...
| Build on top of pi-subagents' delegation modes instead of new tool  | pi-subagents already does parallel fan-out.        | Its modes are single and flat parallel; workflows need fan-in topology, programmatic input schemas, and per-stage HIL beyond pi-subagents' frontmatter. |
