...
| Native wheel inside/outside/boundaries | Corrected gate positions transcript at row 15 and asserts it stays there at both boundaries and without overflow, locally and over serialized remote transport; outside control still moves transcript to 14/16. See review repair below |
...
Candidate `89671c2311c061aac8e6a45f7c92ce836061822c` had a valid SSH signature and clean worktree. Base `df5dd4bca54725ae9f36b14411de1884ce5bb43e` is an ancestor; the diff has no workflow adapter edits. Correction: this initial verification missed a runtime containment defect. Its earlier readiness statement is withdrawn. The consolidated review and repair below supersede the initial boundary claims; adapter progression still requires the parent's independent gate.
...
All three review findings share one root cause: completion-reviewer `[P2] Prevent boundary wheel events from scrolling the transcript`, evidence-reviewer `[P1] Contain boundary wheel input before transcript fallback`, and risk-reviewer `[P1] Contain remaining wheel delta before transcript fallback`.

The pinned pi-tui wheel router calls the native hit-tested widget's `scrollBy`, breaks its hit loop for `overscroll: "contain"`, then forwards any returned remainder to the primary transcript. The original gate masked downward leakage by leaving the transcript at its bottom and omitted the upward transcript assertion. Earlier statements that those tests proved boundary containment were incorrect.

The repair overrides only `ScrollWidget.scrollBy`: native clamping/movement runs unchanged, then the widget returns zero unused delta. Actual native layout still decides which widget receives input. No global interception, dependency patch, API, adapter or editor-binding change was added.

Durable red/green evidence:
- `bun /tmp/widget-host-completion-boundaries.ts` initially exited 1 with transcript 15 to 14 at non-overflow/up and top/up, and 15 to 16 at bottom/down. After repair all three preserve 15 and exit 0.
- `npx vitest --run --project unit test/unit/reactive-widget-scroll-contract.test.ts` with the strengthened tests and the override absent exited 1: three tests failed, transcript 33/14/14 instead of 15. `/tmp/widget-containment-all-red.log` retains the output. Restoring the override passes all five tests. The tests now exercise local clipped top/bottom and non-overflow in both directions, plus serialized-remote top/bottom, content shrink to non-overflow and outside control. Remote outside input still moves the midrange transcript to 14/16 without moving the widget.
- The exact root and package commands in the validation section passed again: 64 tests across 10 files and 13 tests across three files. `npm run check`, `npm run build`, and docs validation passed, including both typechecks and shrinkwrap. Logs: `/tmp/widget-containment-{root,package,check,build,docs}.log`.
- Scoped `qlty smells` found no findings; `qlty metrics --functions` gives the new `scrollBy` cognitive complexity 0 and unchanged maximum 4. Existing supplemental configuration was preserved. No plugin lint or whole-repository metrics claim is made.

Fresh macOS tmux containment check used the actual source CLI and an observed isolated RPC child, with `/tmp/widget-containment-terminal-extension.ts` in dedicated session `widget-containment-4516`, 80x24. The temporary fixture's attempted custom transcript seed did not appear, so it is not evidence; the scrollable startup/reload transcript supplied the midrange control instead. Ten injected up-wheel events at row 2 moved away from its end. An up-wheel at widget row 20 with visible widget rows 0 to 4 left the entire capture byte-identical. An outside up-wheel changed the transcript. Thirty down-wheels inside reached widget rows 25 to 29; another inside down-wheel again left the entire capture byte-identical, while outside down-wheel changed the transcript. Typing `typing intact` remained in the editor. Captures: `/tmp/widget-containment-terminal-{before,top,outside-up,bottom-before,bottom-after,outside-down,typing}.txt`. Both `cmp` boundary checks passed, and outside controls differed. Two Ctrl+C inputs closed the dedicated session. No model call was requested; existing no-model/Herdr notices were not repaired.

Terminal bytes were injected through tmux, not physical mouse forwarding. Non-overflow containment is proven by the durable local/remote runtime gate, not this live terminal scenario. Linux/Windows and Option/Alt encoding remain unverified. No contract amendments received; only the separately gated adapter and reviewer approval remain deferred.
